...
public interface DeviceFilter<D extends Parcelable> extends Parcelable {
    ...
    private protected String getDeviceDisplayName(D d);

    synchronized int getMediumType();

    private protected boolean matches(D d);

    static synchronized <D extends Parcelable> boolean matches(DeviceFilter<D> filter, D device) {
