...
public class ChooseTypeAndAccountActivity extends Activity implements AccountManagerCallback<Bundle> {
    ...
    public synchronized void onCancelButtonClicked(View view) {
        onBackPressed();
    }

    public synchronized void onOkButtonClicked(View view) {
        if (this.mSelectedItemIndex == this.mPossiblyVisibleAccounts.size()) {
            startChooseAccountTypeActivity();
    ...
    protected synchronized void runAddAccountForAuthenticator(String type) {
        if (Log.isLoggable(TAG, 2)) {
    ...
    private synchronized void setNonLabelThemeAndCallSuperCreate(Bundle savedInstanceState) {
        setTheme(16974132);
        super.onCreate(savedInstanceState);
    }

    private synchronized void onAccountSelected(Account account) {
        Log.d(TAG, "selected account " + account);
        setResultAndFinish(account.name, account.type);
    }

    private synchronized void setResultAndFinish(String accountName, String accountType) {
    ...
    private synchronized void startChooseAccountTypeActivity() {
        if (Log.isLoggable(TAG, 2)) {
    ...
    private synchronized int getItemIndexToSelect(ArrayList<Account> accounts, String selectedAccountName, boolean selectedAddNewAccount) {
    ...
    private synchronized String[] getListOfDisplayableOptions(ArrayList<Account> accounts) {
    ...
    private synchronized LinkedHashMap<Account, Integer> getAcceptableAccountChoices(AccountManager accountManager) {
    ...
    private synchronized Set<String> getReleventAccountTypes(Intent intent) {
    ...
    private synchronized Set<Account> getAllowableAccountSet(Intent intent) {
    ...
    private synchronized void overrideDescriptionIfSupplied(String descriptionOverride) {
        TextView descriptionView = (TextView) findViewById(16908937);
    ...
    private final synchronized void populateUIAccountList(String[] listItems) {
