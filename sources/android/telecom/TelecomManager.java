...
public class TelecomManager {
    ...
    private protected static final String EXTRA_IS_HANDOVER = "android.telecom.extra.IS_HANDOVER";
    ...
    private protected static final int TTY_MODE_OFF = 0;
    ...
    public synchronized TelecomManager(Context context) {
        this(context, null);
    }

    public synchronized TelecomManager(Context context, ITelecomService telecomServiceImpl) {
    ...
    private protected PhoneAccountHandle getUserSelectedOutgoingPhoneAccount() {
        try {
            if (isServiceConnected()) {
    ...
    private protected void setUserSelectedOutgoingPhoneAccount(PhoneAccountHandle accountHandle) {
        try {
            if (isServiceConnected()) {
    ...
    private protected PhoneAccountHandle getSimCallManager(int userId) {
        try {
            if (isServiceConnected()) {
    ...
    private protected List<PhoneAccountHandle> getCallCapablePhoneAccounts(boolean includeDisabledAccounts) {
        try {
            if (isServiceConnected()) {
    ...
    public synchronized void clearAccountsForPackage(String packageName) {
        try {
    ...
    private protected boolean setDefaultDialer(String packageName) {
        try {
            if (isServiceConnected()) {
    ...
    private protected int getCurrentTtyMode() {
        try {
            if (isServiceConnected()) {
