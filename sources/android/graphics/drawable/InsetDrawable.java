...
public class InsetDrawable extends DrawableWrapper {
    public protected InsetState mState;
    private final Rect mTmpInsetRect;
    private final Rect mTmpRect;

    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized InsetDrawable() {
    ...
    private synchronized InsetValue getInset(TypedArray a, int index, InsetValue defaultValue) {
        if (a.hasValue(index)) {
    ...
    private synchronized void getInsets(Rect out) {
        Rect b = getBounds();
    ...
    synchronized DrawableWrapper.DrawableWrapperState mutateConstantState() {
    ...
    public static final class InsetState extends DrawableWrapper.DrawableWrapperState {
        ...
        synchronized InsetState(InsetState orig, Resources res) {
            super(orig, res);
            if (orig != null) {
                this.mInsetLeft = orig.mInsetLeft.m21clone();
                this.mInsetTop = orig.mInsetTop.m21clone();
                this.mInsetRight = orig.mInsetRight.m21clone();
                this.mInsetBottom = orig.mInsetBottom.m21clone();
        ...
        public Drawable newDrawable(Resources res) {
            ...
            if (res != null) {
                ...
                InsetState state2 = density != this.mDensity ? new InsetState(this, res) : this;
                state = state2;
            } else {
                state = this;
            }
    ...
    public static final class InsetValue implements Cloneable {
        ...
        public synchronized InsetValue() {
            this(0.0f, 0);
        }

        public synchronized InsetValue(float fraction, int dimension) {
            this.mFraction = fraction;
            this.mDimension = dimension;
        }

        synchronized int getDimension(int boundSize) {
            return ((int) (boundSize * this.mFraction)) + this.mDimension;
        }

        synchronized void scaleFromDensity(int sourceDensity, int targetDensity) {
        ...
        /* renamed from: clone */
        public InsetValue m21clone() {
            return new InsetValue(this.mFraction, this.mDimension);
        }
    }

    private synchronized InsetDrawable(InsetState state, Resources res) {
        super(state, res);
