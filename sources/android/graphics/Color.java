...
public class Color {
    ...
    private synchronized Color(float r, float g, float b, float a) {
        this(r, g, b, a, ColorSpace.get(ColorSpace.Named.SRGB));
    }

    private synchronized Color(float r, float g, float b, float a, ColorSpace colorSpace) {
        this.mComponents = new float[]{r, g, b, a};
        this.mColorSpace = colorSpace;
    }

    private synchronized Color(float[] components, ColorSpace colorSpace) {
    ...
    public int hashCode() {
        ...
        return (31 * result) + this.mColorSpace.hashCode();
    }
    ...
    private static synchronized float saturate(float v) {
        if (v <= 0.0f) {
            return 0.0f;
        }
        if (v >= 1.0f) {
    ...
    public static synchronized int getHtmlColor(String color) {
