...
public class ApplicationPackageManager extends PackageManager {
    ...
    public protected final IPackageManager mPM;
    @GuardedBy("mLock")
    ...
    synchronized UserManager getUserManager() {
    ...
    public synchronized PackageInfo getPackageInfoAsUser(String packageName, int flags, int userId) throws PackageManager.NameNotFoundException {
        try {
    ...
    @Override // android.content.pm.PackageManager
    public synchronized Intent getCarLaunchIntentForPackage(String packageName) {
        return getLaunchIntentForPackageAndCategory(packageName, Intent.CATEGORY_CAR_LAUNCHER);
    }

    private synchronized Intent getLaunchIntentForPackageAndCategory(String packageName, String category) {
        Intent intentToResolve = new Intent(Intent.ACTION_MAIN);
    ...
    public synchronized int getPackageUidAsUser(String packageName, int userId) throws PackageManager.NameNotFoundException {
        return getPackageUidAsUser(packageName, 0, userId);
    }

    public synchronized int getPackageUidAsUser(String packageName, int flags, int userId) throws PackageManager.NameNotFoundException {
        try {
    ...
    public synchronized ApplicationInfo getApplicationInfoAsUser(String packageName, int flags, int userId) throws PackageManager.NameNotFoundException {
        try {
    ...
    private static synchronized ApplicationInfo maybeAdjustApplicationInfo(ApplicationInfo info) {
    ...
    @Override // android.content.pm.PackageManager
    public synchronized List<SharedLibraryInfo> getSharedLibrariesAsUser(int flags, int userId) {
        try {
    ...
    private protected boolean shouldShowRequestPermissionRationale(String permission) {
        try {
    ...
    public synchronized int getUidForSharedUser(String sharedUserName) throws PackageManager.NameNotFoundException {
        try {
    ...
    @Override // android.content.pm.PackageManager
    public synchronized int getInstantAppCookieMaxSize() {
    ...
    private protected boolean setInstantAppCookie(byte[] cookie) {
        try {
    ...
    public synchronized ResolveInfo resolveActivityAsUser(Intent intent, int flags, int userId) {
        try {
    ...
    public synchronized List<ResolveInfo> queryIntentActivitiesAsUser(Intent intent, int flags, int userId) {
        try {
    ...
    public synchronized List<ResolveInfo> queryBroadcastReceiversAsUser(Intent intent, int flags, int userId) {
        try {
    ...
    @Override // android.content.pm.PackageManager
    public synchronized ResolveInfo resolveServiceAsUser(Intent intent, int flags, int userId) {
        try {
    ...
    public synchronized List<ResolveInfo> queryIntentServicesAsUser(Intent intent, int flags, int userId) {
        try {
    ...
    public synchronized List<ResolveInfo> queryIntentContentProvidersAsUser(Intent intent, int flags, int userId) {
        try {
    ...
    public synchronized ProviderInfo resolveContentProviderAsUser(String name, int flags, int userId) {
        try {
    ...
    @Override // android.content.pm.PackageManager
    public synchronized List<ProviderInfo> queryContentProviders(String processName, int uid, int flags, String metaDataKey) {
        try {
    ...
    public Drawable getDrawable(String packageName, int resId, ApplicationInfo appInfo) {
        ...
        if (resId != 0) {
            try {
                ...
                Drawable dr = r.getDrawableForDensity(resId, FeatureOption.FO_APP_ICON_DENSITY, null);
                if (dr != null) {
    ...
    public Drawable getUserBadgedIcon(Drawable icon, UserHandle user) {
        ...
        Drawable badge = new LauncherIcons(this.mContext).getBadgeDrawable(R.drawable.ic_corp_icon_badge_case, getUserBadgeColor(user));
    ...
    private synchronized int getUserBadgeColor(UserHandle user) {
        return IconDrawableFactory.getUserBadgeColor(getUserManager(), user.getIdentifier());
    }

    public synchronized Drawable getUserBadgeForDensity(UserHandle user, int density) {
    ...
    public synchronized Drawable getUserBadgeForDensityNoBackground(UserHandle user, int density) {
    ...
    private synchronized Drawable getDrawableForDensity(int drawableId, int density) {
        if (density <= 0) {
    ...
    private synchronized Drawable getManagedProfileIconForDensity(UserHandle user, int drawableId, int density) {
        if (isManagedProfile(user.getIdentifier())) {
    ...
    public synchronized Resources getResourcesForApplicationAsUser(String appPackageName, int userId) throws PackageManager.NameNotFoundException {
        if (userId < 0) {
    ...
    public private protected static void configurationChanged() {
        synchronized (sSync) {
            sIconCache.clear();
            sStringCache.clear();
        }
    ...
    public private ApplicationPackageManager(ContextImpl context, IPackageManager pm) {
        this.mContext = context;
        this.mPM = pm;
    }

    private synchronized Drawable getCachedIcon(ResourceName name) {
        synchronized (sSync) {
    ...
    private synchronized void putCachedIcon(ResourceName name, Drawable dr) {
        synchronized (sSync) {
    ...
    public static synchronized void handlePackageBroadcast(int cmd, String[] pkgList, boolean hasPkgInfo) {
    ...
    public static final class ResourceName {
        ...
        synchronized ResourceName(String _packageName, int _iconId) {
            this.packageName = _packageName;
            this.iconId = _iconId;
        }

        synchronized ResourceName(ApplicationInfo aInfo, int _iconId) {
            this(aInfo.packageName, _iconId);
        }

        synchronized ResourceName(ComponentInfo cInfo, int _iconId) {
            this(cInfo.applicationInfo.packageName, _iconId);
        }

        synchronized ResourceName(ResolveInfo rInfo, int _iconId) {
    ...
    private synchronized CharSequence getCachedString(ResourceName name) {
        synchronized (sSync) {
    ...
    private synchronized void putCachedString(ResourceName name, CharSequence cs) {
        synchronized (sSync) {
    ...
    public synchronized int installExistingPackageAsUser(String packageName, int userId) throws PackageManager.NameNotFoundException {
        return installExistingPackageAsUser(packageName, 0, userId);
    }

    private synchronized int installExistingPackageAsUser(String packageName, int installReason, int userId) throws PackageManager.NameNotFoundException {
        try {
    ...
    public synchronized int getMoveStatus(int moveId) {
        try {
    ...
    public synchronized void registerMoveCallback(PackageManager.MoveCallback callback, Handler handler) {
    ...
    public synchronized void unregisterMoveCallback(PackageManager.MoveCallback callback) {
        synchronized (this.mDelegates) {
    ...
    public synchronized int movePackage(String packageName, VolumeInfo vol) {
        String volumeUuid;
        try {
    ...
    private protected VolumeInfo getPackageCurrentVolume(ApplicationInfo app) {
    ...
    @VisibleForTesting
    protected synchronized VolumeInfo getPackageCurrentVolume(ApplicationInfo app, StorageManager storage) {
        if (app.isInternal()) {
    ...
    public synchronized List<VolumeInfo> getPackageCandidateVolumes(ApplicationInfo app) {
    ...
    @VisibleForTesting
    protected synchronized List<VolumeInfo> getPackageCandidateVolumes(ApplicationInfo app, StorageManager storageManager, IPackageManager pm) {
    ...
    @VisibleForTesting
    protected synchronized boolean isForceAllowOnExternal(Context context) {
        return Settings.Global.getInt(context.getContentResolver(), Settings.Global.FORCE_ALLOW_ON_EXTERNAL, 0) != 0;
    }

    @VisibleForTesting
    protected synchronized boolean isAllow3rdPartyOnInternal(Context context) {
        return context.getResources().getBoolean(R.bool.config_allow3rdPartyAppOnInternal);
    }

    private synchronized boolean isPackageCandidateVolume(ContextImpl context, ApplicationInfo app, VolumeInfo vol, IPackageManager pm) {
    ...
    @Override // android.content.pm.PackageManager
    public synchronized int movePrimaryStorage(VolumeInfo vol) {
        String volumeUuid;
        try {
            if (VolumeInfo.ID_PRIVATE_INTERNAL.equals(vol.id)) {
                volumeUuid = StorageManager.UUID_PRIVATE_INTERNAL;
    ...
    @Override // android.content.pm.PackageManager
    public synchronized VolumeInfo getPrimaryStorageCurrentVolume() {
        StorageManager storage = (StorageManager) this.mContext.getSystemService(StorageManager.class);
    ...
    @Override // android.content.pm.PackageManager
    public synchronized List<VolumeInfo> getPrimaryStorageCandidateVolumes() {
        StorageManager storage = (StorageManager) this.mContext.getSystemService(StorageManager.class);
    ...
    private static synchronized boolean isPrimaryStorageCandidateVolume(VolumeInfo vol) {
        if (VolumeInfo.ID_PRIVATE_INTERNAL.equals(vol.getId())) {
            return true;
        }
    ...
    private protected void deletePackage(String packageName, IPackageDeleteObserver observer, int flags) {
        deletePackageAsUser(packageName, observer, flags, this.mContext.getUserId());
    }

    public synchronized void deletePackageAsUser(String packageName, IPackageDeleteObserver observer, int flags, int userId) {
        try {
    ...
    public synchronized void clearApplicationUserData(String packageName, IPackageDataObserver observer) {
        try {
    ...
    public synchronized void deleteApplicationCacheFiles(String packageName, IPackageDataObserver observer) {
        try {
    ...
    public synchronized void deleteApplicationCacheFilesAsUser(String packageName, int userId, IPackageDataObserver observer) {
        try {
    ...
    public synchronized void freeStorageAndNotify(String volumeUuid, long idealStorageSize, IPackageDataObserver observer) {
        try {
    ...
    public synchronized void freeStorage(String volumeUuid, long freeStorageSize, IntentSender pi) {
        try {
    ...
    public synchronized boolean isPackageSuspendedForUser(String packageName, int userId) {
        try {
    ...
    private protected void getPackageSizeInfoAsUser(String packageName, int userHandle, IPackageStatsObserver observer) {
    ...
    public synchronized void addPreferredActivityAsUser(IntentFilter filter, int match, ComponentName[] set, ComponentName activity, int userId) {
        try {
    ...
    public synchronized void replacePreferredActivity(IntentFilter filter, int match, ComponentName[] set, ComponentName activity) {
        try {
    ...
    public synchronized void replacePreferredActivityAsUser(IntentFilter filter, int match, ComponentName[] set, ComponentName activity, int userId) {
        try {
    ...
    public synchronized ComponentName getHomeActivities(List<ResolveInfo> outActivities) {
        try {
    ...
    public synchronized void flushPackageRestrictionsAsUser(int userId) {
        try {
    ...
    public synchronized boolean setApplicationHiddenSettingAsUser(String packageName, boolean hidden, UserHandle user) {
        try {
    ...
    public synchronized boolean getApplicationHiddenSettingAsUser(String packageName, UserHandle user) {
        try {
    ...
    public synchronized KeySet getKeySetByAlias(String packageName, String alias) {
    ...
    public synchronized KeySet getSigningKeySet(String packageName) {
        Preconditions.checkNotNull(packageName);
        try {
    ...
    public synchronized boolean isSignedBy(String packageName, KeySet ks) {
        Preconditions.checkNotNull(packageName);
    ...
    public synchronized boolean isSignedByExactly(String packageName, KeySet ks) {
        Preconditions.checkNotNull(packageName);
        Preconditions.checkNotNull(ks);
        try {
    ...
    @Override // android.content.pm.PackageManager
    public synchronized VerifierDeviceIdentity getVerifierDeviceIdentity() {
        try {
    ...
    public synchronized boolean isPackageAvailable(String packageName) {
        try {
    ...
    public synchronized void addCrossProfileIntentFilter(IntentFilter filter, int sourceUserId, int targetUserId, int flags) {
        try {
    ...
    public synchronized void clearCrossProfileIntentFilters(int sourceUserId) {
        try {
    ...
    public synchronized Drawable loadItemIcon(PackageItemInfo itemInfo, ApplicationInfo appInfo) {
        Bitmap bitmap;
        if (FeatureOption.FO_APP_ICON_OVERLAY_ENABLED && (bitmap = xpPackageManager.getOverrideAppIcon(itemInfo.packageName)) != null) {
    ...
    public synchronized Drawable loadUnbadgedItemIcon(PackageItemInfo itemInfo, ApplicationInfo appInfo) {
        if (itemInfo.showUserIcon != -10000) {
    ...
    private synchronized Drawable getBadgedDrawable(Drawable drawable, Drawable badgeDrawable, Rect badgeLocation, boolean tryBadgeInPlace) {
        Bitmap bitmap;
    ...
    private synchronized boolean isManagedProfile(int userId) {
    ...
    private static class MoveCallbackDelegate extends IPackageMoveObserver.Stub implements Handler.Callback {
        ...
        public synchronized MoveCallbackDelegate(PackageManager.MoveCallback callback, Looper looper) {
        ...
        public synchronized void onCreated(int moveId, Bundle extras) {
        ...
        @Override // android.content.pm.IPackageMoveObserver
        public synchronized void onStatusChanged(int moveId, int status, long estMillis) {
            SomeArgs args = SomeArgs.obtain();
            args.argi1 = moveId;
            args.argi2 = status;
    ...
    public class OnPermissionsChangeListenerDelegate extends IOnPermissionsChangeListener.Stub implements Handler.Callback {
        ...
        public synchronized void onPermissionsChanged(int uid) {
    ...
    @Override // android.content.pm.PackageManager
    public synchronized String getInstantAppAndroidId(String packageName, UserHandle user) {
        try {
    ...
    private static class DexModuleRegisterResult {
        ...
        private synchronized DexModuleRegisterResult(String dexModulePath, boolean success, String message) {
    ...
    private static class DexModuleRegisterCallbackDelegate extends IDexModuleRegisterCallback.Stub implements Handler.Callback {
        ...
        synchronized DexModuleRegisterCallbackDelegate(PackageManager.DexModuleRegisterCallback callback) {
    ...
    @Override // android.content.pm.PackageManager
    public synchronized String getSystemTextClassifierPackageName() {
        try {
    ...
    @Override // android.content.pm.PackageManager
    public synchronized boolean isPackageStateProtected(String packageName, int userId) {
        try {
