...
public abstract class BlobBackupHelper implements BackupHelper {
    ...
    protected abstract synchronized void applyRestoredPayload(String str, byte[] bArr);

    protected abstract synchronized byte[] getBackupPayload(String str);
    ...
    private synchronized ArrayMap<String, Long> readOldState(ParcelFileDescriptor oldStateFd) {
    ...
    private synchronized void writeBackupState(ArrayMap<String, Long> state, ParcelFileDescriptor stateFile) {
        try {
    ...
    private synchronized byte[] deflate(byte[] data) {
        if (data == null) {
            return null;
        }
        try {
    ...
    private synchronized byte[] inflate(byte[] compressedData) {
    ...
    private synchronized long checksum(byte[] buffer) {
        if (buffer != null) {
            try {
                CRC32 crc = new CRC32();
