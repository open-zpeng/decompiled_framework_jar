...
public class FragmentBreadCrumbs extends ViewGroup implements FragmentManager.OnBackStackChangedListener {
    ...
    public synchronized FragmentBreadCrumbs(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private synchronized BackStackRecord createBackStackEntry(CharSequence title, CharSequence shortTitle) {
        if (title == null) {
            return null;
        }
    ...
    private synchronized int getPreEntryCount() {
        return (this.mTopEntry != null ? 1 : 0) + (this.mParentEntry != null ? 1 : 0);
    }

    private synchronized FragmentManager.BackStackEntry getPreEntry(int index) {
    ...
    synchronized void updateCrumbs() {
        int i;
        ...
        while (i3 < numViews4) {
            ...
            child.findViewById(android.R.id.title).setEnabled(i3 < numViews4 + (-1));
