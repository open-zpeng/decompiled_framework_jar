...
public class FreezePeriod {
    ...
    private synchronized FreezePeriod(int startDay, int endDay) {
    ...
    synchronized boolean isWrapped() {
        return this.mEndDay < this.mStartDay;
    }

    synchronized int getEffectiveEndDay() {
        if (!isWrapped()) {
            return this.mEndDay;
        }
    ...
    public synchronized boolean contains(LocalDate localDate) {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized boolean after(LocalDate localDate) {
    ...
    public synchronized Pair<LocalDate, LocalDate> toCurrentOrFutureRealDates(LocalDate now) {
    ...
    private static synchronized MonthDay dayOfYearToMonthDay(int dayOfYear) {
        LocalDate date = LocalDate.ofYearDay(2001, dayOfYear);
        return MonthDay.of(date.getMonth(), date.getDayOfMonth());
    }

    private static synchronized int dayOfYearDisregardLeapYear(LocalDate date) {
        return date.withYear(2001).getDayOfYear();
    }

    public static synchronized int distanceWithoutLeapYear(LocalDate first, LocalDate second) {
        return (dayOfYearDisregardLeapYear(first) - dayOfYearDisregardLeapYear(second)) + (365 * (first.getYear() - second.getYear()));
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static synchronized List<FreezePeriod> canonicalizePeriods(List<FreezePeriod> intervals) {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public static synchronized void validatePeriods(List<FreezePeriod> periods) {
        int separation;
    ...
    */
    public static synchronized void validateAgainstPreviousFreezePeriod(java.util.List<android.app.admin.FreezePeriod> r10, java.time.LocalDate r11, java.time.LocalDate r12, java.time.LocalDate r13) {
        /*
