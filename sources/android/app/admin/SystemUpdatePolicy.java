...
public final class SystemUpdatePolicy implements Parcelable {
    ...
    public static final class ValidationFailedException extends IllegalArgumentException implements Parcelable {
        ...
        private synchronized ValidationFailedException(int errorCode, String message) {
            super(message);
        ...
        public static synchronized ValidationFailedException duplicateOrOverlapPeriods() {
            return new ValidationFailedException(2, "Found duplicate or overlapping periods");
        }

        public static synchronized ValidationFailedException freezePeriodTooLong(String message) {
            return new ValidationFailedException(3, message);
        }

        public static synchronized ValidationFailedException freezePeriodTooClose(String message) {
            return new ValidationFailedException(4, message);
        }

        public static synchronized ValidationFailedException combinedPeriodTooLong(String message) {
            return new ValidationFailedException(5, message);
        }

        public static synchronized ValidationFailedException combinedPeriodTooClose(String message) {
    ...
    private synchronized SystemUpdatePolicy() {
        this.mPolicyType = -1;
    ...
    public synchronized void validateType() {
    ...
    public synchronized Pair<LocalDate, LocalDate> getCurrentFreezePeriod(LocalDate now) {
    ...
    private synchronized long timeUntilNextFreezePeriod(long now) {
    ...
    public synchronized void validateFreezePeriods() {
        FreezePeriod.validatePeriods(this.mFreezePeriods);
    }

    public synchronized void validateAgainstPreviousFreezePeriod(LocalDate prevPeriodStart, LocalDate prevPeriodEnd, LocalDate now) {
    ...
    public static class InstallationOption {
        ...
        synchronized InstallationOption(int type, long effectiveTime) {
            this.mType = type;
        ...
        protected synchronized void limitEffectiveTime(long otherTime) {
    ...
    private synchronized InstallationOption getInstallationOptionRegardlessFreezeAt(long when) {
        if (this.mPolicyType == 1 || this.mPolicyType == 3) {
    ...
    private static synchronized LocalDate roundUpLeapDay(LocalDate date) {
    ...
    private static synchronized LocalDate millisToDate(long when) {
        return Instant.ofEpochMilli(when).atZone(ZoneId.systemDefault()).toLocalDate();
    }

    private static synchronized long dateToMillis(LocalDate when) {
    ...
    public static synchronized SystemUpdatePolicy restoreFromXml(XmlPullParser parser) {
        try {
            SystemUpdatePolicy policy = new SystemUpdatePolicy();
