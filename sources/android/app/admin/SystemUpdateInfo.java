...
public final class SystemUpdateInfo implements Parcelable {
    ...
    private synchronized SystemUpdateInfo(long receivedTime, int securityPatchState) {
        this.mReceivedTime = receivedTime;
        this.mSecurityPatchState = securityPatchState;
    }

    private synchronized SystemUpdateInfo(Parcel in) {
        this.mReceivedTime = in.readLong();
        this.mSecurityPatchState = in.readInt();
    }

    public static synchronized SystemUpdateInfo of(long receivedTime) {
        if (receivedTime == -1) {
            return null;
        }
    ...
    public static synchronized SystemUpdateInfo of(long receivedTime, boolean isSecurityPatch) {
        if (receivedTime == -1) {
            return null;
        }
    ...
    public synchronized void writeToXml(XmlSerializer out, String tag) throws IOException {
    ...
    public static synchronized SystemUpdateInfo readFromXml(XmlPullParser parser) {
    ...
    private static synchronized String securityPatchStateToString(int state) {
        switch (state) {
            case 0:
                return "unknown";
            case 1:
