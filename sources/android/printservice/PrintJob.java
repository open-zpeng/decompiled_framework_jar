...
public final class PrintJob {
    ...
    public synchronized PrintJob(Context context, PrintJobInfo jobInfo, IPrintServiceClient client) {
    ...
    private synchronized boolean isInImmutableState() {
        int state = this.mCachedInfo.getState();
        return state == 5 || state == 7 || state == 6;
    }

    private synchronized boolean setState(int state, String error) {
        try {
