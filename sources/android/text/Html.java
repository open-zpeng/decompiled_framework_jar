...
public class Html {
    ...
    public static class HtmlParser {
        ...
        private synchronized HtmlParser() {
        }
    }
    ...
    private static synchronized void withinHtml(StringBuilder out, Spanned text, int option) {
        if ((option & 1) == 0) {
    ...
    private static synchronized void encodeTextAlignmentByDiv(StringBuilder out, Spanned text, int option) {
        int len = text.length();
        int i = 0;
        while (i < len) {
    ...
    private static synchronized void withinDiv(StringBuilder out, Spanned text, int start, int end, int option) {
        int i = start;
        while (i < end) {
    ...
    private static synchronized String getTextDirection(Spanned text, int start, int end) {
    ...
    private static synchronized String getTextStyles(Spanned text, int start, int end, boolean forceNoVerticalMargin, boolean includeTextAlign) {
        String margin = null;
    ...
    private static synchronized void withinBlockquote(StringBuilder out, Spanned text, int start, int end, int option) {
        if ((option & 1) == 0) {
    ...
    private static synchronized void withinBlockquoteIndividual(StringBuilder out, Spanned text, int start, int end) {
    ...
    private static synchronized void withinBlockquoteConsecutive(StringBuilder out, Spanned text, int start, int end) {
        out.append("<p");
    ...
    private static synchronized void withinParagraph(StringBuilder out, Spanned text, int start, int end) {
        int j = start;
        while (j < end) {
            ...
            for (int i2 = 0; i2 < style.length; i2++) {
                ...
                if (style[i2] instanceof ForegroundColorSpan) {
                    ...
                    out.append(String.format("<span style=\"color:#%06X;\">", Integer.valueOf(16777215 & color)));
                }
    ...
    public protected static void withinStyle(StringBuilder out, CharSequence text, int start, int end) {
        char d;
        int i = start;
        while (i < end) {
