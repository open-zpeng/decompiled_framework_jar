...
public class Time {
    ...
    private synchronized void initialize(String timezoneId) {
    ...
    private synchronized boolean parseInternal(String s) {
        int len = s.length();
        if (len < 8) {
    ...
    private synchronized void checkChar(String s, int spos, char expected) {
    ...
    private static synchronized int getChar(String s, int spos, int mul) {
        char c = s.charAt(spos);
    ...
    private synchronized boolean parse3339Internal(String s) {
        int offset;
        int len = s.length();
        if (len < 10) {
    ...
    public static class TimeCalculator {
        ...
        public synchronized TimeCalculator(String timezoneId) {
            this.zoneInfo = lookupZoneInfo(timezoneId);
        }

        public synchronized long toMillis(boolean ignoreDst) {
            if (ignoreDst) {
        ...
        public synchronized String format(String format) {
            if (format == null) {
                format = "%c";
            }
        ...
        private synchronized void updateZoneInfoFromTimeZone() {
            if (!this.zoneInfo.getID().equals(this.timezone)) {
                this.zoneInfo = lookupZoneInfo(this.timezone);
            }
        }

        private static synchronized ZoneInfo lookupZoneInfo(String timezoneId) {
            try {
        ...
        public synchronized void switchTimeZone(String timezone) {
        ...
        public synchronized String format2445(boolean hasTime) {
        ...
        private synchronized char toChar(int n) {
            if (n < 0 || n > 9) {
                return ' ';
            }
            return (char) (n + 48);
        ...
        public synchronized String toStringInternal() {
        ...
        public static synchronized int compare(TimeCalculator aObject, TimeCalculator bObject) {
        ...
        public synchronized void copyFieldsToTime(Time time) {
        ...
        public synchronized void copyFieldsFromTime(Time time) {
