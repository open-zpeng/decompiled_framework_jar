...
public final class BidiFormatter {
    ...
    public static final class Builder {
        ...
        private synchronized void initialize(boolean isRtlContext) {
    ...
    private synchronized BidiFormatter(boolean isRtlContext, int flags, TextDirectionHeuristic heuristic) {
        this.mIsRtlContext = isRtlContext;
        this.mFlags = flags;
    ...
    public synchronized String markAfter(CharSequence str, TextDirectionHeuristic heuristic) {
    ...
    public synchronized String markBefore(CharSequence str, TextDirectionHeuristic heuristic) {
        boolean isRtl = heuristic.isRtl(str, 0, str.length());
    ...
    public static synchronized BidiFormatter getDefaultInstanceFromContext(boolean isRtlContext) {
        return isRtlContext ? DEFAULT_RTL_INSTANCE : DEFAULT_LTR_INSTANCE;
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static synchronized boolean isRtlLocale(Locale locale) {
        return TextUtils.getLayoutDirectionFromLocale(locale) == 1;
    }

    private static synchronized int getExitDir(CharSequence str) {
        return new DirectionalityEstimator(str, false).getExitDir();
    }

    private static synchronized int getEntryDir(CharSequence str) {
    ...
    public static class DirectionalityEstimator {
        ...
        public static synchronized byte getDirectionality(int codePoint) {
        ...
        synchronized DirectionalityEstimator(CharSequence text, boolean isHtml) {
            this.text = text;
            this.isHtml = isHtml;
            this.length = text.length();
        }

        synchronized int getEntryDir() {
            this.charIndex = 0;
        ...
        private static synchronized byte getCachedDirectionality(char c) {
            return c < 1792 ? DIR_TYPE_CACHE[c] : getDirectionality(c);
        }

        synchronized byte dirTypeForward() {
        ...
        synchronized byte dirTypeBackward() {
        ...
        private synchronized byte skipTagForward() {
        ...
        private synchronized byte skipTagBackward() {
            int initialCharIndex = this.charIndex;
            while (this.charIndex > 0) {
                CharSequence charSequence = this.text;
        ...
        private synchronized byte skipEntityForward() {
            while (this.charIndex < this.length) {
                CharSequence charSequence = this.text;
                int i = this.charIndex;
                this.charIndex = i + 1;
        ...
        private synchronized byte skipEntityBackward() {
            int initialCharIndex = this.charIndex;
            while (this.charIndex > 0) {
                CharSequence charSequence = this.text;
                int i = this.charIndex - 1;
