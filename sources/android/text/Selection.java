...
public class Selection {
    ...
    private synchronized Selection() {
    }
    ...
    private static synchronized int getSelectionMemory(CharSequence text) {
        if (text instanceof Spanned) {
    ...
    private static synchronized void setSelection(Spannable text, int start, int stop, int memory) {
        int ostart = getSelectionStart(text);
        int oend = getSelectionEnd(text);
        if (ostart != start || oend != stop) {
            text.setSpan(SELECTION_START, start, start, MetricsProto.MetricsEvent.DIALOG_VPN_APP_CONFIG);
            text.setSpan(SELECTION_END, stop, stop, 34);
            updateMemory(text, memory);
        }
    }

    private static synchronized void updateMemory(Spannable text, int memory) {
        if (memory > -1) {
    ...
    private static synchronized void removeMemory(Spannable text) {
    ...
    private static synchronized void extendSelection(Spannable text, int index, int memory) {
    ...
    private static synchronized void setSelectionAndMemory(Spannable text, Layout layout, int line, int end, int direction, boolean extend) {
        int move;
        int move2;
        int newMemory;
        if (layout.getParagraphDirection(line) == layout.getParagraphDirection(line + direction)) {
            ...
            int move3 = layout.getLineStart(newMemory2);
            move = move3;
            move2 = -1;
        }
        if (extend) {
    ...
    private static synchronized int findEdge(Spannable text, Layout layout, int dir) {
    ...
    private static synchronized int chooseHorizontal(Layout layout, int direction, int off1, int off2) {
