...
public class ArrowKeyMovementMethod extends BaseMovementMethod implements MovementMethod {
    ...
    private static synchronized boolean isSelecting(Spannable buffer) {
        return MetaKeyKeyListener.getMetaState(buffer, 1) == 1 || MetaKeyKeyListener.getMetaState(buffer, 2048) != 0;
    }

    private static synchronized int getCurrentLineTop(Spannable buffer, Layout layout) {
        return layout.getLineTop(layout.getLineForOffset(Selection.getSelectionEnd(buffer)));
    }

    private static synchronized int getPageHeight(TextView widget) {
        Rect rect = new Rect();
