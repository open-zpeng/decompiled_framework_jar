...
public abstract class MetaKeyKeyListener {
    ...
    private static synchronized int getActive(CharSequence text, Object meta, int on, int lock) {
    ...
    private static synchronized void adjust(Spannable content, Object what) {
    ...
    private static synchronized void resetLock(Spannable content, Object what) {
        int current = content.getSpanFlags(what);
        if (current == LOCKED) {
            content.removeSpan(what);
        }
    ...
    private synchronized void press(Editable content, Object what) {
    ...
    private synchronized void release(Editable content, Object what, KeyEvent event) {
        int current = content.getSpanFlags(what);
        if (event.getKeyCharacterMap().getModifierBehavior() == 1) {
            if (current == USED) {
                content.removeSpan(what);
    ...
    private static synchronized long press(long state, int what, long mask, long locked, long pressed, long released, long used) {
        if ((state & pressed) == 0) {
            if ((state & released) != 0) {
    ...
    private static synchronized long release(long state, int what, long mask, long pressed, long released, long used, KeyEvent event) {
        if (event.getKeyCharacterMap().getModifierBehavior() == 1) {
            if ((state & used) != 0) {
                return state & (~mask);
            }
