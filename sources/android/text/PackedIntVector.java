...
public class PackedIntVector {
    ...
    public synchronized PackedIntVector(int columns) {
        this.mColumns = columns;
        this.mValueGap = new int[2 * columns];
    }

    public synchronized int getValue(int row, int column) {
    ...
    public synchronized void setValue(int row, int column, int value) {
    ...
    private synchronized void setValueInternal(int row, int column, int value) {
        if (row >= this.mRowGapStart) {
            row += this.mRowGapLength;
        }
        int[] valuegap = this.mValueGap;
    ...
    public synchronized void adjustValuesBelow(int startRow, int column, int delta) {
    ...
    public synchronized void insertAt(int row, int[] values) {
        if (row < 0 || row > size()) {
            ...
            for (int i3 = i2 - 1; i3 >= 0; i3--) {
                setValueInternal(row, i3, values[i3]);
            }
        }
    }
    ...
    private final synchronized void growBuffer() {
        int columns = this.mColumns;
    ...
    private final synchronized void moveValueGapTo(int column, int where) {
        int[] valuegap = this.mValueGap;
    ...
    private final synchronized void moveRowGapTo(int where) {
