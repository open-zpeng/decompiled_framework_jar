...
public class ImsService extends Service {
    ...
    public private final IBinder mImsServiceController = new IImsServiceController.Stub() { // from class: android.telephony.ims.compat.ImsService.1
    ...
    @VisibleForTesting
    public synchronized SparseArray<ImsFeature> getFeatures(int slotId) {
        return this.mFeaturesBySlot.get(slotId);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public synchronized IImsMMTelFeature createEmergencyMMTelFeatureInternal(int slotId, IImsFeatureStatusCallback c) {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public synchronized IImsMMTelFeature createMMTelFeatureInternal(int slotId, IImsFeatureStatusCallback c) {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public synchronized IImsRcsFeature createRcsFeatureInternal(int slotId, IImsFeatureStatusCallback c) {
    ...
    private synchronized void setupFeature(ImsFeature f, int slotId, int featureType, IImsFeatureStatusCallback c) {
        f.setContext(this);
        f.setSlotId(slotId);
    ...
    private synchronized void addImsFeature(int slotId, int featureType, ImsFeature f) {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public synchronized void removeImsFeature(int slotId, int featureType, IImsFeatureStatusCallback c) {
        synchronized (this.mFeaturesBySlot) {
            SparseArray<ImsFeature> features = this.mFeaturesBySlot.get(slotId);
            if (features == null) {
    ...
    public synchronized MMTelFeature onCreateEmergencyMMTelImsFeature(int slotId) {
        return null;
    }

    public synchronized MMTelFeature onCreateMMTelImsFeature(int slotId) {
        return null;
    }

    public synchronized RcsFeature onCreateRcsFeature(int slotId) {
        return null;
    }
}
