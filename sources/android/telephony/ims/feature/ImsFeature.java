...
public abstract class ImsFeature {
    ...
    protected abstract synchronized IInterface getBinder();
    ...
    public static class CapabilityCallback extends IImsCapabilityCallback.Stub {
        public final synchronized void onCapabilitiesStatusChanged(int config) throws RemoteException {
            onCapabilitiesStatusChanged(new Capabilities(config));
        }

        public synchronized void onQueryCapabilityConfiguration(int capability, int radioTech, boolean isEnabled) {
        }

        public synchronized void onChangeCapabilityConfigurationError(int capability, int radioTech, int reason) {
        }

        public synchronized void onCapabilitiesStatusChanged(Capabilities config) {
        }
    }
    ...
    protected static class CapabilityCallbackProxy {
        ...
        public synchronized CapabilityCallbackProxy(IImsCapabilityCallback c) {
            this.mCallback = c;
        }
    ...
    public static class Capabilities {
        ...
        public synchronized Capabilities() {
            this.mCapabilities = 0;
        }

        protected synchronized Capabilities(int capabilities) {
            this.mCapabilities = 0;
            this.mCapabilities = capabilities;
        }

        public synchronized void addCapabilities(int capabilities) {
            this.mCapabilities |= capabilities;
        }

        public synchronized void removeCapabilities(int capabilities) {
            this.mCapabilities &= ~capabilities;
        }

        public synchronized boolean isCapable(int capabilities) {
            return (this.mCapabilities & capabilities) == capabilities;
        }

        public synchronized Capabilities copy() {
    ...
    public final synchronized void initialize(Context context, int slotId) {
        this.mContext = context;
        this.mSlotId = slotId;
    }

    public synchronized int getFeatureState() {
        int i;
    ...
    public final synchronized void addCapabilityCallback(IImsCapabilityCallback c) {
        this.mCapabilityCallbacks.register(c);
    }

    public final synchronized void removeCapabilityCallback(IImsCapabilityCallback c) {
        this.mCapabilityCallbacks.unregister(c);
    }

    @VisibleForTesting
    public synchronized Capabilities queryCapabilityStatus() {
        Capabilities copy;
        synchronized (this.mLock) {
    ...
    @VisibleForTesting
    public final synchronized void requestChangeEnabledCapabilities(CapabilityChangeRequest request, IImsCapabilityCallback c) {
        if (request == null) {
    ...
    public final synchronized void notifyCapabilitiesStatusChanged(Capabilities c) {
        synchronized (this.mLock) {
