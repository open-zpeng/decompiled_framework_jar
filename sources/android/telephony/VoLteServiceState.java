...
public final class VoLteServiceState implements Parcelable {
    ...
    public static synchronized VoLteServiceState newFromBundle(Bundle m) {
        VoLteServiceState ret = new VoLteServiceState();
        ret.setFromNotifierBundle(m);
        return ret;
    }

    public synchronized VoLteServiceState() {
        initialize();
    }

    private protected VoLteServiceState(int srvccState) {
        initialize();
        this.mSrvccState = srvccState;
    }

    public synchronized VoLteServiceState(VoLteServiceState s) {
        copyFrom(s);
    }
    ...
    protected synchronized void copyFrom(VoLteServiceState s) {
        this.mSrvccState = s.mSrvccState;
    }

    public synchronized VoLteServiceState(Parcel in) {
    ...
    public synchronized void validateInput() {
    }
    ...
    private synchronized void setFromNotifierBundle(Bundle m) {
        this.mSrvccState = m.getInt("mSrvccState");
    }

    public synchronized void fillInNotifierBundle(Bundle m) {
        m.putInt("mSrvccState", this.mSrvccState);
    }

    public synchronized int getSrvccState() {
