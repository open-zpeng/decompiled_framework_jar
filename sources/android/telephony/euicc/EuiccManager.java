...
public class EuiccManager {
    ...
    public synchronized EuiccManager(Context context) {
    ...
    public synchronized void updateSubscriptionNickname(int subscriptionId, String nickname, PendingIntent callbackIntent) {
        if (!isEnabled()) {
            sendUnavailableError(callbackIntent);
            return;
        }
    ...
    public synchronized void retainSubscriptionsForFactoryReset(PendingIntent callbackIntent) {
        if (!isEnabled()) {
            sendUnavailableError(callbackIntent);
            return;
        }
    ...
    private static synchronized void sendUnavailableError(PendingIntent callbackIntent) {
        try {
    ...
    private static synchronized IEuiccController getIEuiccController() {
