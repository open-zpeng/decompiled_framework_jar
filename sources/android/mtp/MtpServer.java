...
public class MtpServer implements Runnable {
    ...
    public synchronized MtpServer(MtpDatabase database, FileDescriptor controlFd, boolean usePtp, Runnable onTerminate, String deviceInfoManufacturer, String deviceInfoModel, String deviceInfoDeviceVersion, String deviceInfoSerialNumber) {
    ...
    public synchronized void sendObjectAdded(int handle) {
        native_send_object_added(handle);
    }

    public synchronized void sendObjectRemoved(int handle) {
        native_send_object_removed(handle);
    }

    public synchronized void sendDevicePropertyChanged(int property) {
        native_send_device_property_changed(property);
    }

    public synchronized void addStorage(MtpStorage storage) {
        native_add_storage(storage);
    }

    public synchronized void removeStorage(MtpStorage storage) {
        native_remove_storage(storage.getStorageId());
    }

    public static synchronized void configure(boolean usePtp) {
