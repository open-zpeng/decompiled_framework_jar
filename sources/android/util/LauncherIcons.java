...
public final class LauncherIcons {
    ...
    public synchronized LauncherIcons(Context context) {
        this.mRes = context.getResources();
        this.mIconSize = this.mRes.getDimensionPixelSize(17104896);
    }

    public synchronized Drawable wrapIconDrawableWithShadow(Drawable drawable) {
    ...
    private synchronized Bitmap getShadowBitmap(AdaptiveIconDrawable d) {
        ...
        synchronized (this.mShadowCache) {
            ...
            int bitmapSize = (int) (shadowSize + (2.0f * blur) + keyShadowDistance);
    ...
    public synchronized Drawable getBadgeDrawable(int foregroundRes, int backgroundColor) {
        return getBadgedDrawable(null, foregroundRes, backgroundColor);
    }

    public synchronized Drawable getBadgedDrawable(Drawable base, int foregroundRes, int backgroundColor) {
        Resources sysRes = Resources.getSystem();
        Drawable badgeShadow = sysRes.getDrawable(R.drawable.ic_corp_icon_badge_shadow);
        Drawable badgeColor = sysRes.getDrawable(R.drawable.ic_corp_icon_badge_color).getConstantState().newDrawable().mutate();
    ...
    private static class ShadowDrawable extends DrawableWrapper {
        ...
        public synchronized ShadowDrawable(Bitmap shadow, Drawable dr) {
            super(dr);
            this.mState = new MyConstantState(shadow, dr.getConstantState());
        }

        synchronized ShadowDrawable(MyConstantState state) {
        ...
        private static class MyConstantState extends Drawable.ConstantState {
            ...
            synchronized MyConstantState(Bitmap shadow, Drawable.ConstantState childState) {
                this.mShadow = shadow;
