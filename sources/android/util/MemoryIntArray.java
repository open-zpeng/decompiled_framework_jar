...
public final class MemoryIntArray implements Parcelable, Closeable {
    ...
    public synchronized MemoryIntArray(int size) throws IOException {
    ...
    private synchronized MemoryIntArray(Parcel parcel) throws IOException {
        this.mCloseGuard = dalvik.system.CloseGuard.get();
        this.mFd = -1;
        this.mIsOwner = false;
    ...
    public synchronized int get(int index) throws IOException {
        enforceNotClosed();
        enforceValidIndex(index);
        return nativeGet(this.mFd, this.mMemoryAddr, index);
    }

    public synchronized void set(int index, int value) throws IOException {
        enforceNotClosed();
        enforceWritable();
        enforceValidIndex(index);
    ...
    private synchronized void enforceNotClosed() {
        if (isClosed()) {
            throw new IllegalStateException("cannot interact with a closed instance");
        }
    }

    private synchronized void enforceValidIndex(int index) throws IOException {
        int size = size();
    ...
    private synchronized void enforceWritable() {
        if (!isWritable()) {
