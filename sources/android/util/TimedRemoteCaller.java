...
public abstract class TimedRemoteCaller<T> {
    ...
    public synchronized TimedRemoteCaller(long callTimeoutMillis) {
        this.mCallTimeoutMillis = callTimeoutMillis;
    }

    protected final synchronized int onBeforeRemoteCall() {
        int sequenceId;
    ...
    protected final synchronized void onRemoteMethodResult(T result, int sequence) {
        synchronized (this.mLock) {
    ...
    protected final synchronized T getResultTimed(int sequence) throws TimeoutException {
