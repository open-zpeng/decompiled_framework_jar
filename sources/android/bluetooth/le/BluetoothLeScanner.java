...
public final class BluetoothLeScanner {
    ...
    public synchronized BluetoothLeScanner(IBluetoothManager bluetoothManager) {
    ...
    private synchronized int startScan(List<ScanFilter> filters, ScanSettings settings, WorkSource workSource, ScanCallback callback, PendingIntent callbackIntent, List<List<ResultStorageDescriptor>> resultStorages) {
        IBluetoothGatt gatt;
        ...
        synchronized (this.mLeScanClients) {
            ...
            if (gatt2 == null) {
                ...
                if (callback != null) {
                    BleScanCallbackWrapper wrapper = new BleScanCallbackWrapper(gatt2, filters, settings, workSource, callback, resultStorages);
                    wrapper.startRegistration();
                } else {
                    try {
                        gatt2.startScanForIntent(callbackIntent, settings, filters, ActivityThread.currentOpPackageName());
    ...
    public class BleScanCallbackWrapper extends IScannerCallback.Stub {
        ...
        public synchronized void startRegistration() {
            ...
            try {
                ...
                wait(SurfaceView.SurfaceViewFactory.BACKGROUND_TRANSACTION_DELAY);
        ...
        synchronized void flushPendingBatchResults() {
            if (this.mScannerId <= 0) {
                Log.e(BluetoothLeScanner.TAG, "Error state, mLeHandle: " + this.mScannerId);
                return;
            }
        ...
        @Override // android.bluetooth.le.IScannerCallback
        public synchronized void onScanResult(final ScanResult scanResult) {
            if (this.mScannerId <= 0) {
                return;
            }
        ...
        @Override // android.bluetooth.le.IScannerCallback
        public synchronized void onBatchScanResults(final List<ScanResult> results) {
            Handler handler = new Handler(Looper.getMainLooper());
        ...
        @Override // android.bluetooth.le.IScannerCallback
        public synchronized void onFoundOrLost(final boolean onFound, final ScanResult scanResult) {
            if (this.mScannerId <= 0) {
                return;
            }
            Handler handler = new Handler(Looper.getMainLooper());
        ...
        @Override // android.bluetooth.le.IScannerCallback
        public synchronized void onScanManagerErrorCallback(int errorCode) {
            if (this.mScannerId <= 0) {
                return;
            }
    ...
    private synchronized int postCallbackErrorOrReturn(ScanCallback callback, int errorCode) {
        if (callback == null) {
            return errorCode;
        }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public synchronized void postCallbackError(final ScanCallback callback, final int errorCode) {
    ...
    private synchronized boolean isSettingsConfigAllowedForScan(ScanSettings settings) {
    ...
    private synchronized boolean isSettingsAndFilterComboAllowed(ScanSettings settings, List<ScanFilter> filterList) {
        int callbackType = settings.getCallbackType();
        if ((callbackType & 6) != 0) {
            if (filterList == null) {
                return false;
    ...
    private synchronized boolean isHardwareResourcesAvailableForScan(ScanSettings settings) {
        int callbackType = settings.getCallbackType();
