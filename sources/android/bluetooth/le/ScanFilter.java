...
public final class ScanFilter implements Parcelable {
    ...
    private synchronized ScanFilter(String name, String deviceAddress, ParcelUuid uuid, ParcelUuid uuidMask, ParcelUuid serviceDataUuid, byte[] serviceData, byte[] serviceDataMask, int manufacturerId, byte[] manufacturerData, byte[] manufacturerDataMask) {
    ...
    public static synchronized boolean matchesServiceUuids(ParcelUuid uuid, ParcelUuid parcelUuidMask, List<ParcelUuid> uuids) {
        if (uuid == null) {
            return true;
        }
        if (uuids == null) {
    ...
    private static synchronized boolean matchesServiceUuid(UUID uuid, UUID mask, UUID data) {
        return BitUtils.maskedEquals(data, uuid, mask);
    }

    private synchronized boolean matchesPartialData(byte[] data, byte[] dataMask, byte[] parsedData) {
    ...
    public synchronized boolean isAllFieldsEmpty() {
