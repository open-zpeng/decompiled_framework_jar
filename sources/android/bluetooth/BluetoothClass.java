...
public final class BluetoothClass implements Parcelable {
    ...
    private protected static final int PROFILE_A2DP = 1;
    public static final int PROFILE_A2DP_SINK = 6;
    private protected static final int PROFILE_HEADSET = 0;
    ...
    public synchronized int getClassOfDevice() {
        return this.mClass;
    }

    public synchronized byte[] getClassOfDeviceBytes() {
        byte[] bytes = ByteBuffer.allocate(4).order(ByteOrder.BIG_ENDIAN).putInt(this.mClass).array();
        return Arrays.copyOfRange(bytes, 1, bytes.length);
    }

    private protected boolean doesClassMatch(int profile) {
        int deviceClass;
        int deviceClass2;
        int deviceClass3;
        if (profile == 1) {
