...
public final class BluetoothHealth implements BluetoothProfile {
    ...
    public synchronized boolean registerAppConfiguration(String name, int dataType, int role, int channelType, BluetoothHealthCallback callback) {
        ...
        if (service != null) {
            try {
                boolean result = service.registerAppConfiguration(config, wrapper);
                return result;
    ...
    public boolean unregisterAppConfiguration(BluetoothHealthAppConfiguration config) {
        ...
        if (service != null && isEnabled() && config != null) {
            try {
                boolean result = service.unregisterAppConfiguration(config);
                return result;
            } catch (RemoteException e) {
                Log.e(TAG, e.toString());
                return false;
    ...
    public synchronized boolean connectChannelToSink(BluetoothDevice device, BluetoothHealthAppConfiguration config, int channelType) {
        IBluetoothHealth service = this.mService;
        if (service != null && isEnabled() && isValidDevice(device) && config != null) {
            try {
    ...
    public static class BluetoothHealthCallbackWrapper extends IBluetoothHealthCallback.Stub {
        ...
        public synchronized BluetoothHealthCallbackWrapper(BluetoothHealthCallback callback) {
            this.mCallback = callback;
        }

        @Override // android.bluetooth.IBluetoothHealthCallback
        public synchronized void onHealthAppConfigurationStatusChange(BluetoothHealthAppConfiguration config, int status) {
            this.mCallback.onHealthAppConfigurationStatusChange(config, status);
        }

        @Override // android.bluetooth.IBluetoothHealthCallback
        public synchronized void onHealthChannelStateChange(BluetoothHealthAppConfiguration config, BluetoothDevice device, int prevState, int newState, ParcelFileDescriptor fd, int channelId) {
            this.mCallback.onHealthChannelStateChange(config, device, prevState, newState, fd, channelId);
        }
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized BluetoothHealth(Context context, BluetoothProfile.ServiceListener l) {
    ...
    private synchronized boolean checkAppParam(String name, int role, int channelType, BluetoothHealthCallback callback) {
