...
public class BluetoothGattCharacteristic implements Parcelable {
    ...
    public private int mInstance;
    protected int mKeySize;
    protected int mPermissions;
    protected int mProperties;
    public private BluetoothGattService mService;
    ...
    synchronized BluetoothGattCharacteristic(BluetoothGattService service, UUID uuid, int instanceId, int properties, int permissions) {
        this.mKeySize = 16;
        initCharacteristic(service, uuid, instanceId, properties, permissions);
    }

    public synchronized BluetoothGattCharacteristic(UUID uuid, int instanceId, int properties, int permissions) {
        this.mKeySize = 16;
        initCharacteristic(null, uuid, instanceId, properties, permissions);
    }

    private synchronized void initCharacteristic(BluetoothGattService service, UUID uuid, int instanceId, int properties, int permissions) {
        this.mUuid = uuid;
    ...
    private synchronized BluetoothGattCharacteristic(Parcel in) {
        this.mKeySize = 16;
    ...
    synchronized BluetoothGattDescriptor getDescriptor(UUID uuid, int instanceId) {
    ...
    public private protected void setService(BluetoothGattService service) {
        this.mService = service;
    }
    ...
    private protected void setKeySize(int keySize) {
    ...
    private synchronized int getTypeLen(int formatType) {
        return formatType & 15;
    }

    private synchronized int unsignedByteToInt(byte b) {
        return b & 255;
    }

    private synchronized int unsignedBytesToInt(byte b0, byte b1) {
        return unsignedByteToInt(b0) + (unsignedByteToInt(b1) << 8);
    }

    private synchronized int unsignedBytesToInt(byte b0, byte b1, byte b2, byte b3) {
        return unsignedByteToInt(b0) + (unsignedByteToInt(b1) << 8) + (unsignedByteToInt(b2) << 16) + (unsignedByteToInt(b3) << 24);
    }

    private synchronized float bytesToFloat(byte b0, byte b1) {
    ...
    private synchronized float bytesToFloat(byte b0, byte b1, byte b2, byte b3) {
    ...
    private synchronized int unsignedToSigned(int unsigned, int size) {
    ...
    private synchronized int intToSignedBits(int i, int size) {
        if (i < 0) {
