...
public class ScaleGestureDetector {
    ...
    public protected final OnScaleGestureListener mListener;
    public protected int mMinSpan;
    private float mPrevSpan;
    private float mPrevSpanX;
    private float mPrevSpanY;
    private long mPrevTime;
    private boolean mQuickScaleEnabled;
    public protected int mSpanSlop;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        while (i2 < count2) {
            if (skipIndex == i2) {
                ...
                count = count2;
                devSumY2 += Math.abs(event.getX(i2) - focusX);
        ...
        float spanX = (devSumY2 / div) * 2.0f;
        float devX = devY * 2.0f;
        float span = inAnchoredScaleMode() ? devX : (float) Math.hypot(spanX, devX);
        ...
        if (!this.mInProgress && span >= minSpan) {
            if (!wasInProgress) {
                float abs = Math.abs(span - this.mInitialSpan);
                float focusY2 = this.mSpanSlop;
                if (abs <= focusY2) {
                }
            }
            this.mCurrSpanX = spanX;
    ...
    private synchronized boolean inAnchoredScaleMode() {
