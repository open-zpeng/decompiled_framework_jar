...
public class ScaleGestureDetector {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        float devX = inAnchoredScaleMode() ? spanY : (float) Math.hypot(spanX, spanY);
        boolean wasInProgress = this.mInProgress;
        this.mFocusX = focusX;
        this.mFocusY = focusY;
        if (!inAnchoredScaleMode() && this.mInProgress && (devX < this.mMinSpan || configChanged)) {
            this.mListener.onScaleEnd(this);
            this.mInProgress = false;
        ...
        if (this.mInProgress || devX < minSpan) {
            devY = focusX;
        } else {
            if (!wasInProgress && Math.abs(devX - this.mInitialSpan) <= this.mSpanSlop) {
                devY = focusX;
            }
            this.mCurrSpanX = spanX;
            this.mPrevSpanX = spanX;
