...
public abstract class Window {
    ...
    @UnsupportedAppUsage
    public static final int FEATURE_MAX = 15;
    ...
    public boolean shouldCloseOnTouch(Context context, MotionEvent event) {
        ...
        boolean isOutsideEvent = this.mCloseOnTouchOutside && isOutside;
        if (isOutsideEvent && SharedDisplayManager.enable()) {
            try {
                int motionScreenId = SharedDisplayManager.findScreenId(event);
                WindowManager.LayoutParams lp = getAttributes();
                if (lp != null && SharedDisplayManager.sharedValid(lp.sharedId)) {
                    targetScreenId = SharedDisplayManager.findScreenId(lp.sharedId);
                } else {
                    WindowManager wm = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
                    String packageName = context.getBasePackageName();
                    targetScreenId = SharedDisplayManager.findScreenId(wm.getSharedId(packageName));
                }
                if (targetScreenId != motionScreenId) {
                    return false;
                }
            } catch (Exception e) {
            }
    ...
    public void setBackgroundDrawableResource(int resId) {
        try {
            setBackgroundDrawable(this.mContext.getApplicationContext().getDrawable(resId));
        } catch (Exception e) {
        }
    }
