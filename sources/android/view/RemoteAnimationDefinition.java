...
public class RemoteAnimationDefinition implements Parcelable {
    ...
    private protected RemoteAnimationDefinition() {
        this.mTransitionAnimationMap = new SparseArray<>();
    }

    private protected void addRemoteAnimation(int transition, @WindowConfiguration.ActivityType int activityTypeFilter, RemoteAnimationAdapter adapter) {
        this.mTransitionAnimationMap.put(transition, new RemoteAnimationAdapterEntry(adapter, activityTypeFilter));
    }

    private protected void addRemoteAnimation(int transition, RemoteAnimationAdapter adapter) {
        addRemoteAnimation(transition, 0, adapter);
    }

    public synchronized boolean hasTransition(int transition, ArraySet<Integer> activityTypes) {
        return getAdapter(transition, activityTypes) != null;
    }

    public synchronized RemoteAnimationAdapter getAdapter(int transition, ArraySet<Integer> activityTypes) {
    ...
    public synchronized RemoteAnimationDefinition(Parcel in) {
        int size = in.readInt();
    ...
    public synchronized void setCallingPid(int pid) {
    ...
    public static class RemoteAnimationAdapterEntry implements Parcelable {
        ...
        synchronized RemoteAnimationAdapterEntry(RemoteAnimationAdapter adapter, int activityTypeFilter) {
            this.adapter = adapter;
            this.activityTypeFilter = activityTypeFilter;
        }

        private synchronized RemoteAnimationAdapterEntry(Parcel in) {
