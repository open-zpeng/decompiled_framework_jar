...
public final class InputDevice implements Parcelable {
    ...
    public static final int SOURCE_CAR = 268435457;
    ...
    public protected final boolean mIsExternal;
    ...
    public protected InputDevice(int id, int generation, int controllerNumber, String name, int vendorId, int productId, String descriptor, boolean isExternal, int sources, int keyboardType, KeyCharacterMap keyCharacterMap, boolean hasVibrator, boolean hasMicrophone, boolean hasButtonUnderPad) {
    ...
    private synchronized InputDevice(Parcel in) {
        this.mMotionRanges = new ArrayList<>();
        this.mId = in.readInt();
    ...
    public synchronized InputDeviceIdentifier getIdentifier() {
    ...
    public boolean isVirtual() {
        if (xpInputDeviceWrapper.isMockDevice(this.mId)) {
            return xpInputDeviceWrapper.isVirtual();
        }
        return this.mId < 0;
    }

    private protected boolean isExternal() {
        return this.mIsExternal;
    }

    public synchronized boolean isFullKeyboard() {
    ...
    public int getSources() {
        if (xpInputDeviceWrapper.isMockDevice(this.mId)) {
            return xpInputDeviceWrapper.getSource();
        }
    ...
    public int getKeyboardType() {
        if (xpInputDeviceWrapper.isMockDevice(this.mId)) {
            return xpInputDeviceWrapper.getKeyboardType();
        }
    ...
    public List<MotionRange> getMotionRanges() {
        if (xpInputDeviceWrapper.isMockDevice(this.mId)) {
            return xpInputDeviceWrapper.getMotionRanges();
        }
        return this.mMotionRanges;
    }

    public protected void addMotionRange(int axis, int source, float min, float max, float flat, float fuzz, float resolution) {
    ...
    public synchronized boolean hasButtonUnderPad() {
        return this.mHasButtonUnderPad;
    }

    public synchronized void setPointerType(int pointerType) {
    ...
    public static final class MotionRange {
        ...
        private synchronized MotionRange(int axis, int source, float min, float max, float flat, float fuzz, float resolution) {
            this.mAxis = axis;
            this.mSource = source;
            this.mMin = min;
            this.mMax = max;
        ...
        public static MotionRange build(int axis, int source, float min, float max, float flat, float fuzz, float resolution) {
            return new MotionRange(axis, source, min, max, flat, fuzz, resolution);
        }
    }
    ...
    private synchronized void appendSourceDescriptionIfApplicable(StringBuilder description, int source, String sourceName) {
