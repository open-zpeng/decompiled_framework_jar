...
public class PathInterpolator extends BaseInterpolator implements NativeInterpolatorFactory {
    ...
    public synchronized PathInterpolator(Resources res, Resources.Theme theme, AttributeSet attrs) {
        TypedArray a;
        if (theme != null) {
    ...
    private synchronized void parseInterpolatorFromTypeArray(TypedArray a) {
        if (a.hasValue(4)) {
    ...
    private synchronized void initQuad(float controlX, float controlY) {
        Path path = new Path();
    ...
    private synchronized void initCubic(float x1, float y1, float x2, float y2) {
        Path path = new Path();
        path.moveTo(0.0f, 0.0f);
    ...
    private synchronized void initPath(Path path) {
