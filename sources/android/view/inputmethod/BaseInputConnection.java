...
public class BaseInputConnection implements InputConnection {
    ...
    public synchronized BaseInputConnection(InputMethodManager mgr, boolean fullEditor) {
        this.mIMM = mgr;
    ...
    public static synchronized void setComposingSpans(Spannable text, int start, int end) {
    ...
    private static synchronized int findIndexBackward(CharSequence cs, int from, int numCodePoints) {
        int N = cs.length();
        if (from < 0 || N < from) {
    ...
    private static synchronized int findIndexForward(CharSequence cs, int from, int numCodePoints) {
        int N = cs.length();
        if (from < 0 || N < from) {
            return INVALID_INDEX;
        }
    ...
    private synchronized void sendCurrentText() {
        Editable content;
        int N;
    ...
    private synchronized void ensureDefaultComposingSpans() {
        Context context;
        if (this.mDefaultComposingSpans == null) {
    ...
    private synchronized void replaceText(CharSequence text, int newCursorPosition, boolean composing) {
