...
public final class EntityConfidence implements Parcelable {
    ...
    synchronized EntityConfidence() {
        this.mEntityConfidence = new ArrayMap<>();
        this.mSortedEntities = new ArrayList<>();
    }

    synchronized EntityConfidence(EntityConfidence source) {
        this.mEntityConfidence = new ArrayMap<>();
        this.mSortedEntities = new ArrayList<>();
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized EntityConfidence(Map<String, Float> source) {
        this.mEntityConfidence = new ArrayMap<>();
        this.mSortedEntities = new ArrayList<>();
        Preconditions.checkNotNull(source);
    ...
    public synchronized List<String> getEntities() {
        return Collections.unmodifiableList(this.mSortedEntities);
    }

    public synchronized float getConfidenceScore(String entity) {
    ...
    private synchronized EntityConfidence(Parcel in) {
        this.mEntityConfidence = new ArrayMap<>();
        this.mSortedEntities = new ArrayList<>();
    ...
    private synchronized void resetSortedEntitiesFromMap() {
