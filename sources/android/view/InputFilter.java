...
public abstract class InputFilter extends IInputFilter.Stub {
    ...
    private protected InputFilter(Looper looper) {
    ...
    public final synchronized void install(IInputFilterHost host) {
    ...
    @Override // android.view.IInputFilter
    public final synchronized void filterInputEvent(InputEvent event, int policyFlags) {
        this.mH.obtainMessage(3, policyFlags, 0, event).sendToTarget();
    }

    public synchronized void sendInputEvent(InputEvent event, int policyFlags) {
        if (event == null) {
    ...
    private protected void onInputEvent(InputEvent event, int policyFlags) {
        sendInputEvent(event, policyFlags);
    }

    public synchronized void onInstalled() {
    }

    public synchronized void onUninstalled() {
    }
