...
public class ViewHierarchyEncoder {
    ...
    public synchronized ViewHierarchyEncoder(ByteArrayOutputStream stream) {
        this.mStream = new DataOutputStream(stream);
    }

    public synchronized void beginObject(Object o) {
        startPropertyMap();
        addProperty("meta:__name__", o.getClass().getName());
        addProperty("meta:__hash__", o.hashCode());
    }

    public synchronized void endObject() {
        endPropertyMap();
    }
    ...
    public synchronized void addProperty(String name, short s) {
        writeShort(createPropertyIndex(name));
        writeShort(s);
    }
    ...
    public synchronized void addPropertyKey(String name) {
        writeShort(createPropertyIndex(name));
    }

    private synchronized short createPropertyIndex(String name) {
    ...
    private synchronized void startPropertyMap() {
        try {
    ...
    private synchronized void endPropertyMap() {
        writeShort((short) 0);
    }

    private synchronized void writeBoolean(boolean v) {
        try {
    ...
    private synchronized void writeShort(short s) {
        try {
    ...
    private synchronized void writeFloat(float v) {
        try {
    ...
    private synchronized void writeString(String s) {
        if (s == null) {
            s = "";
        }
        try {
