...
public final class MifareClassic extends BasicTagTechnology {
    ...
    public synchronized MifareClassic(Tag tag) throws RemoteException {
        super(tag, 8);
        NfcA a = NfcA.get(tag);
    ...
    public synchronized boolean isEmulated() {
    ...
    private synchronized boolean authenticate(int sector, byte[] key, boolean keyA) throws IOException {
    ...
    private static synchronized void validateSector(int sector) {
        if (sector < 0 || sector >= 40) {
            throw new IndexOutOfBoundsException("sector out of bounds: " + sector);
        }
    }

    private static synchronized void validateBlock(int block) {
        if (block < 0 || block >= 256) {
            throw new IndexOutOfBoundsException("block out of bounds: " + block);
        }
    }

    private static synchronized void validateValueOperand(int value) {
        if (value < 0) {
