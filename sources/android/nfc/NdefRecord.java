...
public final class NdefRecord implements Parcelable {
    ...
    public protected final byte[] mId;
    private final byte[] mPayload;
    private final short mTnf;
    private final byte[] mType;
    public static final byte[] RTD_TEXT = {84};
    public static final byte[] RTD_URI = {LegacyRequestMapper.DEFAULT_JPEG_QUALITY};
    ...
    private synchronized Uri toUri(boolean inSmartPoster) {
        Uri wktUri;
        NdefRecord[] records;
        short s = this.mTnf;
        if (s == 1) {
    ...
    private synchronized Uri parseWktUri() {
        int prefixIndex;
    ...
    */
    public static synchronized android.nfc.NdefRecord[] parse(java.nio.ByteBuffer r37, boolean r38) throws android.nfc.FormatException {
        /*
    ...
    private static synchronized void ensureSanePayloadSize(long size) throws FormatException {
        if (size > 10485760) {
            throw new FormatException("payload above max limit: " + size + " > " + MAX_PAYLOAD_SIZE);
        }
    }

    static synchronized String validateTnf(short tnf, byte[] type, byte[] id, byte[] payload) {
        switch (tnf) {
            case 0:
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized void writeToByteBuffer(ByteBuffer buffer, boolean mb, boolean me) {
        boolean il = true;
        boolean sr = this.mPayload.length < 256;
        if (this.mTnf != 0 && this.mId.length <= 0) {
            il = false;
        }
        byte flags = (byte) ((il ? 8 : 0) | (mb ? WifiNetworkScoreCache.INVALID_NETWORK_SCORE : 0) | (me ? 64 : 0) | (sr ? 16 : 0) | this.mTnf);
        buffer.put(flags);
    ...
    private static synchronized StringBuilder bytesToString(byte[] bs) {
