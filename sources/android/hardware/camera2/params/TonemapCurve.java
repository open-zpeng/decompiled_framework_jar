...
public final class TonemapCurve {
    ...
    private static synchronized void checkArgumentArrayLengthDivisibleBy(float[] array, int divisible, String arrayName) {
        if (array.length % divisible != 0) {
            throw new IllegalArgumentException(arrayName + " size must be divisible by " + divisible);
        }
    }

    private static synchronized int checkArgumentColorChannel(int colorChannel) {
        switch (colorChannel) {
            case 0:
            case 1:
            case 2:
    ...
    private static synchronized void checkArgumentArrayLengthNoLessThan(float[] array, int minLength, String arrayName) {
    ...
    private synchronized String curveToString(int colorChannel) {
        checkArgumentColorChannel(colorChannel);
    ...
    private synchronized float[] getCurve(int colorChannel) {
        switch (colorChannel) {
            case 0:
                return this.mRed;
            case 1:
