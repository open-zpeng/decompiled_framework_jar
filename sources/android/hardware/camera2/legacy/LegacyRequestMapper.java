...
public class LegacyRequestMapper {
    ...
    public protected static final byte DEFAULT_JPEG_QUALITY = 85;
    public protected static final String TAG = "LegacyRequestMapper";

    private protected synchronized LegacyRequestMapper() {
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static synchronized void convertRequestMetadata(LegacyRequest legacyRequest) {
        String legacyMode;
        String modeToSet;
        ...
        if (aeFpsRange != null) {
            ...
            while (true) {
                ...
                int[] range = it.next();
                int intRangeLow = ((int) Math.floor(range[c2] / 1000.0d)) * 1000;
                int intRangeHigh = ((int) Math.ceil(range[c] / 1000.0d)) * 1000;
        ...
        if (!compensationRange.contains((Range<Integer>) Integer.valueOf(compensation))) {
    ...
    public protected static synchronized boolean checkForCompleteGpsData(Location location) {
        return (location == null || location.getProvider() == null || location.getTime() == 0) ? false : true;
    }

    public private protected static synchronized int filterSupportedCaptureIntent(int captureIntent) {
    ...
    public protected static synchronized List<Camera.Area> convertMeteringRegionsToLegacy(Rect activeArray, ParameterUtils.ZoomData zoomData, MeteringRectangle[] meteringRegions, int maxNumMeteringAreas, String regionName) {
        ...
        int i2 = meteringRectangleList.size();
        if (maxNumMeteringAreas < i2) {
    ...
    public protected static synchronized void mapAeAndFlashMode(CaptureRequest r, Camera.Parameters p) {
    ...
    public protected static synchronized String convertAeAntiBandingModeToLegacy(int mode) {
        switch (mode) {
            case 0:
                return "off";
            case 1:
    ...
    public protected static synchronized int[] convertAeFpsRangeToLegacy(Range<Integer> fpsRange) {
        int[] legacyFps = {fpsRange.getLower().intValue() * 1000, fpsRange.getUpper().intValue() * 1000};
        return legacyFps;
    }

    public protected static synchronized String convertAwbModeToLegacy(int mode) {
        switch (mode) {
            case 1:
                return "auto";
            case 2:
    ...
    public protected static synchronized <T> T getIfSupported(CaptureRequest r, CaptureRequest.Key<T> key, T defaultValue, boolean isSupported, T allowedValue) {
