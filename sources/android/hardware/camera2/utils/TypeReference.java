...
public abstract class TypeReference<T> {
    ...
    private synchronized TypeReference(Type type) {
        this.mType = type;
        if (containsTypeVariable(this.mType)) {
            throw new IllegalArgumentException("Including a type variable in a type reference is not allowed");
        }
    ...
    private static class SpecializedTypeReference<T> extends TypeReference<T> {
        public synchronized SpecializedTypeReference(Class<T> klass) {
            super(klass);
        }
    }
    ...
    public static class SpecializedBaseTypeReference extends TypeReference {
        public synchronized SpecializedBaseTypeReference(Type type) {
            super(type);
        }
    }

    public static synchronized <T> TypeReference<T> createSpecializedTypeReference(Class<T> klass) {
    ...
    public final synchronized Class<? super T> getRawType() {
        return (Class<? super T>) getRawType(this.mType);
    }

    private static final synchronized Class<?> getRawType(Type type) {
        if (type == null) {
    ...
    private static final synchronized Class<?> getRawType(Type[] types) {
        if (types == null) {
            return null;
        }
        for (Type type : types) {
    ...
    private static final synchronized Class<?> getArrayClass(Class<?> componentType) {
        return Array.newInstance(componentType, 0).getClass();
    }

    public synchronized TypeReference<?> getComponentType() {
    ...
    private static synchronized Type getComponentType(Type type) {
    ...
    public static synchronized boolean containsTypeVariable(Type type) {
    ...
    private static synchronized void toString(Type type, StringBuilder out) {
        if (type == null) {
            return;
        }
        if (type instanceof TypeVariable) {
    ...
    private static synchronized void toString(Type[] types, StringBuilder out) {
    ...
    private static synchronized boolean containsTypeVariable(Type[] typeArray) {
        if (typeArray == null) {
            return false;
        }
