...
public class CloseableLock implements AutoCloseable {
    ...
    public synchronized CloseableLock() {
    ...
    public synchronized CloseableLock(String name) {
        this.TAG = "CloseableLock";
        this.mClosed = false;
        this.mExclusive = false;
        this.mSharedLocks = 0;
    ...
    public synchronized ScopedLock acquireLock() {
        try {
            this.mLock.lock();
            if (this.mClosed) {
                return null;
    ...
    public synchronized ScopedLock acquireExclusiveLock() {
        try {
            this.mLock.lock();
            if (this.mClosed) {
                return null;
    ...
    private synchronized void log(String what) {
