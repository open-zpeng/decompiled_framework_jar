...
public class CameraAccessException extends AndroidException {
    ...
    public protected static final long serialVersionUID = 5630338637471475675L;
    ...
    public static synchronized String getDefaultMessage(int problem) {
        switch (problem) {
            case 1:
    ...
    private static synchronized String getCombinedMessage(int problem, String message) {
        String problemString = getProblemString(problem);
        return String.format("%s (%d): %s", problemString, Integer.valueOf(problem), message);
    }

    private static synchronized String getProblemString(int problem) {
        if (problem != 1000) {
            switch (problem) {
                case 1:
