...
public abstract class CameraMetadata<TKey> {
    ...
    protected abstract synchronized Class<TKey> getKeyClass();

    protected abstract synchronized <T> T getProtected(TKey tkey);

    /* JADX INFO: Access modifiers changed from: protected */
    public synchronized void setNativeInstance(CameraMetadataNative nativeInstance) {
    ...
    public synchronized <TKey> ArrayList<TKey> getKeys(Class<?> type, Class<TKey> keyClass, CameraMetadata<TKey> instance, int[] filterTags) {
        String keyName;
        long vendorId;
        ...
        for (Field field : fields) {
            if (field.getType().isAssignableFrom(keyClass) && (field.getModifiers() & 1) != 0) {
                try {
                    ...
                    if ((instance == 0 || instance.getProtected(obj) != null) && shouldKeyBeAdded(obj, field, filterTags)) {
                        keyList.add(obj);
                    }
    ...
    private static synchronized <TKey> boolean shouldKeyBeAdded(TKey key, Field field, int[] filterTags) {
