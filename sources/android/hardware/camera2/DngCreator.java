...
public final class DngCreator implements AutoCloseable {
    ...
    public DngCreator setLocation(Location location) {
        ...
        String latRef = latitude >= FeatureOption.FO_BOOT_POLICY_CPU ? GPS_LAT_REF_NORTH : GPS_LAT_REF_SOUTH;
        String longRef = longitude >= FeatureOption.FO_BOOT_POLICY_CPU ? GPS_LONG_REF_EAST : GPS_LONG_REF_WEST;
    ...
    private synchronized void writeByteBuffer(int width, int height, ByteBuffer pixels, OutputStream dngOutput, int pixelStride, int rowStride, long offset) throws IOException {
        if (width <= 0 || height <= 0) {
    ...
    private static synchronized void yuvToRgb(byte[] yuvData, int outOffset, byte[] rgbOut) {
        float y = yuvData[0] & 255;
        float cb = yuvData[1] & 255;
        float cr = yuvData[2] & 255;
        float r = (1.402f * (cr - 128.0f)) + y;
    ...
    private static synchronized void colorToRgb(int color, int outOffset, byte[] rgbOut) {
        rgbOut[outOffset] = (byte) Color.red(color);
        rgbOut[outOffset + 1] = (byte) Color.green(color);
        rgbOut[outOffset + 2] = (byte) Color.blue(color);
    }

    private static synchronized ByteBuffer convertToRGB(Image yuvImage) {
        ...
        while (true) {
            ...
            if (i2 < height) {
                ...
                yBuf.get(yFullRow);
                uBuf.position(uRowStride * i3);
                uBuf.get(uFullRow);
                vBuf.position(vRowStride * i3);
                vBuf.get(vFullRow);
                int j = 0;
                while (j < width) {
                    int halfW = j / 2;
    ...
    private static synchronized ByteBuffer convertToRGB(Bitmap argbBitmap) {
    ...
    private static synchronized int[] toExifLatLong(double value) {
