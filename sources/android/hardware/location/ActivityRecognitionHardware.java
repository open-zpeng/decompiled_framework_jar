...
public class ActivityRecognitionHardware extends IActivityRecognitionHardware.Stub {
    ...
    private synchronized ActivityRecognitionHardware(Context context) {
        nativeInitialize();
    ...
    public static synchronized ActivityRecognitionHardware getInstance(Context context) {
    ...
    public synchronized String[] getSupportedActivities() {
        checkPermissions();
        return this.mSupportedActivities;
    }

    @Override // android.hardware.location.IActivityRecognitionHardware
    public synchronized boolean isActivitySupported(String activity) {
        checkPermissions();
    ...
    @Override // android.hardware.location.IActivityRecognitionHardware
    public synchronized boolean registerSink(IActivityRecognitionHardwareSink sink) {
        checkPermissions();
        return this.mSinks.register(sink);
    }

    @Override // android.hardware.location.IActivityRecognitionHardware
    public synchronized boolean unregisterSink(IActivityRecognitionHardwareSink sink) {
        checkPermissions();
        return this.mSinks.unregister(sink);
    }

    @Override // android.hardware.location.IActivityRecognitionHardware
    public synchronized boolean enableActivityEvent(String activity, int eventType, long reportLatencyNs) {
        checkPermissions();
        int activityType = getActivityType(activity);
        if (activityType == -1) {
            return false;
    ...
    @Override // android.hardware.location.IActivityRecognitionHardware
    public synchronized boolean disableActivityEvent(String activity, int eventType) {
        checkPermissions();
        int activityType = getActivityType(activity);
        if (activityType == -1) {
            return false;
    ...
    private synchronized void onActivityChanged(Event[] events) {
    ...
    private synchronized String getActivityName(int activityType) {
    ...
    private synchronized int getActivityType(String activity) {
    ...
    private synchronized String[] fetchSupportedActivities() {
    ...
    private class SinkList extends RemoteCallbackList<IActivityRecognitionHardwareSink> {
        ...
        public synchronized void onCallbackDied(IActivityRecognitionHardwareSink callback) {
        ...
        private synchronized void disableActivityEventIfEnabled(int activityType, int eventType) {
