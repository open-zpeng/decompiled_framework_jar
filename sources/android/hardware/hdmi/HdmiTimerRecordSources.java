...
public class HdmiTimerRecordSources {
    ...
    private synchronized HdmiTimerRecordSources() {
    }
    ...
    private static synchronized void checkTimerRecordSourceInputs(TimerInfo timerInfo, HdmiRecordSources.RecordSource source) {
        if (timerInfo == null) {
    ...
    private static synchronized void checkTimeValue(int hour, int minute) {
        if (hour < 0 || hour > 23) {
    ...
    private static synchronized void checkDurationValue(int hour, int minute) {
        if (hour < 0 || hour > 99) {
    ...
    public static class TimeUnit {
        ...
        synchronized TimeUnit(int hour, int minute) {
            this.mHour = hour;
            this.mMinute = minute;
        }

        synchronized int toByteArray(byte[] data, int index) {
            data[index] = toBcdByte(this.mHour);
            data[index + 1] = toBcdByte(this.mMinute);
            return 2;
        }

        static synchronized byte toBcdByte(int value) {
    ...
    public static final class Time extends TimeUnit {
        private synchronized Time(int hour, int minute) {
            super(hour, minute);
        }
    }
    ...
    public static final class Duration extends TimeUnit {
        private synchronized Duration(int hour, int minute) {
            super(hour, minute);
        }
    }
    ...
    public static final class TimerInfo {
        ...
        private synchronized TimerInfo(int dayOfMonth, int monthOfYear, Time startTime, Duration duration, int recordingSequence) {
    ...
    public static final class TimerRecordSource {
        ...
        private synchronized TimerRecordSource(TimerInfo timerInfo, HdmiRecordSources.RecordSource recordSource) {
    ...
    private static class ExternalSourceDecorator extends HdmiRecordSources.RecordSource {
        ...
        private synchronized ExternalSourceDecorator(HdmiRecordSources.RecordSource recordSource, int externalSourceSpecifier) {
