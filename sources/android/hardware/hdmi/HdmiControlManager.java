...
public final class HdmiControlManager {
    ...
    public synchronized HdmiControlManager(IHdmiControlService service) {
    ...
    private static synchronized boolean hasDeviceType(int[] types, int type) {
        if (types == null) {
            return false;
        }
        for (int t : types) {
    ...
    public HdmiClient getClient(int type) {
        ...
        if (type == 0) {
            ...
            return null;
        } else if (type == 4 && this.mHasPlaybackDevice) {
    ...
    private synchronized IHdmiHotplugEventListener getHotplugEventListenerWrapper(final HotplugEventListener listener) {
