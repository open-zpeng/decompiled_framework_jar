...
public class Camera {
    ...
    private protected static final int CAMERA_HAL_API_VERSION_1_0 = 256;
    ...
    private static HashMap<Integer, Camera> mIdToCameraHandleMap = new HashMap<>();
    private String cur_params;
    private IAppOpsService mAppOps;
    private IAppOpsCallback mAppOpsCallback;
    private AutoFocusCallback mAutoFocusCallback;
    private AutoFocusMoveCallback mAutoFocusMoveCallback;
    public int mCamId;
    private CmdControlCallback mCmdControlCallback;
    ...
    private final HashMap<String, List<registerCmdControlCallback>> mkeyToCallbacksMap = new HashMap<>();
    ...
    public interface CmdControlCallback {
        void onCmdParasData(Parameters parameters, Camera camera);
    }
    ...
    /* loaded from: classes.dex */
    public interface registerCmdControlCallback {
        void onChangeEvent(String str, int i);
    ...
    private final native String native_getCmdControlParameters();

    public protected final native String native_getParameters();

    private final native void native_release();

    private final native int native_setCmdControlParameter(int i, int i2);

    public protected final native void native_setParameters(String str);

    public protected final native int native_setup(Object obj, int i, int i2, String str);
    ...
    public final native void native_setCmdControlParameters(String str);

    private protected final native boolean previewEnabled();

    public final native void reconnect() throws IOException;

    public final native void setDisplayOrientation(int i);

    private protected final native void setPreviewSurface(Surface surface) throws IOException;
    ...
    public void registerPropCallback(ArrayList<String> ids, registerCmdControlCallback callback) {
        if (ids == null) {
            Log.d(TAG, "ids is null");
            ...
            while (it.hasNext()) {
                String id = it.next();
                List<registerCmdControlCallback> callbacks = this.mkeyToCallbacksMap.get(id);
                if (callbacks == null) {
                    callbacks = new ArrayList();
                    this.mkeyToCallbacksMap.put(id, callbacks);
                }
    ...
    public synchronized void unregisterPropCallback(ArrayList<String> ids, registerCmdControlCallback callback) {
        Log.d(TAG, "unregisterPropCallback: " + callback);
        if (ids == null) {
            Log.d(TAG, "ids is null");
            throw new IllegalArgumentException("input data cannot be null");
        } else if (callback == null) {
            ...
            while (it.hasNext()) {
                String id = it.next();
                Log.d(TAG, "unregisterPropCallback id: " + id);
                List<registerCmdControlCallback> callbacks = this.mkeyToCallbacksMap.get(id);
                if (callbacks != null) {
                    callbacks.remove(callback);
                    if (callbacks.isEmpty()) {
                        this.mkeyToCallbacksMap.remove(id);
                    }
                }
            }
        }
    ...
    public static Camera open(int cameraId) {
        ...
        camera.mCamId = cameraId;
        mIdToCameraHandleMap.put(Integer.valueOf(camera.mCamId), camera);
        return camera;
    }

    public static Camera getCameraHandle(int cameraId) {
        Camera camera = mIdToCameraHandleMap.get(Integer.valueOf(cameraId));
        return camera;
    }
    ...
    private protected static Camera openLegacy(int cameraId, int halVersion) {
        if (halVersion < 256) {
    ...
    private synchronized Camera(int cameraId, int halVersion) {
    ...
    private synchronized int cameraInitVersion(int cameraId, int halVersion) {
    ...
    private synchronized int cameraInitNormal(int cameraId) {
        return cameraInitVersion(cameraId, -2);
    }

    public synchronized int cameraInitUnspecified(int cameraId) {
        return cameraInitVersion(cameraId, -1);
    }

    synchronized Camera(int cameraId) {
    ...
    public static synchronized boolean checkInitErrors(int err) {
        return err != 0;
    }

    public static synchronized Camera openUninitialized() {
        return new Camera();
    }
    ...
    private synchronized void initAppOps() {
    ...
    private synchronized void releaseAppOps() {
        try {
    ...
    public final void release() {
        ...
        releaseAppOps();
        mIdToCameraHandleMap.remove(Integer.valueOf(this.mCamId));
        Log.e(TAG, "release size=" + mIdToCameraHandleMap.size());
        Log.e(TAG, "mCamId=" + this.mCamId);
    }
    ...
    public final void setCmdControlCallback(CmdControlCallback cb) {
        this.mCmdControlCallback = cb;
    }
    ...
    public final void set(byte[] callbackBuffer) {
        _addCallbackBuffer(callbackBuffer, 16);
    }

    private protected final void addRawImageCallbackBuffer(byte[] callbackBuffer) {
        addCallbackBuffer(callbackBuffer, 128);
    }

    public protected final void addCallbackBuffer(byte[] callbackBuffer, int msgType) {
    ...
    public final synchronized Allocation createPreviewAllocation(RenderScript rs, int usage) throws RSIllegalArgumentException {
    ...
    public final synchronized void setPreviewCallbackAllocation(Allocation previewAllocation) throws IOException {
    ...
    public protected static void postEventFromNative(Object camera_ref, int what, int arg1, int arg2, Object obj) {
    ...
    private static void postCmdControlParams(Object camera_ref, String params) {
        Log.e(TAG, " postCmdControlParams" + params);
        Camera c = (Camera) ((WeakReference) camera_ref).get();
        if (c == null) {
            return;
        }
        c.handleCmdControlParams(camera_ref, params);
    }

    private void handleCmdControlParams(Object camera_ref, String params) {
        Log.e(TAG, " handleCmdControlParams new" + params);
        Log.e(TAG, " handleCmdControlParams cur" + this.cur_params);
        Camera c = (Camera) ((WeakReference) camera_ref).get();
        if (c == null) {
            return;
        }
        String[] cur_value = new String[10];
        String[] new_value = new String[10];
        Parameters p = new Parameters();
        String[] key = {"display-mode", "overlay-workst", "transparentchassis-workst", "work-status", "out-mode"};
        if (p.same(this.cur_params, params)) {
            return;
        }
        p.unflatten(params);
        int value = 0;
        new_value[0] = p.get("display-mode");
        new_value[1] = p.get("overlay-workst");
        new_value[2] = p.get("transparentchassis-workst");
        new_value[3] = p.get("work-status");
        new_value[4] = p.get("out-mode");
        if (this.cur_params == null) {
            this.cur_params = params;
        } else {
            p.unflatten(this.cur_params);
            cur_value[0] = p.get("display-mode");
            cur_value[1] = p.get("overlay-workst");
            cur_value[2] = p.get("transparentchassis-workst");
            cur_value[3] = p.get("work-status");
            cur_value[4] = p.get("out-mode");
        }
        this.cur_params = params;
        while (true) {
            int i = value;
            ...
            if (!p.same(new_value[i], cur_value[i])) {
                int value2 = Integer.parseInt(new_value[i]);
                handleOnChangeEvent(key[i], value2);
            }
            value = i + 1;
        }
    }

    protected void handleOnChangeEvent(String key, int value) {
        List<registerCmdControlCallback> callbacks;
        synchronized (this) {
            callbacks = this.mkeyToCallbacksMap.get(key);
        }
        if (callbacks == null) {
            Log.w(TAG, "handleOnChangeEvent callback is null");
            return;
        }
        for (registerCmdControlCallback l : callbacks) {
            Log.d(TAG, "++++handleOnChangeEvent: " + key + " callback:" + l);
            l.onChangeEvent(key, value);
        }
    }
    ...
    public final synchronized boolean disableShutterSound() {
    ...
    public static class IAppOpsCallbackWrapper extends IAppOpsCallback.Stub {
        ...
        synchronized IAppOpsCallbackWrapper(Camera camera) {
    ...
    public final synchronized void setDetailedErrorCallback(ErrorCallback cb) {
    ...
    public static synchronized Parameters getParametersCopy(Parameters parameters) {
        if (parameters == null) {
    ...
    public void setCmdControlParameters(Parameters params) {
        native_setCmdControlParameters(params.flatten());
    }

    public Parameters getCmdControlParameters() {
        Parameters p = new Parameters();
        String s = native_getCmdControlParameters();
        p.unflatten(s);
        return p;
    }
    ...
    public class Parameters {
        ...
        private static final String AVM_CALIBRATION = "calibration";
        private static final String AVM_DISPLAYMODE = "display-mode";
        private static final String AVM_FINETUNEMODE = "finetune-mode";
        private static final String AVM_OVERLAYWORKST = "overlay-workst";
        private static final String AVM_TRANSPCWORKST = "transparentchassis-workst";
        private static final String AVM_WORKSTATUS = "work-status";
        private static final String CIU_OUTMODE = "out-mode";
        ...
        private static final String PIXEL_FORMAT_RGB888 = "rgb888";
        ...
        /* JADX INFO: Access modifiers changed from: private */
        public synchronized Camera getOuter() {
            return Camera.this;
        }

        public synchronized boolean same(Parameters other) {
            if (this == other) {
                return true;
            }
        ...
        private synchronized void set(String key, List<Area> areas) {
            if (areas == null) {
                set(key, "(0,0,0,0,0)");
                return;
            }
        ...
        private synchronized String cameraFormatForPixelFormat(int pixel_format) {
            if (pixel_format != 4) {
                if (pixel_format != 20) {
                    if (pixel_format != 41) {
                        if (pixel_format != 256) {
        ...
        private synchronized int pixelFormatForCameraFormat(String format) {
            if (format == null) {
                return 0;
            }
            ...
            if (!format.equals(PIXEL_FORMAT_RGB888)) {
                return 0;
            }
            return 41;
        }
        ...
        public ArrayList<String> getRegisterPropertyIds() {
            ArrayList<String> propertyIds = new ArrayList<>();
            propertyIds.add(AVM_DISPLAYMODE);
            propertyIds.add(AVM_OVERLAYWORKST);
            propertyIds.add(AVM_TRANSPCWORKST);
            propertyIds.add(AVM_WORKSTATUS);
            propertyIds.add(CIU_OUTMODE);
            return propertyIds;
        }

        public void setAvmDisplayMode(int value) {
            set(AVM_DISPLAYMODE, value);
        }

        public void setAvmCalibration(int value) {
            set(AVM_CALIBRATION, value);
        }

        public void setAvmOverlayWorkSt(int value) {
            set(AVM_OVERLAYWORKST, value);
        }

        public void setAvmTransparentChassisWorkSt(int value) {
            set(AVM_TRANSPCWORKST, value);
        }

        public void setAvmFineTuneMode(int value) {
            set(AVM_FINETUNEMODE, value);
        }

        public void setAvmOutMode(int value) {
            set(CIU_OUTMODE, value);
        }

        public void setAvmWorkState(int value) {
            set(AVM_WORKSTATUS, value);
        }

        public int getAvmDisplayMode() {
            return getInt(AVM_DISPLAYMODE);
        }

        public int getAvmCalibration() {
            return getInt(AVM_CALIBRATION);
        }

        public int getAvmOverlayWorkSt() {
            return getInt(AVM_OVERLAYWORKST);
        }

        public int getAvmTransparentChassisWorkSt() {
            return getInt(AVM_TRANSPCWORKST);
        }

        public int getAvmFineTuneMode() {
            return getInt(AVM_FINETUNEMODE);
        }

        public int getAvmWorkState() {
            return getInt(AVM_WORKSTATUS);
        }

        private synchronized ArrayList<String> split(String str) {
            if (str == null) {
                return null;
            }
            TextUtils.StringSplitter splitter = new TextUtils.SimpleStringSplitter(',');
        ...
        private synchronized ArrayList<Integer> splitInt(String str) {
            if (str == null) {
                return null;
            }
            TextUtils.StringSplitter splitter = new TextUtils.SimpleStringSplitter(',');
        ...
        private synchronized void splitInt(String str, int[] output) {
            if (str == null) {
                return;
            }
            TextUtils.StringSplitter splitter = new TextUtils.SimpleStringSplitter(',');
        ...
        private synchronized void splitFloat(String str, float[] output) {
            if (str == null) {
                return;
            }
            TextUtils.StringSplitter splitter = new TextUtils.SimpleStringSplitter(',');
        ...
        private synchronized float getFloat(String key, float defaultValue) {
            try {
        ...
        private synchronized ArrayList<Size> splitSize(String str) {
            if (str == null) {
                return null;
            }
            TextUtils.StringSplitter splitter = new TextUtils.SimpleStringSplitter(',');
        ...
        private synchronized Size strToSize(String str) {
            if (str == null) {
                return null;
            }
        ...
        private synchronized ArrayList<int[]> splitRange(String str) {
            int endIndex;
        ...
        public protected ArrayList<Area> splitArea(String str) {
            int endIndex;
            if (str == null || str.charAt(0) != '(' || str.charAt(str.length() - 1) != ')') {
        ...
        /* JADX INFO: Access modifiers changed from: private */
        public synchronized boolean same(String s1, String s2) {
            if (s1 == null && s2 == null) {
                return true;
            }
