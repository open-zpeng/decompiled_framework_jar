...
public abstract class SensorManager {
    ...
    protected abstract synchronized boolean cancelTriggerSensorImpl(TriggerEventListener triggerEventListener, Sensor sensor, boolean z);

    /* JADX INFO: Access modifiers changed from: protected */
    public abstract synchronized int configureDirectChannelImpl(SensorDirectChannel sensorDirectChannel, Sensor sensor, int i);

    protected abstract synchronized SensorDirectChannel createDirectChannelImpl(MemoryFile memoryFile, HardwareBuffer hardwareBuffer);

    protected abstract synchronized void destroyDirectChannelImpl(SensorDirectChannel sensorDirectChannel);

    protected abstract synchronized boolean flushImpl(SensorEventListener sensorEventListener);

    protected abstract synchronized List<Sensor> getFullDynamicSensorList();

    /* JADX INFO: Access modifiers changed from: protected */
    public abstract synchronized List<Sensor> getFullSensorList();

    protected abstract synchronized boolean initDataInjectionImpl(boolean z);

    protected abstract synchronized boolean injectSensorDataImpl(Sensor sensor, float[] fArr, int i, long j);

    protected abstract synchronized void registerDynamicSensorCallbackImpl(DynamicSensorCallback dynamicSensorCallback, Handler handler);

    protected abstract synchronized boolean registerListenerImpl(SensorEventListener sensorEventListener, Sensor sensor, int i, Handler handler, int i2, int i3);

    protected abstract synchronized boolean requestTriggerSensorImpl(TriggerEventListener triggerEventListener, Sensor sensor);

    protected abstract synchronized boolean setOperationParameterImpl(SensorAdditionalInfo sensorAdditionalInfo);

    protected abstract synchronized void unregisterDynamicSensorCallbackImpl(DynamicSensorCallback dynamicSensorCallback);

    protected abstract synchronized void unregisterListenerImpl(SensorEventListener sensorEventListener, Sensor sensor);
    ...
    public Sensor getDefaultSensor(int type) {
        ...
        wakeUpSensor = (type == 8 || type == 17 || type == 22 || type == 23 || type == 24 || type == 25 || type == 26 || type == 32) ? true : true;
        for (Sensor sensor : l) {
    ...
    public synchronized void destroyDirectChannel(SensorDirectChannel channel) {
        destroyDirectChannelImpl(channel);
    }

    @Deprecated
    private protected int configureDirectChannel(SensorDirectChannel channel, Sensor sensor, int rateLevel) {
    ...
    private static synchronized boolean remapCoordinateSystemImpl(float[] inR, int X, int Y, float[] outR) {
        ...
        if ((X & 124) == 0 && (Y & 124) == 0) {
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    if (i4 < i) {
                        if (x == i4) {
                            outR[j2 + i4] = sx ? -inR[j2 + 0] : inR[j2 + 0];
                        }
                        if (y == i4) {
                            outR[j2 + i4] = sy ? -inR[j2 + 1] : inR[j2 + 1];
                        }
                        if (z == i4) {
                            outR[j2 + i4] = sz ? -inR[j2 + 2] : inR[j2 + 2];
                        }
                        i3 = i4 + 1;
                        i = 3;
                    }
    ...
    private synchronized LegacySensorManager getLegacySensorManager() {
    ...
    private static synchronized int getDelay(int rate) {
        switch (rate) {
            case 0:
                return 0;
            case 1:
    ...
    public synchronized boolean setOperationParameter(SensorAdditionalInfo parameter) {
