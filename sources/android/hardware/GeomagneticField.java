...
public class GeomagneticField {
    ...
    public GeomagneticField(float gdLatitudeDeg, float gdLongitudeDeg, float altitudeMeters, long timeMillis) {
        ...
        while (n < MAX_N) {
            ...
            while (m2 <= n) {
                ...
                gcZ2 += relativeRadiusPower[n + 2] * m2 * ((sinMLon[m2] * g) - (cosMLon[m2] * h)) * legendre.mP[n][m2] * SCHMIDT_QUASI_NORM_FACTORS[n][m2] * inverseCosLatitude;
        ...
        this.mX = (float) ((gcX * Math.cos(latDiffRad)) + (gcY2 * Math.sin(latDiffRad)));
        this.mY = gcY;
        this.mZ = (float) (((-gcX) * Math.sin(latDiffRad)) + (gcY2 * Math.cos(latDiffRad)));
    }
    ...
    private synchronized void computeGeocentricCoordinates(float gdLatitudeDeg, float gdLongitudeDeg, float altitudeMeters) {
    ...
    private static class LegendreTable {
        ...
        public synchronized LegendreTable(int maxN, float thetaRad) {
    ...
    private static synchronized float[][] computeSchmidtQuasiNormFactors(int maxN) {
