...
public class NetworkSecurityTrustManager extends X509ExtendedTrustManager {
    ...
    public synchronized NetworkSecurityTrustManager(NetworkSecurityConfig config) {
        if (config == null) {
    ...
    public synchronized List<X509Certificate> checkServerTrusted(X509Certificate[] certs, String authType, String host) throws CertificateException {
        List<X509Certificate> trustedChain = this.mDelegate.checkServerTrusted(certs, authType, host);
        checkPins(trustedChain);
        return trustedChain;
    }

    private synchronized void checkPins(List<X509Certificate> chain) throws CertificateException {
    ...
    private synchronized boolean isPinningEnforced(List<X509Certificate> chain) throws CertificateException {
        if (chain.isEmpty()) {
            return false;
        }
