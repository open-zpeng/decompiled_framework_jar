...
public class KeyStoreCryptoOperationChunkedStreamer implements KeyStoreCryptoOperationStreamer {
    ...
    public interface Stream {
        synchronized OperationResult finish(byte[] bArr, byte[] bArr2);

        synchronized OperationResult update(byte[] bArr);
    }

    public synchronized KeyStoreCryptoOperationChunkedStreamer(Stream operation) {
        this(operation, 65536);
    }

    public synchronized KeyStoreCryptoOperationChunkedStreamer(Stream operation, int maxChunkSize) {
    ...
    public synchronized byte[] update(byte[] input, int inputOffset, int inputLength) throws KeyStoreException {
        byte[] result;
        byte[] chunk;
    ...
    @Override // android.security.keystore.KeyStoreCryptoOperationStreamer
    public synchronized byte[] doFinal(byte[] input, int inputOffset, int inputLength, byte[] signature, byte[] additionalEntropy) throws KeyStoreException {
        if (inputLength == 0) {
    ...
    public synchronized byte[] flush() throws KeyStoreException {
        String str;
    ...
    @Override // android.security.keystore.KeyStoreCryptoOperationStreamer
    public synchronized long getConsumedInputSizeBytes() {
        return this.mConsumedInputSizeBytes;
    }

    @Override // android.security.keystore.KeyStoreCryptoOperationStreamer
    public synchronized long getProducedOutputSizeBytes() {
    ...
    public static class MainDataStream implements Stream {
        ...
        public synchronized MainDataStream(KeyStore keyStore, IBinder operationToken) {
            this.mKeyStore = keyStore;
            this.mOperationToken = operationToken;
        }

        @Override // android.security.keystore.KeyStoreCryptoOperationChunkedStreamer.Stream
        public synchronized OperationResult update(byte[] input) {
            return this.mKeyStore.update(this.mOperationToken, null, input);
        }

        @Override // android.security.keystore.KeyStoreCryptoOperationChunkedStreamer.Stream
        public synchronized OperationResult finish(byte[] signature, byte[] additionalEntropy) {
