...
public final class RecoveryCertPath implements Parcelable {
    ...
    public static synchronized RecoveryCertPath createRecoveryCertPath(CertPath certPath) throws CertificateException {
        try {
    ...
    public synchronized CertPath getCertPath() throws CertificateException {
        return decodeCertPath(this.mEncodedCertPath);
    }

    private synchronized RecoveryCertPath(byte[] encodedCertPath) {
        this.mEncodedCertPath = (byte[]) Preconditions.checkNotNull(encodedCertPath);
    }

    private synchronized RecoveryCertPath(Parcel in) {
    ...
    private static synchronized byte[] encodeCertPath(CertPath certPath) throws CertificateEncodingException {
        Preconditions.checkNotNull(certPath);
        return certPath.getEncoded(CERT_PATH_ENCODING);
    }

    private static synchronized CertPath decodeCertPath(byte[] bytes) throws CertificateException {
