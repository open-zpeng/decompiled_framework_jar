...
public class SystemKeyStore {
    ...
    private synchronized SystemKeyStore() {
    }
    ...
    public static synchronized String toHexString(byte[] keyData) {
        if (keyData == null) {
            return null;
        }
    ...
    public synchronized String generateNewKeyHexString(int numBits, String algName, String keyName) throws NoSuchAlgorithmException {
        return toHexString(generateNewKey(numBits, algName, keyName));
    }

    public synchronized byte[] generateNewKey(int numBits, String algName, String keyName) throws NoSuchAlgorithmException {
    ...
    private synchronized File getKeyFile(String keyName) {
    ...
    public synchronized String retrieveKeyHexString(String keyName) throws IOException {
        return toHexString(retrieveKey(keyName));
    }

    public synchronized byte[] retrieveKey(String keyName) throws IOException {
        File keyFile = getKeyFile(keyName);
        if (!keyFile.exists()) {
            return null;
        }
    ...
    public synchronized void deleteKey(String keyName) {
        File keyFile = getKeyFile(keyName);
        if (!keyFile.exists()) {
            throw new IllegalArgumentException();
        }
