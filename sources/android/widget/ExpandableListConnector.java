...
class ExpandableListConnector extends BaseAdapter implements Filterable {
    ...
    public synchronized ExpandableListConnector(ExpandableListAdapter expandableListAdapter) {
        setExpandableListAdapter(expandableListAdapter);
    }

    public synchronized void setExpandableListAdapter(ExpandableListAdapter expandableListAdapter) {
    ...
    public synchronized PositionMetadata getUnflattenedPos(int flPos) {
        int insertPosition;
        int groupPos;
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized PositionMetadata getFlattenedPos(ExpandableListPosition pos) {
        ArrayList<GroupMetadata> egml = this.mExpGroupMetadataList;
        int numExpGroups = egml.size();
        int leftExpGroupIndex = 0;
        int rightExpGroupIndex = numExpGroups - 1;
    ...
    public int getItemViewType(int flatListPos) {
        ...
        if (this.mExpandableListAdapter instanceof HeterogeneousExpandableList) {
            ...
            if (pos.type == 2) {
                ...
                int childType2 = adapter.getChildType(retValue2, pos.childPos);
                childType = childType2 + adapter.getGroupTypeCount();
            }
            retValue = childType;
        } else {
            int retValue3 = pos.type;
            if (retValue3 == 2) {
                retValue = 0;
            } else {
                retValue = 1;
    ...
    public synchronized void refreshExpGroupMetadataList(boolean forceChildrenCountRefresh, boolean syncGroupPositions) {
        int gChildrenCount;
        ArrayList<GroupMetadata> egml = this.mExpGroupMetadataList;
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized boolean collapseGroup(PositionMetadata posMetadata) {
    ...
    synchronized boolean expandGroup(int groupPos) {
        ExpandableListPosition elGroupPos = ExpandableListPosition.obtain(2, groupPos, -1, -1);
        PositionMetadata pm = getFlattenedPos(elGroupPos);
        elGroupPos.recycle();
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized boolean expandGroup(PositionMetadata posMetadata) {
    ...
    public synchronized boolean isGroupExpanded(int groupPosition) {
    ...
    public synchronized void setMaxExpGroupCount(int maxExpGroupCount) {
        this.mMaxExpGroupCount = maxExpGroupCount;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized ExpandableListAdapter getAdapter() {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized ArrayList<GroupMetadata> getExpandedGroupMetadataList() {
        return this.mExpGroupMetadataList;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized void setExpandedGroupMetadataList(ArrayList<GroupMetadata> expandedGroupMetadataList) {
    ...
    synchronized int findGroupPosition(long groupIdToMatch, int seedGroupPosition) {
    ...
    public static class GroupMetadata implements Parcelable, Comparable<GroupMetadata> {
        ...
        private synchronized GroupMetadata() {
        }

        static synchronized GroupMetadata obtain(int flPos, int lastChildFlPos, int gPos, long gId) {
        ...
        public synchronized int compareTo(GroupMetadata another) {
            if (another == null) {
    ...
    public static class PositionMetadata {
        ...
        private synchronized PositionMetadata() {
        }

        static synchronized PositionMetadata obtain(int flatListPos, int type, int groupPos, int childPos, GroupMetadata groupMetadata, int groupInsertIndex) {
        ...
        private static synchronized PositionMetadata getRecycledOrCreate() {
            synchronized (sPool) {
                if (sPool.size() > 0) {
