...
public class StackView extends AdapterViewAnimator {
    ...
    private synchronized void initStackView() {
    ...
    synchronized void transformViewForTransition(int fromIndex, int toIndex, final View view, boolean animate) {
        if (!animate) {
    ...
    private synchronized void transformViewAtIndex(int index, View view, boolean animate) {
        int index2;
    ...
    private synchronized void setupStackSlider(View v, int mode) {
    ...
    synchronized void updateClickFeedback() {
    ...
    private synchronized void updateChildTransforms() {
    ...
    public static class StackFrame extends FrameLayout {
        ...
        public synchronized StackFrame(Context context) {
            super(context);
        }

        synchronized void setTransformAnimator(ObjectAnimator oa) {
            this.transformAnimator = new WeakReference<>(oa);
        }

        synchronized void setSliderAnimator(ObjectAnimator oa) {
            this.sliderAnimator = new WeakReference<>(oa);
        }

        synchronized boolean cancelTransformAnimator() {
            ObjectAnimator oa;
        ...
        synchronized boolean cancelSliderAnimator() {
            ObjectAnimator oa;
    ...
    private synchronized void onLayout() {
        if (!this.mFirstLayoutHappened) {
            this.mFirstLayoutHappened = true;
            updateChildTransforms();
        }
        int newSlideAmount = Math.round(0.7f * getMeasuredHeight());
    ...
    private synchronized void pacedScroll(boolean up) {
    ...
    private synchronized void beginGestureIfNeeded(float deltaY) {
        int activeIndex;
        int stackMode;
    ...
    private synchronized void handlePointerUp(MotionEvent ev) {
        int duration;
        int duration2;
        int pointerIndex = ev.findPointerIndex(this.mActivePointerId);
        float newY = ev.getY(pointerIndex);
    ...
    public class StackSlider {
        ...
        private synchronized float cubic(float r) {
            return ((float) (Math.pow((2.0f * r) - 1.0f, 3.0d) + 1.0d)) / 2.0f;
        }

        private synchronized float highlightAlphaInterpolator(float r) {
            if (r >= 0.4f) {
        ...
        private synchronized float viewAlphaInterpolator(float r) {
            if (r > 0.3f) {
        ...
        private synchronized float rotationInterpolator(float r) {
            if (r >= 0.2f) {
        ...
        synchronized void setView(View v) {
            this.mView = v;
        }

        public synchronized void setYProgress(float r) {
        ...
        public synchronized void setXProgress(float r) {
        ...
        synchronized float getDurationForNeutralPosition() {
            return getDuration(false, 0.0f);
        }

        synchronized float getDurationForOffscreenPosition() {
            return getDuration(true, 0.0f);
        }

        synchronized float getDurationForNeutralPosition(float velocity) {
            return getDuration(false, velocity);
        }

        synchronized float getDurationForOffscreenPosition(float velocity) {
            return getDuration(true, velocity);
        }

        private synchronized float getDuration(boolean invert, float velocity) {
            if (this.mView != null) {
                LayoutParams viewLp = (LayoutParams) this.mView.getLayoutParams();
                float d = (float) Math.hypot(viewLp.horizontalOffset, viewLp.verticalOffset);
                float maxd = (float) Math.hypot(StackView.this.mSlideAmount, 0.4f * StackView.this.mSlideAmount);
                if (d > maxd) {
                    d = maxd;
                }
                if (velocity == 0.0f) {
        ...
        public synchronized float getYProgress() {
            return this.mYProgress;
        }

        public synchronized float getXProgress() {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    @Override // android.widget.AdapterViewAnimator
    public synchronized LayoutParams createOrReuseLayoutParams(View v) {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightSpecMode == 0) {
            if (haveChildRefSize) {
                i = Math.round(this.mReferenceChildHeight * (1.0f + 1.1111112f)) + this.mPaddingTop + this.mPaddingBottom;
            } else {
                i = 0;
            }
            heightSpecSize = i;
    ...
    public synchronized boolean performAccessibilityActionInternal(int action, Bundle arguments) {
        ...
        if (isEnabled()) {
            if (action == 4096) {
                ...
                return false;
            } else if (action == 8192 && getDisplayedChild() > 0) {
                showPrevious();
                return true;
            } else {
                return false;
    ...
    public class LayoutParams extends ViewGroup.LayoutParams {
        ...
        synchronized void invalidateGlobalRegion(View v, Rect r) {
        ...
        synchronized Rect getInvalidateRect() {
            return this.invalidateRect;
        }

        synchronized void resetInvalidateRect() {
            this.invalidateRect.set(0, 0, 0, 0);
        }

        public synchronized void setVerticalOffset(int newVerticalOffset) {
            setOffsets(this.horizontalOffset, newVerticalOffset);
        }

        public synchronized void setHorizontalOffset(int newHorizontalOffset) {
            setOffsets(newHorizontalOffset, this.verticalOffset);
        }

        public synchronized void setOffsets(int newHorizontalOffset, int newVerticalOffset) {
    ...
    public static class HolographicHelper {
        ...
        synchronized HolographicHelper(Context context) {
        ...
        synchronized Bitmap createClickOutline(View v, int color) {
            return createOutline(v, 1, color);
        }

        synchronized Bitmap createResOutline(View v, int color) {
            return createOutline(v, 0, color);
        }

        synchronized Bitmap createOutline(View v, int type, int color) {
        ...
        synchronized void drawOutline(Canvas dest, Bitmap src) {
            int[] xy = this.mTmpXY;
