...
public class TableRow extends LinearLayout {
    ...
    private synchronized void initTableRow() {
    ...
    public synchronized void setColumnCollapsed(int columnIndex, boolean collapsed) {
    ...
    private synchronized void mapIndexAndColumns() {
        if (this.mColumnToChildIndex == null) {
            ...
            int virtualCount = 0;
            int virtualCount2 = 0;
            while (virtualCount2 < count) {
                View child = getChildAt(virtualCount2);
                LayoutParams layoutParams = (LayoutParams) child.getLayoutParams();
                if (layoutParams.column >= virtualCount) {
                    virtualCount = layoutParams.column;
                }
                int virtualCount3 = virtualCount;
                int virtualCount4 = 0;
                while (virtualCount4 < layoutParams.span) {
                    columnToChild.put(virtualCount3, virtualCount2);
                    virtualCount4++;
                    virtualCount3++;
                }
                virtualCount2++;
                virtualCount = virtualCount3;
            }
    ...
    public synchronized void measureChildBeforeLayout(View child, int childIndex, int widthMeasureSpec, int totalWidth, int heightMeasureSpec, int totalHeight) {
        if (this.mConstrainedColumnWidths != null) {
            ...
            int columnWidth = 0;
            for (int columnWidth2 = 0; columnWidth2 < span; columnWidth2++) {
                columnWidth += constrainedColumnWidths[childIndex + columnWidth2];
            }
            int i = lp.gravity;
            ...
            if (isHorizontalGravity) {
                ...
                int measureMode2 = absoluteGravity & 7;
                if (measureMode2 == 1) {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized int[] getColumnsWidths(int widthMeasureSpec, int heightMeasureSpec) {
        int spec;
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized void setColumnsWidthConstraints(int[] columnWidths) {
    ...
    public class ChildrenTracker implements ViewGroup.OnHierarchyChangeListener {
        ...
        /* JADX INFO: Access modifiers changed from: private */
        public synchronized void setOnHierarchyChangeListener(ViewGroup.OnHierarchyChangeListener listener) {
