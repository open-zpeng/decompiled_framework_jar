...
public class DefaultDataHandler implements ContentInsertHandler {
    ...
    public synchronized void insert(ContentResolver contentResolver, InputStream in) throws IOException, SAXException {
        this.mContentResolver = contentResolver;
        Xml.parse(in, Xml.Encoding.UTF_8, this);
    }

    @Override // android.content.ContentInsertHandler
    public synchronized void insert(ContentResolver contentResolver, String in) throws SAXException {
        this.mContentResolver = contentResolver;
        Xml.parse(in, this);
    }

    private synchronized void parseRow(Attributes atts) throws SAXException {
        Uri uri;
        Uri uri2;
        String uriStr = atts.getValue("uri");
        if (uriStr != null) {
            uri2 = Uri.parse(uriStr);
            if (uri2 == null) {
        ...
        this.mUris.push(uri2);
    }

    private synchronized Uri insertRow() {
