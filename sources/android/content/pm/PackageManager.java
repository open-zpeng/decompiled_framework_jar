...
public abstract class PackageManager {
    ...
    public static final String FEATURE_TBOX = "android.hardware.tbox";
    ...
    public static final String FEATURE_USBNET = "android.hardware.usbnet";
    ...
    private protected static final int INSTALL_REPLACE_EXISTING = 2;
    @SystemApi
    ...
    @Deprecated
    private protected static final int MOVE_EXTERNAL_MEDIA = 2;
    ...
    @Deprecated
    private protected static final int MOVE_INTERNAL = 1;
    ...
    private protected static final int NO_NATIVE_LIBRARIES = -114;
    ...
    private protected abstract void addCrossProfileIntentFilter(IntentFilter intentFilter, int i, int i2, int i3);
    ...
    private protected abstract void clearApplicationUserData(String str, IPackageDataObserver iPackageDataObserver);

    private protected abstract void clearCrossProfileIntentFilters(int i);
    ...
    private protected abstract void deleteApplicationCacheFiles(String str, IPackageDataObserver iPackageDataObserver);

    private protected abstract void deleteApplicationCacheFilesAsUser(String str, int i, IPackageDataObserver iPackageDataObserver);

    /* JADX INFO: Access modifiers changed from: private */
    public abstract void deletePackage(String str, IPackageDeleteObserver iPackageDeleteObserver, int i);

    private protected abstract void deletePackageAsUser(String str, IPackageDeleteObserver iPackageDeleteObserver, int i, int i2);

    public abstract void extendVerificationTimeout(int i, int i2, long j);

    private protected abstract void flushPackageRestrictionsAsUser(int i);

    private protected abstract void freeStorage(String str, long j, IntentSender intentSender);

    private protected abstract void freeStorageAndNotify(String str, long j, IPackageDataObserver iPackageDataObserver);
    ...
    private protected abstract boolean getApplicationHiddenSettingAsUser(String str, UserHandle userHandle);
    ...
    public abstract synchronized Intent getCarLaunchIntentForPackage(String str);
    ...
    private protected abstract ComponentName getHomeActivities(List<ResolveInfo> list);
    ...
    public abstract synchronized String getInstantAppAndroidId(String str, UserHandle userHandle);

    public abstract byte[] getInstantAppCookie();

    public abstract int getInstantAppCookieMaxBytes();

    public abstract synchronized int getInstantAppCookieMaxSize();
    ...
    private protected abstract KeySet getKeySetByAlias(String str, String str2);
    ...
    private protected abstract int getMoveStatus(int i);

    public abstract String getNameForUid(int i);

    public abstract String[] getNamesForUids(int[] iArr);

    private protected abstract List<VolumeInfo> getPackageCandidateVolumes(ApplicationInfo applicationInfo);

    private protected abstract VolumeInfo getPackageCurrentVolume(ApplicationInfo applicationInfo);
    ...
    @Deprecated
    private protected abstract void getPackageSizeInfoAsUser(String str, int i, IPackageStatsObserver iPackageStatsObserver);
    ...
    private protected abstract int getPackageUidAsUser(String str, int i, int i2) throws NameNotFoundException;
    ...
    public abstract synchronized List<VolumeInfo> getPrimaryStorageCandidateVolumes();

    public abstract synchronized VolumeInfo getPrimaryStorageCurrentVolume();
    ...
    public abstract synchronized List<SharedLibraryInfo> getSharedLibrariesAsUser(int i, int i2);

    public abstract String getSharedSystemSharedLibraryPackageName();

    private protected abstract KeySet getSigningKeySet(String str);
    ...
    private protected abstract Drawable getUserBadgeForDensity(UserHandle userHandle, int i);
    ...
    public abstract synchronized VerifierDeviceIdentity getVerifierDeviceIdentity();
    ...
    private protected abstract int installExistingPackageAsUser(String str, int i) throws NameNotFoundException;
    ...
    private protected abstract boolean isPackageSuspendedForUser(String str, int i);
    ...
    private protected abstract boolean isSignedBy(String str, KeySet keySet);

    private protected abstract boolean isSignedByExactly(String str, KeySet keySet);

    private protected abstract boolean isUpgrade();
    ...
    private protected abstract int movePackage(String str, VolumeInfo volumeInfo);

    public abstract synchronized int movePrimaryStorage(VolumeInfo volumeInfo);
    ...
    private protected abstract List<ResolveInfo> queryIntentContentProvidersAsUser(Intent intent, int i, int i2);
    ...
    private protected abstract void registerMoveCallback(MoveCallback moveCallback, Handler handler);
    ...
    @Deprecated
    private protected abstract void replacePreferredActivity(IntentFilter intentFilter, int i, ComponentName[] componentNameArr, ComponentName componentName);
    ...
    private protected abstract ProviderInfo resolveContentProviderAsUser(String str, int i, int i2);

    public abstract ResolveInfo resolveService(Intent intent, int i);

    public abstract synchronized ResolveInfo resolveServiceAsUser(Intent intent, int i, int i2);
    ...
    private protected abstract boolean setApplicationHiddenSettingAsUser(String str, boolean z, UserHandle userHandle);
    ...
    private protected abstract boolean setInstantAppCookie(byte[] bArr);
    ...
    private protected abstract void unregisterMoveCallback(MoveCallback moveCallback);
    ...
    @Deprecated
    private protected List<ResolveInfo> queryBroadcastReceivers(Intent intent, int flags, int userId) {
    ...
    private protected void freeStorageAndNotify(long freeStorageSize, IPackageDataObserver observer) {
        freeStorageAndNotify(null, freeStorageSize, observer);
    }

    private protected void freeStorage(long freeStorageSize, IntentSender pi) {
        freeStorage(null, freeStorageSize, pi);
    }

    @Deprecated
    private protected void getPackageSizeInfo(String packageName, IPackageStatsObserver observer) {
        getPackageSizeInfoAsUser(packageName, getUserId(), observer);
    }

    private protected void addPreferredActivityAsUser(IntentFilter filter, int match, ComponentName[] set, ComponentName activity, int userId) {
    ...
    public static synchronized boolean isMoveStatusFinished(int status) {
        return status < 0 || status > 100;
    }

    /* loaded from: classes.dex */
    public static abstract class MoveCallback {
        public abstract synchronized void onStatusChanged(int i, int i2, long j);

        public synchronized void onCreated(int moveId, Bundle extras) {
        }
    }

    private protected static String installStatusToString(int status, String msg) {
    ...
    private protected static String installStatusToString(int status) {
        if (status != -117) {
            if (status != -115) {
                if (status == 1) {
    ...
    public static synchronized int installStatusToPublicStatus(int status) {
        if (status != -117) {
            if (status != -115) {
                if (status != 1) {
                    switch (status) {
    ...
    public static synchronized String deleteStatusToString(int status, String msg) {
    ...
    private protected static String deleteStatusToString(int status) {
        if (status == 1) {
    ...
    public static synchronized int deleteStatusToPublicStatus(int status) {
        if (status == 1) {
            return 0;
        }
        switch (status) {
    ...
    public static synchronized String permissionFlagToString(int flag) {
        if (flag != 4) {
            if (flag != 8) {
                if (flag != 16) {
                    if (flag != 32) {
    ...
    public static class LegacyPackageDeleteObserver extends PackageDeleteObserver {
        ...
        public synchronized LegacyPackageDeleteObserver(IPackageDeleteObserver legacy) {
            this.mLegacy = legacy;
        }

        @Override // android.app.PackageDeleteObserver
        public synchronized void onPackageDeleted(String basePackageName, int returnCode, String msg) {
