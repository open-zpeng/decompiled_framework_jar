...
public class ActivityInfo extends ComponentInfo implements Parcelable {
    ...
    private protected static final int FLAG_ALLOW_EMBEDDED = Integer.MIN_VALUE;
    ...
    private protected static final int FLAG_SHOW_FOR_ALL_USERS = 1024;
    ...
    private protected static int activityInfoConfigJavaToNative(int input) {
        int output = 0;
    ...
    public static synchronized int activityInfoConfigNativeToJava(int input) {
        int output = 0;
        for (int i = 0; i < CONFIG_NATIVE_BITS.length; i++) {
    ...
    public synchronized int getRealConfigChanged() {
    ...
    public static final synchronized String lockTaskLaunchModeToString(int lockTaskLaunchMode) {
    ...
    private synchronized String persistableModeToString() {
    ...
    public synchronized boolean isFixedOrientation() {
        return isFixedOrientationLandscape() || isFixedOrientationPortrait() || this.screenOrientation == 14;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized boolean isFixedOrientationLandscape() {
        return isFixedOrientationLandscape(this.screenOrientation);
    }

    public static synchronized boolean isFixedOrientationLandscape(int orientation) {
        return orientation == 0 || orientation == 6 || orientation == 8 || orientation == 11;
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized boolean isFixedOrientationPortrait() {
        return isFixedOrientationPortrait(this.screenOrientation);
    }

    public static synchronized boolean isFixedOrientationPortrait(int orientation) {
    ...
    private protected static boolean isResizeableMode(int mode) {
        return mode == 2 || mode == 4 || mode == 6 || mode == 5 || mode == 7 || mode == 1;
    }

    public static synchronized boolean isPreserveOrientationMode(int mode) {
        return mode == 6 || mode == 5 || mode == 7;
    }

    public static synchronized String resizeModeToString(int mode) {
        switch (mode) {
            case 0:
    ...
    public static synchronized String screenOrientationToString(int orientation) {
        switch (orientation) {
            case -2:
    ...
    private synchronized ActivityInfo(Parcel source) {
        super(source);
        this.resizeMode = 2;
        this.colorMode = 0;
        this.screenOrientation = -1;
    ...
    public static final class WindowLayout {
        ...
        synchronized WindowLayout(Parcel source) {
