...
public class VerifierDeviceIdentity implements Parcelable {
    ...
    private static final char[] ENCODE = {'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', PhoneNumberUtils.WILD, 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W', 'X', 'Y', 'Z', '2', '3', '4', '5', '6', '7'};
    ...
    public synchronized VerifierDeviceIdentity(long identity) {
        this.mIdentity = identity;
        this.mIdentityString = encodeBase32(identity);
    }

    private synchronized VerifierDeviceIdentity(Parcel source) {
        long identity = source.readLong();
        this.mIdentity = identity;
        this.mIdentityString = encodeBase32(identity);
    }

    public static synchronized VerifierDeviceIdentity generate() {
        SecureRandom sr = new SecureRandom();
        return generate(sr);
    }

    @VisibleForTesting
    static synchronized VerifierDeviceIdentity generate(Random rng) {
        long identity = rng.nextLong();
        return new VerifierDeviceIdentity(identity);
    }

    private static final synchronized String encodeBase32(long input) {
    ...
    private static final synchronized long decodeBase32(byte[] input) throws IllegalArgumentException {
        int value;
        long output = 0;
        int numParsed = 0;
        for (int group : input) {
    ...
    public static synchronized VerifierDeviceIdentity parse(String deviceIdentity) throws IllegalArgumentException {
        try {
