...
public abstract class SplitDependencyLoader<E extends Exception> {
    public protected final SparseArray<int[]> mDependencies;

    public private abstract synchronized void constructSplit(int i, int[] iArr, int i2) throws Exception;

    public private abstract synchronized boolean isSplitCached(int i);

    public private synchronized SplitDependencyLoader(SparseArray<int[]> dependencies) {
        this.mDependencies = dependencies;
    }

    public private synchronized void loadDependenciesForSplit(int splitIdx) throws Exception {
    ...
    public protected synchronized int[] collectConfigSplitIndices(int splitIdx) {
        int[] deps = this.mDependencies.get(splitIdx);
    ...
    public static class IllegalDependencyException extends Exception {
        public protected synchronized IllegalDependencyException(String message) {
            super(message);
        }
    }

    public protected static synchronized int[] append(int[] src, int elem) {
        if (src == null) {
            return new int[]{elem};
        }
    ...
    public static synchronized SparseArray<int[]> createDependenciesFromPackage(PackageParser.PackageLite pkg) throws IllegalDependencyException {
        int depIdx;
        int targetIdx;
