...
public class LauncherApps {
    ...
    public protected final PackageManager mPm;
    public protected final ILauncherApps mService;
    ...
    public synchronized LauncherApps(Context context, ILauncherApps service) {
    ...
    private synchronized void logErrorForInvalidProfileAccess(UserHandle target) {
    ...
    private synchronized List<LauncherActivityInfo> convertToActivityList(ParceledListSlice<ResolveInfo> activities, UserHandle user) {
        if (activities == null) {
    ...
    private synchronized List<ShortcutInfo> maybeUpdateDisabledMessage(List<ShortcutInfo> shortcuts) {
        if (shortcuts == null) {
            return null;
        }
    ...
    @Deprecated
    public synchronized List<ShortcutInfo> getShortcutInfo(String packageName, List<String> ids, UserHandle user) {
    ...
    @Deprecated
    public synchronized int getShortcutIconResId(ShortcutInfo shortcut) {
        return shortcut.getIconResourceId();
    }

    @Deprecated
    public synchronized int getShortcutIconResId(String packageName, String shortcutId, UserHandle user) {
        ShortcutQuery q = new ShortcutQuery();
        q.setPackage(packageName);
    ...
    public synchronized ParcelFileDescriptor getShortcutIconFd(ShortcutInfo shortcut) {
        return getShortcutIconFd(shortcut.getPackage(), shortcut.getId(), shortcut.getUserId());
    }

    public synchronized ParcelFileDescriptor getShortcutIconFd(String packageName, String shortcutId, UserHandle user) {
        return getShortcutIconFd(packageName, shortcutId, user.getIdentifier());
    }

    private synchronized ParcelFileDescriptor getShortcutIconFd(String packageName, String shortcutId, int userId) {
        try {
    ...
    private synchronized Drawable loadDrawableResourceFromPackage(String packageName, int resId, UserHandle user, int density) {
        if (resId == 0) {
            return null;
        }
        try {
    ...
    public protected void startShortcut(String packageName, String shortcutId, Rect sourceBounds, Bundle startActivityOptions, int userId) {
        try {
    ...
    private synchronized int findCallbackLocked(Callback callback) {
        if (callback == null) {
    ...
    private synchronized void removeCallbackLocked(Callback callback) {
    ...
    private synchronized void addCallbackLocked(Callback callback, Handler handler) {
        removeCallbackLocked(callback);
        if (handler == null) {
            handler = new Handler();
        }
    ...
    public static class CallbackMessageHandler extends Handler {
        ...
        public static class CallbackInfo {
            ...
            private synchronized CallbackInfo() {
            }
        }

        public synchronized CallbackMessageHandler(Looper looper, Callback callback) {
        ...
        public synchronized void postOnPackageAdded(String packageName, UserHandle user) {
        ...
        public synchronized void postOnPackageRemoved(String packageName, UserHandle user) {
            CallbackInfo info = new CallbackInfo();
            info.packageName = packageName;
            info.user = user;
        ...
        public synchronized void postOnPackageChanged(String packageName, UserHandle user) {
            CallbackInfo info = new CallbackInfo();
            info.packageName = packageName;
            info.user = user;
        ...
        public synchronized void postOnPackagesAvailable(String[] packageNames, UserHandle user, boolean replacing) {
            CallbackInfo info = new CallbackInfo();
        ...
        public synchronized void postOnPackagesUnavailable(String[] packageNames, UserHandle user, boolean replacing) {
            CallbackInfo info = new CallbackInfo();
            info.packageNames = packageNames;
            info.replacing = replacing;
            info.user = user;
        ...
        public synchronized void postOnPackagesSuspended(String[] packageNames, Bundle launcherExtras, UserHandle user) {
            CallbackInfo info = new CallbackInfo();
            info.packageNames = packageNames;
            info.user = user;
        ...
        public synchronized void postOnPackagesUnsuspended(String[] packageNames, UserHandle user) {
            CallbackInfo info = new CallbackInfo();
            info.packageNames = packageNames;
            info.user = user;
        ...
        public synchronized void postOnShortcutChanged(String packageName, UserHandle user, List<ShortcutInfo> shortcuts) {
            CallbackInfo info = new CallbackInfo();
            info.packageName = packageName;
            info.user = user;
    ...
    public static final class PinItemRequest implements Parcelable {
        ...
        public synchronized PinItemRequest(IPinItemRequest inner, int type) {
            this.mInner = inner;
        ...
        private synchronized PinItemRequest(Parcel source) {
