...
public final class StringBlock {
    ...
    public synchronized StringBlock(byte[] data, boolean useSparse) {
        this.mNative = nativeCreate(data, 0, data.length);
        this.mUseSparse = useSparse;
    }

    public synchronized StringBlock(byte[] data, int offset, int size, boolean useSparse) {
    ...
    public static final class StyleIDs {
        ...
        synchronized StyleIDs() {
        }
    }

    private synchronized CharSequence applyStyles(String str, int[] style, StyleIDs ids) {
        if (style.length == 0) {
            return str;
        }
                                                ...
                                                if (type != ids.listItemId) {
                                                    if (type == ids.marqueeId) {
                                                        ...
                                                        if (tag.startsWith("font;")) {
                                                            ...
                                                            if (sub5 != null) {
                                                                buffer.setSpan(getColor(sub5, z), style[i + 1], style[i + 2] + 1, 33);
                                                            }
    ...
    private static synchronized CharacterStyle getColor(String color, boolean foreground) {
        int c = -16777216;
    ...
    private static synchronized void addParagraphSpan(Spannable buffer, Object what, int start, int end) {
    ...
    private static synchronized String subtag(String full, String attribute) {
    ...
    public static class Height implements LineHeightSpan.WithDensity {
        ...
        public synchronized Height(int size) {
            this.mSize = size;
        }
    ...
    public private protected StringBlock(long obj, boolean useSparse) {
        this.mNative = obj;
        this.mUseSparse = useSparse;
    }
}
