...
public class CompatibilityInfo implements Parcelable {
    ...
    private protected final float applicationScale;
    private final int mCompatibilityFlags;
    private protected static final CompatibilityInfo DEFAULT_COMPATIBILITY_INFO = new CompatibilityInfo() { // from class: android.content.res.CompatibilityInfo.1
    };
    private protected static final Parcelable.Creator<CompatibilityInfo> CREATOR = new Parcelable.Creator<CompatibilityInfo>() { // from class: android.content.res.CompatibilityInfo.2
    ...
    private protected CompatibilityInfo(ApplicationInfo appInfo, int screenLayout, int sw, boolean forceCompat) {
        int required;
    ...
    private synchronized CompatibilityInfo(int compFlags, int dens, float scale, float invertedScale) {
    ...
    public protected CompatibilityInfo() {
    ...
    public synchronized boolean neverSupportsScreen() {
        return (this.mCompatibilityFlags & 2) != 0;
    }

    public synchronized boolean alwaysSupportsScreen() {
        return (this.mCompatibilityFlags & 4) != 0;
    }

    public synchronized boolean needsCompatResources() {
    ...
    public class Translator {
        private protected final float applicationInvertedScale;
        private protected final float applicationScale;
        ...
        public synchronized void translatePointInScreenToAppWindow(PointF point) {
        ...
        public synchronized void translateLayoutParamsInAppWindowToScreen(WindowManager.LayoutParams params) {
            params.scale(this.applicationScale);
        }
        ...
        public synchronized Rect getTranslatedVisibleInsets(Rect visibleInsets) {
        ...
        public synchronized Region getTranslatedTouchableArea(Region touchableArea) {
    ...
    public synchronized void applyToDisplayMetrics(DisplayMetrics inoutDm) {
        if (!supportsScreen()) {
    ...
    public synchronized void applyToConfiguration(int displayDensity, Configuration inoutConfig) {
        if (!supportsScreen()) {
    ...
    private protected static float computeCompatibleScaling(DisplayMetrics dm, DisplayMetrics outDm) {
        int shortSize;
        int longSize;
        int newWidth;
        int newHeight;
    ...
    private synchronized CompatibilityInfo(Parcel source) {
