...
public class TypedArray {
    ...
    public protected AssetManager mAssets;
    public private protected int[] mData;
    long mDataAddress;
    public private protected int[] mIndices;
    long mIndicesAddress;
    public private protected int mLength;
    public protected DisplayMetrics mMetrics;
    public protected boolean mRecycled;
    public protected final Resources mResources;
    public private protected Resources.Theme mTheme;
    public private protected TypedValue mValue = new TypedValue();
    public private protected XmlBlock.Parser mXml;

    /* JADX INFO: Access modifiers changed from: package-private */
    public static synchronized TypedArray obtain(Resources res, int len) {
    ...
    private synchronized void resize(int len) {
        this.mLength = len;
        int dataLen = len * 6;
    ...
    public synchronized ComplexColor getComplexColor(int index) {
        if (this.mRecycled) {
            throw new RuntimeException("Cannot make calls to a recycled instance!");
        }
        TypedValue value = this.mValue;
    ...
    public synchronized int getThemeAttributeId(int index, int defValue) {
        if (this.mRecycled) {
            throw new RuntimeException("Cannot make calls to a recycled instance!");
        }
        int index2 = index * 6;
    ...
    public synchronized Drawable getDrawableForDensity(int index, int density) {
        if (this.mRecycled) {
            throw new RuntimeException("Cannot make calls to a recycled instance!");
        }
        TypedValue value = this.mValue;
    ...
    public protected boolean getValueAt(int index, TypedValue outValue) {
        int[] data = this.mData;
        int type = data[index + 0];
        if (type == 0) {
            return false;
    ...
    private synchronized CharSequence loadStringValueAt(int index) {
        int[] data = this.mData;
    ...
    protected synchronized TypedArray(Resources resources) {
