...
public final class PrinterDiscoverySession {
    ...
    public interface OnPrintersChangeListener {
        synchronized void onPrintersChanged();
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized PrinterDiscoverySession(IPrintManager printManager, Context context, int userId) {
    ...
    public final synchronized void startPrinterDiscovery(List<PrinterId> priorityList) {
        if (isDestroyed()) {
    ...
    public final synchronized void stopPrinterDiscovery() {
        if (isDestroyed()) {
    ...
    public final synchronized void startPrinterStateTracking(PrinterId printerId) {
        if (isDestroyed()) {
    ...
    public final synchronized void stopPrinterStateTracking(PrinterId printerId) {
        if (isDestroyed()) {
    ...
    public final synchronized void validatePrinters(List<PrinterId> printerIds) {
        if (isDestroyed()) {
    ...
    public final synchronized List<PrinterInfo> getPrinters() {
        if (isDestroyed()) {
    ...
    public final synchronized boolean isPrinterDiscoveryStarted() {
        throwIfNotCalledOnMainThread();
        return this.mIsPrinterDiscoveryStarted;
    }

    public final synchronized void setOnPrintersChangeListener(OnPrintersChangeListener listener) {
        throwIfNotCalledOnMainThread();
    ...
    private synchronized boolean isDestroyedNoCheck() {
    ...
    private synchronized void destroyNoCheck() {
    ...
    public synchronized void handlePrintersAdded(List<PrinterInfo> addedPrinters) {
        if (isDestroyed()) {
            return;
        }
        int i = 0;
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public synchronized void handlePrintersRemoved(List<PrinterId> printerIds) {
        if (isDestroyed()) {
            return;
        }
    ...
    private synchronized void notifyOnPrintersChanged() {
        if (this.mListener != null) {
            this.mListener.onPrintersChanged();
        }
    }

    private static synchronized void throwIfNotCalledOnMainThread() {
    ...
    public static final class PrinterDiscoveryObserver extends IPrinterDiscoveryObserver.Stub {
        ...
        public synchronized PrinterDiscoveryObserver(PrinterDiscoverySession session) {
            this.mWeakSession = new WeakReference<>(session);
        }

        @Override // android.print.IPrinterDiscoveryObserver
        public synchronized void onPrintersAdded(ParceledListSlice printers) {
        ...
        @Override // android.print.IPrinterDiscoveryObserver
        public synchronized void onPrintersRemoved(ParceledListSlice printerIds) {
            PrinterDiscoverySession session = this.mWeakSession.get();
            if (session != null) {
