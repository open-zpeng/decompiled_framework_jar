...
public class RenderScriptGL extends RenderScript {
    ...
    public static class SurfaceConfig {
        ...
        private protected SurfaceConfig() {
            this.mDepthMin = 0;
            this.mDepthPref = 0;
            this.mStencilMin = 0;
            this.mStencilPref = 0;
        ...
        public synchronized SurfaceConfig(SurfaceConfig sc) {
            this.mDepthMin = 0;
            this.mDepthPref = 0;
            this.mStencilMin = 0;
            this.mStencilPref = 0;
        ...
        private synchronized void validateRange(int umin, int upref, int rmin, int rmax) {
    ...
    public synchronized void setSurfaceTexture(SurfaceTexture sur, int w, int h) {
        validate();
        Surface s = null;
        if (sur != null) {
            s = new Surface(sur);
    ...
    private protected void bindRootScript(Script s) {
        validate();
        nContextBindRootScript((int) safeID(s));
    }

    private protected void bindProgramStore(ProgramStore p) {
        validate();
        nContextBindProgramStore((int) safeID(p));
    }

    public synchronized void bindProgramFragment(ProgramFragment p) {
        validate();
        nContextBindProgramFragment((int) safeID(p));
    }

    private protected void bindProgramRaster(ProgramRaster p) {
        validate();
        nContextBindProgramRaster((int) safeID(p));
    }

    private protected void bindProgramVertex(ProgramVertex p) {
        validate();
