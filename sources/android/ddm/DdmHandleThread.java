...
public class DdmHandleThread extends ChunkHandler {
    ...
    private synchronized DdmHandleThread() {
    }
    ...
    private synchronized Chunk handleTHEN(Chunk request) {
    ...
    private synchronized Chunk handleTHST(Chunk request) {
        wrapChunk(request);
    ...
    private synchronized Chunk handleSTKL(Chunk request) {
        ByteBuffer in = wrapChunk(request);
    ...
    private synchronized Chunk createStackChunk(StackTraceElement[] trace, int threadId) {
        int bufferSize = 0 + 4;
