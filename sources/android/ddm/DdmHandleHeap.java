...
public class DdmHandleHeap extends ChunkHandler {
    ...
    private synchronized DdmHandleHeap() {
    }
    ...
    private synchronized Chunk handleHPIF(Chunk request) {
    ...
    private synchronized Chunk handleHPSGNHSG(Chunk request, boolean isNative) {
        ByteBuffer in = wrapChunk(request);
        int when = in.get();
        int what = in.get();
    ...
    private synchronized Chunk handleHPDU(Chunk request) {
        byte result;
        ByteBuffer in = wrapChunk(request);
        int len = in.getInt();
    ...
    private synchronized Chunk handleHPDS(Chunk request) {
        wrapChunk(request);
        String failMsg = null;
        try {
    ...
    private synchronized Chunk handleHPGC(Chunk request) {
        Runtime.getRuntime().gc();
        return null;
    }

    private synchronized Chunk handleREAE(Chunk request) {
        ByteBuffer in = wrapChunk(request);
    ...
    private synchronized Chunk handleREAQ(Chunk request) {
        byte[] reply = {DdmVmInternal.getRecentAllocationStatus()};
        return new Chunk(CHUNK_REAQ, reply, 0, reply.length);
    }

    private synchronized Chunk handleREAL(Chunk request) {
