...
public class DdmHandleProfiling extends ChunkHandler {
    ...
    private synchronized DdmHandleProfiling() {
    }
    ...
    private synchronized Chunk handleMPRS(Chunk request) {
    ...
    private synchronized Chunk handleMPRE(Chunk request) {
        byte result;
        try {
    ...
    private synchronized Chunk handleMPSS(Chunk request) {
        ByteBuffer in = wrapChunk(request);
        int bufferSize = in.getInt();
        int flags = in.getInt();
        try {
    ...
    private synchronized Chunk handleMPSEOrSPSE(Chunk request, String type) {
        try {
            Debug.stopMethodTracing();
            return null;
        } catch (RuntimeException re) {
