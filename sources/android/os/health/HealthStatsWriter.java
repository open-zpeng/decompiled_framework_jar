...
public class HealthStatsWriter {
    ...
    private protected HealthStatsWriter(HealthKeys.Constants constants) {
    ...
    private protected void addTimer(int timerId, int count, long time) {
    ...
    private protected void addMeasurement(int measurementId, long value) {
        int index = this.mConstants.getIndex(1, measurementId);
        this.mMeasurementFields[index] = true;
        this.mMeasurementValues[index] = value;
    }

    private protected void addStats(int key, String name, HealthStatsWriter value) {
    ...
    private protected void addTimers(int key, String name, TimerStat value) {
    ...
    private protected void addMeasurements(int key, String name, long value) {
    ...
    private static synchronized int countBooleanArray(boolean[] fields) {
        int count = 0;
    ...
    private static synchronized <T> int countObjectArray(T[] fields) {
        int count = 0;
        for (T t : fields) {
            if (t != null) {
                count++;
    ...
    private static synchronized void writeHealthStatsWriterMap(Parcel out, ArrayMap<String, HealthStatsWriter> map) {
        int N = map.size();
        out.writeInt(N);
        for (int i = 0; i < N; i++) {
    ...
    private static synchronized <T extends Parcelable> void writeParcelableMap(Parcel out, ArrayMap<String, T> map) {
        int N = map.size();
        out.writeInt(N);
        for (int i = 0; i < N; i++) {
            out.writeString(map.keyAt(i));
    ...
    private static synchronized void writeLongsMap(Parcel out, ArrayMap<String, Long> map) {
        int N = map.size();
        out.writeInt(N);
        for (int i = 0; i < N; i++) {
            out.writeString(map.keyAt(i));
