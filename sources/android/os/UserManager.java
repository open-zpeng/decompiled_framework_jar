...
public class UserManager {
    ...
    private protected static final String DISALLOW_RECORD_AUDIO = "no_record_audio";
    ...
    public protected final IUserManager mService;
    ...
    public static class UserOperationException extends RuntimeException {
        ...
        public synchronized UserOperationException(String message, int userOperationResult) {
            super(message);
    ...
    public synchronized UserManager(Context context, IUserManager service) {
    ...
    public static synchronized boolean isGuestUserEphemeral() {
        return Resources.getSystem().getBoolean(R.bool.config_guestUserEphemeral);
    }

    public synchronized boolean canSwitchUsers() {
    ...
    public synchronized boolean isUserNameSet() {
        try {
    ...
    public synchronized boolean isPrimaryUser() {
    ...
    private protected boolean isUserAdmin(int userId) {
        UserInfo user = getUserInfo(userId);
        return user != null && user.isAdmin();
    }

    @Deprecated
    private protected boolean isLinkedUser() {
    ...
    public synchronized boolean canHaveRestrictedProfile(int userId) {
        try {
    ...
    private protected boolean isGuestUser(int id) {
    ...
    public synchronized int getManagedProfileBadge(int userId) {
        try {
    ...
    public synchronized boolean isEphemeralUser() {
        return isUserEphemeral(UserHandle.myUserId());
    }

    public synchronized boolean isUserEphemeral(int userId) {
        UserInfo user = getUserInfo(userId);
    ...
    public synchronized boolean isUserRunning(int userId) {
        try {
    ...
    private protected boolean isUserUnlocked(int userId) {
        try {
    ...
    public synchronized boolean isUserUnlockingOrUnlocked(UserHandle user) {
        return isUserUnlockingOrUnlocked(user.getIdentifier());
    }

    public synchronized boolean isUserUnlockingOrUnlocked(int userId) {
        try {
    ...
    private protected long getUserStartRealtime() {
        try {
    ...
    private protected long getUserUnlockRealtime() {
        try {
    ...
    private protected boolean hasBaseUserRestriction(String restrictionKey, UserHandle userHandle) {
        try {
    ...
    @Deprecated
    public synchronized void setUserRestriction(String key, boolean value, UserHandle userHandle) {
        try {
    ...
    public synchronized boolean hasUserRestrictionOnAnyUser(String restrictionKey) {
        try {
    ...
    public synchronized UserInfo createGuest(Context context, String name) {
        try {
    ...
    private protected UserInfo createProfileForUser(String name, int flags, int userHandle) {
        return createProfileForUser(name, flags, userHandle, null);
    }

    public synchronized UserInfo createProfileForUser(String name, int flags, int userHandle, String[] disallowedPackages) {
        try {
    ...
    public synchronized UserInfo createProfileForUserEvenWhenDisallowed(String name, int flags, int userHandle, String[] disallowedPackages) {
        try {
    ...
    public synchronized UserInfo createRestrictedProfile(String name) {
        try {
    ...
    public synchronized void setSeedAccountData(int userId, String accountName, String accountType, PersistableBundle accountOptions) {
        try {
    ...
    public synchronized boolean markGuestForDeletion(int userHandle) {
        try {
    ...
    public synchronized void setUserEnabled(int userId) {
        try {
    ...
    public synchronized void setUserAdmin(int userHandle) {
        try {
    ...
    public synchronized void evictCredentialEncryptionKey(int userHandle) {
        try {
    ...
    private protected List<UserInfo> getUsers() {
        try {
    ...
    public synchronized String getUserAccount(int userHandle) {
        try {
    ...
    public synchronized void setUserAccount(int userHandle, String accountName) {
        try {
    ...
    public synchronized UserInfo getPrimaryUser() {
        try {
    ...
    public synchronized boolean canAddMoreUsers() {
        List<UserInfo> users = getUsers(true);
        int totalUserCount = users.size();
        int aliveUserCount = 0;
        for (int aliveUserCount2 = 0; aliveUserCount2 < totalUserCount; aliveUserCount2++) {
            UserInfo user = users.get(aliveUserCount2);
            if (!user.isGuest()) {
                aliveUserCount++;
            }
        }
    ...
    public synchronized boolean canAddMoreManagedProfiles(int userId, boolean allowedToRemoveOne) {
        try {
    ...
    public synchronized boolean isSameProfileGroup(int userId, int otherUserId) {
        try {
    ...
    private protected List<UserInfo> getEnabledProfiles(int userHandle) {
        try {
    ...
    public synchronized int[] getProfileIds(int userId, boolean enabledOnly) {
        try {
    ...
    private protected int[] getProfileIdsWithDisabled(int userId) {
        return getProfileIds(userId, false);
    }

    public synchronized int[] getEnabledProfileIds(int userId) {
        return getProfileIds(userId, true);
    }

    public synchronized int getCredentialOwnerProfile(int userHandle) {
        try {
    ...
    public synchronized boolean requestQuietModeEnabled(boolean enableQuietMode, UserHandle userHandle, IntentSender target) {
        try {
    ...
    private protected Drawable getBadgedIconForUser(Drawable icon, UserHandle user) {
        return this.mContext.getPackageManager().getUserBadgedIcon(icon, user);
    }

    private protected Drawable getBadgedDrawableForUser(Drawable badgedDrawable, UserHandle user, Rect badgeLocation, int badgeDensity) {
        return this.mContext.getPackageManager().getUserBadgedDrawableForDensity(badgedDrawable, user, badgeLocation, badgeDensity);
    }

    private protected CharSequence getBadgedLabelForUser(CharSequence label, UserHandle user) {
    ...
    private protected boolean removeUser(int userHandle) {
        try {
    ...
    public synchronized boolean removeUserEvenWhenDisallowed(int userHandle) {
        try {
    ...
    public synchronized void setUserName(int userHandle, String name) {
        try {
    ...
    public synchronized void setUserIcon(int userHandle, Bitmap icon) {
        try {
    ...
    private protected static int getMaxSupportedUsers() {
    ...
    public synchronized boolean isUserSwitcherEnabled() {
        List<UserInfo> users;
    ...
    private protected int getUserSerialNumber(int userHandle) {
        try {
    ...
    private protected int getUserHandle(int userSerialNumber) {
        try {
    ...
    public synchronized Bundle getApplicationRestrictions(String packageName, UserHandle user) {
        try {
    ...
    public synchronized void setApplicationRestrictions(String packageName, Bundle restrictions, UserHandle user) {
        try {
    ...
    public synchronized void setDefaultGuestRestrictions(Bundle restrictions) {
        try {
    ...
    public synchronized Bundle getDefaultGuestRestrictions() {
        try {
    ...
    public synchronized boolean someUserHasSeedAccount(String accountName, String accountType) {
        try {
    ...
    public static final class EnforcingUser implements Parcelable {
        ...
        public synchronized EnforcingUser(int userId, int userRestrictionSource) {
            this.userId = userId;
            this.userRestrictionSource = userRestrictionSource;
        }

        private synchronized EnforcingUser(Parcel in) {
