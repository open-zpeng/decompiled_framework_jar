...
public class NetworkMetrics {
    ...
    public synchronized NetworkMetrics(int netId, long transports, TokenBucket tb) {
        this.netId = netId;
    ...
    public synchronized Summary getPendingStats() {
    ...
    public synchronized void addDnsResult(int eventType, int returnCode, int latencyMs) {
        ...
        this.pendingSummary.dnsErrorRate.count(isSuccess ? FeatureOption.FO_BOOT_POLICY_CPU : 1.0d);
    }

    public synchronized void addConnectResult(int error, int latencyMs, String ipAddr) {
        if (this.pendingSummary == null) {
            this.pendingSummary = new Summary(this.netId, this.transports);
        }
        boolean isSuccess = this.connectMetrics.addEvent(error, latencyMs, ipAddr);
        this.pendingSummary.connectErrorRate.count(isSuccess ? FeatureOption.FO_BOOT_POLICY_CPU : 1.0d);
        if (ConnectStats.isNonBlocking(error)) {
            this.pendingSummary.connectLatencies.count(latencyMs);
        }
    }

    public synchronized void addTcpStatsResult(int sent, int lost, int rttUs, int sentAckDiffMs) {
        if (this.pendingSummary == null) {
            this.pendingSummary = new Summary(this.netId, this.transports);
        }
    ...
    public static class Summary {
        ...
        public synchronized Summary(int netId, long transports) {
            this.netId = netId;
            this.transports = transports;
        }

        synchronized void merge(Summary that) {
        ...
        public String toString() {
            ...
            j.add(String.format("tcp avg_loss=%.1f%% total_sent=%d total_lost=%d", Double.valueOf(100.0d * this.tcpLossRate.average()), Integer.valueOf(this.tcpLossRate.count), Integer.valueOf((int) this.tcpLossRate.sum)));
    ...
    public static class Metrics {
        ...
        synchronized void merge(Metrics that) {
            this.count += that.count;
            this.sum += that.sum;
            this.max = Math.max(this.max, that.max);
        }

        synchronized void count(double value) {
            count(value, 1);
        }

        synchronized void count(double value, int subcount) {
            this.count += subcount;
            this.sum += value;
