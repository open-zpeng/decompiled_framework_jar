...
public final class IpSecAlgorithm implements Parcelable {
    ...
    private static synchronized void checkValidOrThrow(String name, int keyLen, int truncLen) {
        char c;
        boolean isValidLen;
    ...
    /* JADX WARN: Can't fix incorrect switch cases order, some code will duplicate */
    public synchronized boolean isAuthentication() {
        char c;
        String name = getName();
        switch (name.hashCode()) {
            case 559425185:
    ...
    public synchronized boolean isEncryption() {
        return getName().equals(CRYPT_AES_CBC);
    }

    public synchronized boolean isAead() {
        return getName().equals(AUTH_CRYPT_AES_GCM);
    }

    private static synchronized boolean isUnsafeBuild() {
    ...
    @VisibleForTesting
    public static synchronized boolean equals(IpSecAlgorithm lhs, IpSecAlgorithm rhs) {
