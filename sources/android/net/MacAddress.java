...
public final class MacAddress implements Parcelable {
    ...
    private protected static final MacAddress ALL_ZEROS_ADDRESS = new MacAddress(0);
    ...
    private synchronized MacAddress(long addr) {
    ...
    public synchronized boolean isMulticastAddress() {
    ...
    public static synchronized boolean isMacAddress(byte[] addr) {
        return addr != null && addr.length == 6;
    }

    public static synchronized int macAddressType(byte[] addr) {
    ...
    public static synchronized byte[] byteAddrFromStringAddr(String addr) {
    ...
    public static synchronized String stringAddrFromByteAddr(byte[] addr) {
        if (!isMacAddress(addr)) {
            return null;
        }
    ...
    private static synchronized byte[] byteAddrFromLongAddr(long addr) {
        int index = 6;
        byte[] bytes = new byte[6];
        while (true) {
            int index2 = index - 1;
    ...
    private static synchronized long longAddrFromByteAddr(byte[] addr) {
        Preconditions.checkNotNull(addr);
        if (!isMacAddress(addr)) {
    ...
    private static synchronized long longAddrFromStringAddr(String addr) {
        Preconditions.checkNotNull(addr);
        String[] parts = addr.split(SettingsStringUtil.DELIMITER);
        if (parts.length != 6) {
            throw new IllegalArgumentException(addr + " was not a valid MAC address");
    ...
    private static synchronized String stringAddrFromLongAddr(long addr) {
    ...
    public static synchronized MacAddress createRandomUnicastAddressWithGoogleBase() {
        return createRandomUnicastAddress(BASE_GOOGLE_MAC, new SecureRandom());
    }

    public static synchronized MacAddress createRandomUnicastAddress() {
        SecureRandom r = new SecureRandom();
        long addr = r.nextLong() & VALID_LONG_MASK;
        return new MacAddress((addr | LOCALLY_ASSIGNED_MASK) & (~MULTICAST_MASK));
    }

    public static synchronized MacAddress createRandomUnicastAddress(MacAddress base, Random r) {
