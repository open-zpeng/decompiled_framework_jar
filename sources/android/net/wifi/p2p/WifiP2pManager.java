...
public class WifiP2pManager {
    ...
    private protected static final int CREATE_GROUP = 139277;
    ...
    public interface HandoverMessageListener {
        synchronized void onHandoverMessageAvailable(String str);
    }
    ...
    public interface PersistentGroupInfoListener {
        synchronized void onPersistentGroupInfoAvailable(WifiP2pGroupList wifiP2pGroupList);
    }
    ...
    public static class Channel implements AutoCloseable {
        ...
        public protected AsyncChannel mAsyncChannel = new AsyncChannel();

        public synchronized Channel(Context context, Looper looper, ChannelListener l, Binder binder, WifiP2pManager p2pManager) {
        ...
        /* JADX INFO: Access modifiers changed from: private */
        public synchronized void handleServiceResponse(WifiP2pServiceResponse resp) {
        ...
        private synchronized void handleUpnpServiceResponse(WifiP2pUpnpServiceResponse resp) {
            this.mUpnpServRspListener.onUpnpServiceAvailable(resp.getUniqueServiceNames(), resp.getSrcDevice());
        }

        private synchronized void handleDnsSdServiceResponse(WifiP2pDnsSdServiceResponse resp) {
    ...
    private static synchronized void checkChannel(Channel c) {
        if (c == null) {
            throw new IllegalArgumentException("Channel needs to be initialized");
        }
    }

    private static synchronized void checkServiceInfo(WifiP2pServiceInfo info) {
        if (info == null) {
            throw new IllegalArgumentException("service info is null");
        }
    }

    private static synchronized void checkServiceRequest(WifiP2pServiceRequest req) {
        if (req == null) {
            throw new IllegalArgumentException("service request is null");
        }
    }

    private static synchronized void checkP2pConfig(WifiP2pConfig c) {
        if (c == null) {
    ...
    public synchronized Channel initializeInternal(Context srcContext, Looper srcLooper, ChannelListener listener) {
        return initalizeChannel(srcContext, srcLooper, listener, getP2pStateMachineMessenger(), null);
    }

    private synchronized Channel initalizeChannel(Context srcContext, Looper srcLooper, ChannelListener listener, Messenger messenger, Binder binder) {
        if (messenger == null) {
            return null;
        }
    ...
    public synchronized void listen(Channel c, boolean enable, ActionListener listener) {
        checkChannel(c);
        c.mAsyncChannel.sendMessage(enable ? START_LISTEN : STOP_LISTEN, 0, c.putListener(listener));
    }

    private protected void setWifiP2pChannels(Channel c, int lc, int oc, ActionListener listener) {
        checkChannel(c);
    ...
    private protected void startWps(Channel c, WpsInfo wps, ActionListener listener) {
        checkChannel(c);
    ...
    private protected void setDeviceName(Channel c, String devName, ActionListener listener) {
        checkChannel(c);
    ...
    private protected void setWFDInfo(Channel c, WifiP2pWfdInfo wfdInfo, ActionListener listener) {
        checkChannel(c);
        try {
    ...
    private protected void deletePersistentGroup(Channel c, int netId, ActionListener listener) {
        checkChannel(c);
        c.mAsyncChannel.sendMessage(DELETE_PERSISTENT_GROUP, netId, c.putListener(listener));
    }

    private protected void requestPersistentGroupInfo(Channel c, PersistentGroupInfoListener listener) {
        checkChannel(c);
        c.mAsyncChannel.sendMessage(REQUEST_PERSISTENT_GROUP_INFO, 0, c.putListener(listener));
    }

    private protected void setMiracastMode(int mode) {
        try {
    ...
    public synchronized Messenger getMessenger(Binder binder) {
        try {
    ...
    public synchronized Messenger getP2pStateMachineMessenger() {
        try {
    ...
    public synchronized void getNfcHandoverRequest(Channel c, HandoverMessageListener listener) {
        checkChannel(c);
        c.mAsyncChannel.sendMessage(GET_HANDOVER_REQUEST, 0, c.putListener(listener));
    }

    public synchronized void getNfcHandoverSelect(Channel c, HandoverMessageListener listener) {
        checkChannel(c);
        c.mAsyncChannel.sendMessage(GET_HANDOVER_SELECT, 0, c.putListener(listener));
    }

    public synchronized void initiatorReportNfcHandover(Channel c, String handoverSelect, ActionListener listener) {
        checkChannel(c);
    ...
    public synchronized void responderReportNfcHandover(Channel c, String handoverRequest, ActionListener listener) {
        checkChannel(c);
        Bundle bundle = new Bundle();
