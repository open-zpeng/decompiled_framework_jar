...
public class WifiP2pGroupList implements Parcelable {
    ...
    public protected final LruCache<Integer, WifiP2pGroup> mGroups;
    private final GroupDeleteListener mListener;

    /* loaded from: classes2.dex */
    public interface GroupDeleteListener {
        synchronized void onDeleteGroup(int i);
    }

    public synchronized WifiP2pGroupList() {
        this(null, null);
    }

    private protected WifiP2pGroupList(WifiP2pGroupList source, GroupDeleteListener listener) {
    ...
    private protected Collection<WifiP2pGroup> getGroupList() {
        return this.mGroups.snapshot().values();
    }

    public synchronized void add(WifiP2pGroup group) {
        this.mGroups.put(Integer.valueOf(group.getNetworkId()), group);
    }

    public synchronized void remove(int netId) {
        this.mGroups.remove(Integer.valueOf(netId));
    }

    synchronized void remove(String deviceAddress) {
    ...
    public synchronized int getNetworkId(String deviceAddress) {
    ...
    public synchronized int getNetworkId(String deviceAddress, String ssid) {
    ...
    public synchronized String getOwnerAddr(int netId) {
    ...
    public synchronized boolean contains(int netId) {
        Collection<WifiP2pGroup> groups = this.mGroups.snapshot().values();
        for (WifiP2pGroup grp : groups) {
