...
public class WifiP2pDeviceList implements Parcelable {
    ...
    public synchronized WifiP2pDeviceList(ArrayList<WifiP2pDevice> devices) {
    ...
    private synchronized void validateDevice(WifiP2pDevice device) {
        if (device == null) {
    ...
    private synchronized void validateDeviceAddress(String deviceAddress) {
    ...
    private protected void update(WifiP2pDevice device) {
        updateSupplicantDetails(device);
        this.mDevices.get(device.deviceAddress).status = device.status;
    }

    public synchronized void updateSupplicantDetails(WifiP2pDevice device) {
    ...
    public synchronized void updateGroupCapability(String deviceAddress, int groupCapab) {
    ...
    public synchronized void updateStatus(String deviceAddress, int status) {
        validateDeviceAddress(deviceAddress);
        WifiP2pDevice d = this.mDevices.get(deviceAddress);
        if (d != null) {
            d.status = status;
    ...
    public synchronized boolean remove(WifiP2pDevice device) {
        validateDevice(device);
        return this.mDevices.remove(device.deviceAddress) != null;
    }

    private protected WifiP2pDevice remove(String deviceAddress) {
        validateDeviceAddress(deviceAddress);
        return this.mDevices.remove(deviceAddress);
    }

    public synchronized boolean remove(WifiP2pDeviceList list) {
        boolean ret = false;
    ...
    public synchronized boolean isGroupOwner(String deviceAddress) {
        validateDeviceAddress(deviceAddress);
