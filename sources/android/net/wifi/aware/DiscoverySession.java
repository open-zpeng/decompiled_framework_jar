...
public class DiscoverySession implements AutoCloseable {
    ...
    public static synchronized int getMaxSendRetryCount() {
        return 5;
    }

    public synchronized DiscoverySession(WifiAwareManager manager, int clientId, int sessionId) {
    ...
    public synchronized void sendMessage(PeerHandle peerHandle, int messageId, byte[] message, int retryCount) {
        if (this.mTerminated) {
