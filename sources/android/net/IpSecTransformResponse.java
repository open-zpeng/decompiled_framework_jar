...
public final class IpSecTransformResponse implements Parcelable {
    ...
    public synchronized IpSecTransformResponse(int inStatus) {
        if (inStatus == 0) {
    ...
    public synchronized IpSecTransformResponse(int inStatus, int inResourceId) {
        this.status = inStatus;
        this.resourceId = inResourceId;
    }

    private synchronized IpSecTransformResponse(Parcel in) {
