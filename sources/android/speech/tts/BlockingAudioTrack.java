    ...
    private void blockUntilCompletion(AudioTrack audioTrack) {
        ...
        while (true) {
            ...
            if (currentPosition < lengthInFrames && audioTrack.getPlayState() == 3 && !this.mStopped) {
                ...
                long sleepTimeMs = clip(estimatedTimeMs, (long) MIN_SLEEP_TIME_MS, (long) xpWindowManager.TOAST_DURATION_SHORT);
                if (currentPosition == previousPosition) {
                    blockedTimeMs += sleepTimeMs;
                    if (blockedTimeMs > xpWindowManager.TOAST_DURATION_SHORT) {
