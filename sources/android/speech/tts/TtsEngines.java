...
public class TtsEngines {
    ...
    public synchronized String getDefaultEngine() {
    ...
    public void setDefaultEngine(String engine) {
        String current = getDefaultEngine();
        if (engine != null && !engine.equals(current)) {
            Settings.Secure.putString(this.mContext.getContentResolver(), Settings.Secure.TTS_DEFAULT_SYNTH, engine);
            Log.i(TAG, "setDefaultEngine " + engine);
            this.mContext.sendBroadcast(new Intent(TextToSpeech.ACTION_TTS_ENGINE_CHANGED));
        }
    }

    public synchronized String getHighestRankedEngineName() {
    ...
    public synchronized TextToSpeech.EngineInfo getEngineInfo(String packageName) {
    ...
    private synchronized boolean isSystemEngine(ServiceInfo info) {
        ApplicationInfo appInfo = info.applicationInfo;
        return (appInfo == null || (appInfo.flags & 1) == 0) ? false : true;
    }

    public synchronized boolean isEngineInstalled(String engine) {
        return (engine == null || getEngineInfo(engine) == null) ? false : true;
    }

    private protected Intent getSettingsIntent(String engine) {
        ServiceInfo service;
        String settings;
        PackageManager pm = this.mContext.getPackageManager();
        Intent intent = new Intent(TextToSpeech.Engine.INTENT_ACTION_TTS_SERVICE);
    ...
    private synchronized String settingsActivityFromServiceInfo(ServiceInfo si, PackageManager pm) {
        int type;
    ...
    private synchronized TextToSpeech.EngineInfo getEngineInfo(ResolveInfo resolve, PackageManager pm) {
    ...
    public static class EngineInfoComparator implements Comparator<TextToSpeech.EngineInfo> {
        ...
        private synchronized EngineInfoComparator() {
        }

        @Override // java.util.Comparator
        public synchronized int compare(TextToSpeech.EngineInfo lhs, TextToSpeech.EngineInfo rhs) {
    ...
    public synchronized Locale getLocalePrefForEngine(String engineName, String prefValue) {
    ...
    public synchronized boolean isLocaleSetToDefaultForEngine(String engineName) {
    ...
    private protected Locale parseLocaleString(String localeString) {
        String country;
        String language = "";
        country = "";
        String variant = "";
    ...
    public static synchronized String[] toOldLocaleStringFormat(Locale locale) {
        String[] ret = {"", "", ""};
        try {
    ...
    private static synchronized String parseEnginePrefFromList(String prefValue, String engineName) {
    ...
    private protected synchronized void updateLocalePrefForEngine(String engineName, Locale newLocale) {
    ...
    private synchronized String updateValueInCommaSeparatedList(String list, String key, String newValue) {
