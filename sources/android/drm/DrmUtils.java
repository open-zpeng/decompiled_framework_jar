...
public class DrmUtils {
    ...
    public static synchronized byte[] readBytes(String path) throws IOException {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public static synchronized void writeToFile(String path, byte[] data) throws IOException {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public static synchronized void removeFile(String path) throws IOException {
        File file = new File(path);
        file.delete();
    }

    private static synchronized void quietlyDispose(Closeable closable) {
        if (closable != null) {
            try {
                closable.close();
            } catch (IOException e) {
    ...
    public static class ExtendedMetadataParser {
        ...
        private synchronized int readByte(byte[] constraintData, int arrayIndex) {
            return constraintData[arrayIndex];
        }

        private synchronized String readMultipleBytes(byte[] constraintData, int numberOfBytes, int arrayIndex) {
        ...
        private synchronized ExtendedMetadataParser(byte[] constraintData) {
