...
public class MffEnvironment {
    ...
    public synchronized MffEnvironment(FrameManager frameManager) {
    ...
    public synchronized void setGLEnvironment(GLEnvironment glEnvironment) {
        this.mContext.initGLEnvironment(glEnvironment);
    }

    public synchronized void createGLEnvironment() {
        GLEnvironment glEnvironment = new GLEnvironment();
        glEnvironment.initWithNewContext();
        setGLEnvironment(glEnvironment);
    }

    public synchronized void activateGLEnvironment() {
    ...
    public synchronized void deactivateGLEnvironment() {
        GLEnvironment glEnv = this.mContext.getGLEnvironment();
        if (glEnv != null) {
