...
public class Rectangle extends Quad {
    public synchronized Rectangle() {
    }

    public synchronized Rectangle(float x, float y, float width, float height) {
        super(new Point(x, y), new Point(x + width, y), new Point(x, y + height), new Point(x + width, y + height));
    }

    public synchronized Rectangle(Point origin, Point size) {
        super(origin, origin.plus(size.x, 0.0f), origin.plus(0.0f, size.y), origin.plus(size.x, size.y));
    }

    public static synchronized Rectangle fromRotatedRect(Point center, Point size, float rotation) {
    ...
    private synchronized Rectangle(Point p0, Point p1, Point p2, Point p3) {
        super(p0, p1, p2, p3);
    }

    public static synchronized Rectangle fromCenterVerticalAxis(Point center, Point vAxis, Point size) {
