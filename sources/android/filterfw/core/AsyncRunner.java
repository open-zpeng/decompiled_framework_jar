...
public class AsyncRunner extends GraphRunner {
    ...
    private class AsyncRunnerTask extends AsyncTask<SyncRunner, Void, RunnerResult> {
        ...
        /* JADX INFO: Access modifiers changed from: protected */
        @Override // android.os.AsyncTask
        public synchronized void onCancelled(RunnerResult result) {
            onPostExecute(result);
        }

        /* JADX INFO: Access modifiers changed from: protected */
        @Override // android.os.AsyncTask
        public synchronized void onPostExecute(RunnerResult result) {
            if (AsyncRunner.this.mLogVerbose) {
    ...
    public synchronized AsyncRunner(FilterContext context, Class schedulerClass) {
        super(context);
        this.mSchedulerClass = schedulerClass;
        this.mLogVerbose = Log.isLoggable(TAG, 2);
    }

    public synchronized AsyncRunner(FilterContext context) {
        super(context);
