...
public class ShaderProgram extends Program {
    ...
    private synchronized ShaderProgram() {
    }

    private synchronized ShaderProgram(NativeAllocatorTag tag) {
    }
    ...
    public synchronized ShaderProgram(FilterContext context, String vertexShader, String fragmentShader) {
        this.mGLEnvironment = getGLEnvironment(context);
    ...
    public synchronized Object getHostValue(String variableName) {
        return getUniformValue(variableName);
    }

    public synchronized void setAttributeValues(String attributeName, float[] data, int componentCount) {
    ...
    public synchronized void setAttributeValues(String attributeName, VertexFrame vertexData, int type, int componentCount, int strideInBytes, int offsetInBytes, boolean normalize) {
    ...
    public synchronized void setTargetRegion(Quad region) {
    ...
    public synchronized void setVertexCount(int count) {
    ...
    public synchronized void beginDrawing() {
        if (!beginShaderDrawing()) {
            throw new RuntimeException("Could not prepare shader-program for drawing!");
        }
    }

    private static synchronized GLEnvironment getGLEnvironment(FilterContext context) {
