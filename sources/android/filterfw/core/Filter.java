...
public abstract class Filter {
    ...
    public abstract synchronized void process(FilterContext filterContext);

    public abstract synchronized void setupPorts();
    ...
    private protected static final boolean isAvailable(String filterName) {
    ...
    public final synchronized void initWithValueMap(KeyValueMap valueMap) {
        initFinalPorts(valueMap);
        initRemainingPorts(valueMap);
        this.mStatus = 1;
    }

    public final synchronized void initWithAssignmentString(String assignments) {
        try {
    ...
    public final synchronized void init() throws ProtocolException {
        KeyValueMap valueMap = new KeyValueMap();
        initWithValueMap(valueMap);
    }

    public synchronized String getFilterClassName() {
    ...
    protected synchronized void prepare(FilterContext context) {
    }

    protected synchronized void parametersUpdated(Set<String> updated) {
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public synchronized void delayNextProcess(int millisecs) {
        this.mSleepDelay = millisecs;
        this.mStatus = 4;
    }

    public synchronized FrameFormat getOutputFormat(String portName, FrameFormat inputFormat) {
        return null;
    }

    public final synchronized FrameFormat getInputFormat(String portName) {
    ...
    public final synchronized int getSleepDelay() {
        return 250;
    }
    ...
    public synchronized void tearDown(FilterContext context) {
    }

    public final synchronized int getNumberOfConnectedInputs() {
        int c = 0;
    ...
    public final synchronized int getNumberOfConnectedOutputs() {
        int c = 0;
    ...
    public final synchronized int getNumberOfInputs() {
    ...
    public final synchronized int getNumberOfOutputs() {
    ...
    public final synchronized InputPort getInputPort(String portName) {
        if (this.mInputPorts == null) {
    ...
    public final synchronized OutputPort getOutputPort(String portName) {
        if (this.mInputPorts == null) {
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public final synchronized void pushOutput(String name, Frame frame) {
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public final synchronized Frame pullInput(String name) {
    ...
    public synchronized void fieldPortValueUpdated(String name, FilterContext context) {
    }

    protected synchronized void transferInputPortFrame(String name, FilterContext context) {
        getInputPort(name).transfer(context);
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public synchronized void initProgramInputs(Program program, FilterContext context) {
        if (program != null) {
            for (InputPort inputPort : this.mInputPorts.values()) {
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public synchronized void addInputPort(String name) {
        addMaskedInputPort(name, null);
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public synchronized void addMaskedInputPort(String name, FrameFormat formatMask) {
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public synchronized void addOutputPort(String name, FrameFormat format) {
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public synchronized void addOutputBasedOnInput(String outputName, String inputName) {
    ...
    protected synchronized void addFieldPort(String name, Field field, boolean hasDefault, boolean isFinal) {
        InputPort fieldPort;
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public synchronized void addProgramPort(String name, String varName, Field field, Class varType, boolean hasDefault) {
        field.setAccessible(true);
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public synchronized void closeOutputPort(String name) {
        getOutputPort(name).close();
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public synchronized void setWaitsOnInputPort(String portName, boolean waits) {
        getInputPort(portName).setBlocking(waits);
    }

    protected synchronized void setWaitsOnOutputPort(String portName, boolean waits) {
    ...
    public final synchronized Collection<InputPort> getInputPorts() {
        return this.mInputPorts.values();
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public final synchronized Collection<OutputPort> getOutputPorts() {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public final synchronized void openOutputs() {
        if (this.mLogVerbose) {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public final synchronized void clearOutputs() {
        for (OutputPort outputPort : this.mOutputPorts.values()) {
            outputPort.clear();
        }
    }
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public final synchronized void notifyFieldPortValueUpdated(String name, FilterContext context) {
    ...
    private final synchronized void initFinalPorts(KeyValueMap values) {
        this.mInputPorts = new HashMap<>();
        this.mOutputPorts = new HashMap<>();
        addAndSetFinalPorts(values);
    }

    private final synchronized void initRemainingPorts(KeyValueMap values) {
        addAnnotatedPorts();
        setupPorts();
        setInitialInputValues(values);
    }

    private final synchronized void addAndSetFinalPorts(KeyValueMap values) {
    ...
    private final synchronized void addAnnotatedPorts() {
        Field[] declaredFields;
    ...
    private final synchronized void addFieldGenerator(GenerateFieldPort generator, Field field) {
        String name = generator.name().isEmpty() ? field.getName() : generator.name();
        boolean hasDefault = generator.hasDefault();
        addFieldPort(name, field, hasDefault, false);
    }

    private final synchronized void addProgramGenerator(GenerateProgramPort generator, Field field) {
    ...
    private final synchronized void setInitialInputValues(KeyValueMap values) {
        for (Map.Entry<String, Object> entry : values.entrySet()) {
            setInputValue(entry.getKey(), entry.getValue());
        }
    }

    private final synchronized void setImmediateInputValue(String name, Object value) {
        if (this.mLogVerbose) {
    ...
    private final synchronized void transferInputFrames(FilterContext context) {
        for (InputPort inputPort : this.mInputPorts.values()) {
            inputPort.transfer(context);
        }
    }

    private final synchronized Frame wrapInputValue(String inputName, Object value) {
        Frame simpleFrame;
    ...
    private final synchronized void releasePulledFrames(FilterContext context) {
    ...
    private final synchronized boolean inputConditionsMet() {
    ...
    private final synchronized boolean outputConditionsMet() {
    ...
    private final synchronized void closePorts() {
        if (this.mLogVerbose) {
    ...
    private final synchronized boolean filterMustClose() {
        for (InputPort inputPort : this.mInputPorts.values()) {
