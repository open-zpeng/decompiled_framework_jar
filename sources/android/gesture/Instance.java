...
class Instance {
    ...
    private synchronized Instance(long id, float[] sample, String sampleName) {
        this.id = id;
        this.vector = sample;
    ...
    public static synchronized Instance createInstance(int sequenceType, int orientationType, Gesture gesture, String label) {
        if (sequenceType == 2) {
    ...
    private static synchronized float[] spatialSampler(Gesture gesture) {
        return GestureUtils.spatialSampling(gesture, 16, false);
    }

    private static synchronized float[] temporalSampler(int orientationType, Gesture gesture) {
