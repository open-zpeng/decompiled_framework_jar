...
public class GestureOverlayView extends FrameLayout {
    ...
    private synchronized void setCurrentColor(int color) {
    ...
    public synchronized Paint getGesturePaint() {
    ...
    public synchronized void setPaintAlpha(int alpha) {
    ...
    private synchronized void clear(boolean animated, boolean fireActionPerformed, boolean immediate) {
        setPaintAlpha(255);
    ...
    private synchronized boolean processEvent(MotionEvent event) {
    ...
    private synchronized void touchDown(MotionEvent event) {
    ...
    private synchronized Rect touchMove(MotionEvent event) {
        Rect areaToRefresh;
        float x = event.getX();
        float y = event.getY();
    ...
    private synchronized void touchUp(MotionEvent event, boolean cancel) {
        this.mIsListeningForGestures = false;
        if (this.mCurrentGesture != null) {
            this.mCurrentGesture.addStroke(new GestureStroke(this.mStrokeBuffer));
            if (!cancel) {
                ...
                boolean z = true;
                clear(this.mHandleGestureActions && this.mFadeEnabled, (this.mHandleGestureActions && this.mIsGesturing) ? false : false, false);
            } else {
                cancelGesture(event);
            }
        } else {
    ...
    private synchronized void cancelGesture(MotionEvent event) {
        ArrayList<OnGestureListener> listeners = this.mOnGestureListeners;
        int count = listeners.size();
        for (int i = 0; i < count; i++) {
            listeners.get(i).onGestureCancelled(this, event);
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public synchronized void fireOnGesturePerformed() {
