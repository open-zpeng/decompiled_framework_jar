...
public final class CursorJoiner implements Iterator<Result>, Iterable<Result> {
    ...
    private synchronized int[] buildColumnIndiciesArray(Cursor cursor, String[] columnNames) {
    ...
    private static synchronized void populateValues(String[] values, Cursor cursor, int[] columnIndicies, int startingIndex) {
        for (int i = 0; i < columnIndicies.length; i++) {
            values[(i * 2) + startingIndex] = cursor.getString(columnIndicies[i]);
        }
    }

    private synchronized void incrementCursors() {
        if (this.mCompareResultIsValid) {
            switch (this.mCompareResult) {
                case BOTH:
