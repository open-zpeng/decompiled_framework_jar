...
abstract class LocalListenerHelper<TListener> {
    ...
    public interface ListenerOperation<TListener> {
        synchronized void execute(TListener tlistener) throws RemoteException;
    }

    protected abstract synchronized boolean registerWithServer() throws RemoteException;

    protected abstract synchronized void unregisterFromServer() throws RemoteException;

    /* JADX INFO: Access modifiers changed from: protected */
    public synchronized LocalListenerHelper(Context context, String name) {
        Preconditions.checkNotNull(name);
        this.mContext = context;
        this.mTag = name;
    }

    public synchronized boolean add(TListener listener, Handler handler) {
    ...
    public synchronized void remove(TListener listener) {
        Preconditions.checkNotNull(listener);
        synchronized (this.mListeners) {
    ...
    public synchronized void executeOperation(ListenerOperation<TListener> operation, TListener listener) {
        try {
    ...
    public synchronized void foreach(final ListenerOperation<TListener> operation) {
