...
public final class Geofence implements Parcelable {
    private protected static final Parcelable.Creator<Geofence> CREATOR = new Parcelable.Creator<Geofence>() { // from class: android.location.Geofence.1
    ...
    public static synchronized Geofence createCircle(double latitude, double longitude, float radius) {
        return new Geofence(latitude, longitude, radius);
    }

    private synchronized Geofence(double latitude, double longitude, float radius) {
        checkRadius(radius);
    ...
    private static synchronized void checkRadius(float radius) {
        if (radius <= 0.0f) {
            throw new IllegalArgumentException("invalid radius: " + radius);
        }
    }

    private static synchronized void checkLatLong(double latitude, double longitude) {
    ...
    public static synchronized void checkType(int type) {
        if (type != 1) {
