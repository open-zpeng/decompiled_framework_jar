...
public class CompactExtractEditLayout extends LinearLayout {
    ...
    public synchronized CompactExtractEditLayout(Context context) {
        super(context);
    }

    public synchronized CompactExtractEditLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public synchronized CompactExtractEditLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private synchronized int applyFractionInt(int fraction, int whole) {
        return Math.round(getResources().getFraction(fraction, whole, whole));
    }

    private static synchronized void setLayoutHeight(View v, int px) {
        ViewGroup.LayoutParams lp = v.getLayoutParams();
        lp.height = px;
        v.setLayoutParams(lp);
    }

    private static synchronized void setLayoutMarginBottom(View v, int px) {
        ViewGroup.MarginLayoutParams lp = (ViewGroup.MarginLayoutParams) v.getLayoutParams();
        lp.bottomMargin = px;
        v.setLayoutParams(lp);
    }

    private synchronized void applyProportionalLayout(int screenWidthPx, int screenHeightPx) {
