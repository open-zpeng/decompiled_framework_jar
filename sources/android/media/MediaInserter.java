...
public class MediaInserter {
    ...
    public synchronized MediaInserter(ContentProviderClient provider, int bufferSizePerUri) {
        this.mProvider = provider;
        this.mBufferSizePerUri = bufferSizePerUri;
    }

    public synchronized void insert(Uri tableUri, ContentValues values) throws RemoteException {
        insert(tableUri, values, false);
    }

    public synchronized void insertwithPriority(Uri tableUri, ContentValues values) throws RemoteException {
        insert(tableUri, values, true);
    }

    private synchronized void insert(Uri tableUri, ContentValues values, boolean priority) throws RemoteException {
    ...
    private synchronized void flushAllPriority() throws RemoteException {
    ...
    private synchronized void flush(Uri tableUri, List<ContentValues> list) throws RemoteException {
        if (!list.isEmpty()) {
