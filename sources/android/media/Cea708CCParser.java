...
class Cea708CCParser {
    ...
    public interface DisplayListener {
        synchronized void emitEvent(CaptionEvent captionEvent);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized Cea708CCParser(DisplayListener listener) {
    ...
    private synchronized void emitCaptionEvent(CaptionEvent captionEvent) {
        emitCaptionBuffer();
        this.mListener.emitEvent(captionEvent);
    }

    private synchronized void emitCaptionBuffer() {
    ...
    private synchronized int parseServiceBlockData(byte[] data, int pos) {
    ...
    private synchronized int parseC0(byte[] data, int pos) {
    ...
    private synchronized int parseC1(byte[] data, int pos) {
        int i = this.mCommand;
        switch (i) {
            case 128:
            case 129:
            ...
            case 135:
                ...
                int windowId = windowBitmap + WifiNetworkScoreCache.INVALID_NETWORK_SCORE;
    ...
    private synchronized int parseG0(byte[] data, int pos) {
    ...
    private synchronized int parseG1(byte[] data, int pos) {
        this.mBuffer.append((char) this.mCommand);
        return pos;
    }

    private synchronized int parseExt1(byte[] data, int pos) {
        this.mCommand = data[pos] & 255;
        int pos2 = pos + 1;
        if (this.mCommand >= 0 && this.mCommand <= 31) {
    ...
    private synchronized int parseC2(byte[] data, int pos) {
    ...
    private synchronized int parseC3(byte[] data, int pos) {
    ...
    private synchronized int parseG2(byte[] data, int pos) {
        int i = this.mCommand;
        if (i != 48) {
            switch (i) {
            }
    ...
    private synchronized int parseG3(byte[] data, int pos) {
        int i = this.mCommand;
        return pos;
    }
    ...
    public static class CaptionColor {
        ...
        public synchronized CaptionColor(int opacity, int red, int green, int blue) {
        ...
        public synchronized int getArgbValue() {
    ...
    public static class CaptionEvent {
        ...
        public synchronized CaptionEvent(int type, Object obj) {
            this.type = type;
            this.obj = obj;
        }
    }
    ...
    public static class CaptionPenAttr {
        ...
        public synchronized CaptionPenAttr(int penSize, int penOffset, int textTag, int fontTag, int edgeType, boolean underline, boolean italic) {
    ...
    public static class CaptionPenColor {
        ...
        public synchronized CaptionPenColor(CaptionColor foregroundColor, CaptionColor backgroundColor, CaptionColor edgeColor) {
    ...
    public static class CaptionPenLocation {
        ...
        public synchronized CaptionPenLocation(int row, int column) {
            this.row = row;
            this.column = column;
        }
    }
    ...
    public static class CaptionWindowAttr {
        ...
        public synchronized CaptionWindowAttr(CaptionColor fillColor, CaptionColor borderColor, int borderType, boolean wordWrap, int printDirection, int scrollDirection, int justify, int effectDirection, int effectSpeed, int displayEffect) {
    ...
    public static class CaptionWindow {
        ...
        public synchronized CaptionWindow(int id, boolean visible, boolean rowLock, boolean columnLock, int priority, boolean relativePositioning, int anchorVertical, int anchorHorizontal, int anchorId, int rowCount, int columnCount, int penStyle, int windowStyle) {
            this.id = id;
