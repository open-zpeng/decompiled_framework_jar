...
public class AudioEffect {
    ...
    public static synchronized Descriptor[] queryPreProcessings(int audioSession) {
    ...
    private protected int command(int cmdCode, byte[] command, byte[] reply) throws IllegalStateException {
        checkState("command()");
    ...
    private synchronized void createNativeEventHandler() {
    ...
    private static synchronized void postEventFromNative(Object effect_ref, int what, int arg1, int arg2, Object obj) {
    ...
    private protected void checkState(String methodName) throws IllegalStateException {
        synchronized (this.mStateLock) {
            if (this.mState != 1) {
    ...
    public synchronized void checkStatus(int status) {
        if (isError(status)) {
            switch (status) {
                case -5:
    ...
    public static synchronized int byteArrayToInt(byte[] valueBuf, int offset) {
    ...
    public static synchronized short byteArrayToShort(byte[] valueBuf, int offset) {
        ByteBuffer converter = ByteBuffer.wrap(valueBuf);
        converter.order(ByteOrder.nativeOrder());
    ...
    public static synchronized float byteArrayToFloat(byte[] valueBuf) {
        return byteArrayToFloat(valueBuf, 0);
    }

    public static synchronized float byteArrayToFloat(byte[] valueBuf, int offset) {
        ByteBuffer converter = ByteBuffer.wrap(valueBuf);
        converter.order(ByteOrder.nativeOrder());
        return converter.getFloat(offset);
    }

    public static synchronized byte[] floatToByteArray(float value) {
        ByteBuffer converter = ByteBuffer.allocate(4);
        converter.order(ByteOrder.nativeOrder());
