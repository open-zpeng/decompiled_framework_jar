...
public class MediaRouter {
    ...
    public static class Static implements DisplayManager.DisplayListener {
        ...
        synchronized Static(Context appContext) {
        ...
        synchronized void startMonitoringRoutes(Context appContext) {
        ...
        synchronized void updateAudioRoutes(AudioRoutesInfo newRoutes) {
            int name;
        ...
        synchronized boolean isBluetoothA2dpOn() {
            try {
                if (this.mBluetoothA2dpRoute != null) {
        ...
        synchronized void updateDiscoveryRequest() {
            ...
            for (int routeTypes2 = 0; routeTypes2 < count; routeTypes2++) {
                CallbackInfo cbi = this.mCallbacks.get(routeTypes2);
                if ((cbi.flags & 5) != 0) {
                    routeTypes |= cbi.type;
        ...
        public synchronized Display[] getAllPresentationDisplays() {
            return this.mDisplayService.getDisplays(DisplayManager.DISPLAY_CATEGORY_PRESENTATION);
        }

        private synchronized void updatePresentationDisplays(int changedDisplayId) {
        ...
        synchronized void setSelectedRoute(RouteInfo info, boolean explicit) {
            this.mSelectedRoute = info;
            publishClientSelectedRoute(explicit);
        }

        synchronized void rebindAsUser(int userId) {
        ...
        synchronized void publishClientDiscoveryRequest() {
            if (this.mClient != null) {
                try {
        ...
        synchronized void publishClientSelectedRoute(boolean explicit) {
            if (this.mClient != null) {
                try {
        ...
        synchronized void updateClientState() {
        ...
        synchronized void requestSetVolume(RouteInfo route, int volume) {
        ...
        synchronized void requestUpdateVolume(RouteInfo route, int direction) {
            if (route.mGlobalRouteId != null && this.mClient != null) {
                try {
        ...
        synchronized RouteInfo makeGlobalRoute(MediaRouterClientState.RouteInfo globalRoute) {
        ...
        synchronized void updateGlobalRoute(RouteInfo route, MediaRouterClientState.RouteInfo globalRoute) {
        ...
        synchronized RouteInfo findGlobalRoute(String globalRouteId) {
            int count = this.mRoutes.size();
            for (int i = 0; i < count; i++) {
                RouteInfo route = this.mRoutes.get(i);
        ...
        synchronized boolean isPlaybackActive() {
            if (this.mClient != null) {
                try {
        ...
        public final class Client extends IMediaRouterClient.Stub {
            ...
            @Override // android.media.IMediaRouterClient
            public synchronized void onRestoreRoute() {
    ...
    static synchronized String typesToString(int types) {
    ...
    public synchronized MediaRouter(Context context) {
    ...
    public synchronized RouteInfo getFallbackRoute() {
        return sStatic.mBluetoothA2dpRoute != null ? sStatic.mBluetoothA2dpRoute : sStatic.mDefaultAudioVideo;
    }

    public synchronized RouteCategory getSystemCategory() {
    ...
    public synchronized boolean isRouteAvailable(int types, int flags) {
    ...
    private synchronized int findCallbackInfo(Callback cb) {
    ...
    private protected void selectRouteInt(int types, RouteInfo route, boolean explicit) {
        selectRouteStatic(types, route, explicit);
    }

    static synchronized void selectRouteStatic(int types, RouteInfo route, boolean explicit) {
            ...
            try {
                ...
                if (route != btRoute) {
                    ...
                    for (int i = 3; i < callStack.length; i++) {
                        ...
                        sb.append(caller.getClassName() + "." + caller.getMethodName() + SettingsStringUtil.DELIMITER + caller.getLineNumber());
                        sb.append("  ");
                    }
    ...
    static synchronized void selectDefaultRouteStatic() {
    ...
    static synchronized boolean matchesDeviceAddress(WifiDisplay display, RouteInfo info) {
    ...
    public synchronized void addRouteInt(RouteInfo info) {
        addRouteStatic(info);
    }

    static synchronized void addRouteStatic(RouteInfo info) {
    ...
    public synchronized void removeRouteInt(RouteInfo info) {
        removeRouteStatic(info);
    }

    static synchronized void removeRouteStatic(RouteInfo info) {
    ...
    static synchronized int getRouteCountStatic() {
        return sStatic.mRoutes.size();
    }

    static synchronized RouteInfo getRouteAtStatic(int index) {
        return sStatic.mRoutes.get(index);
    }
    ...
    public synchronized void rebindAsUser(int userId) {
        sStatic.rebindAsUser(userId);
    }

    static synchronized void updateRoute(RouteInfo info) {
        dispatchRouteChanged(info);
    }

    static synchronized void dispatchRouteSelected(int type, RouteInfo info) {
    ...
    static synchronized void dispatchRouteUnselected(int type, RouteInfo info) {
        Iterator<CallbackInfo> it = sStatic.mCallbacks.iterator();
        while (it.hasNext()) {
            CallbackInfo cbi = it.next();
            if (cbi.filterRouteEvent(info)) {
    ...
    static synchronized void dispatchRouteChanged(RouteInfo info) {
        dispatchRouteChanged(info, info.mSupportedTypes);
    }

    static synchronized void dispatchRouteChanged(RouteInfo info, int oldSupportedTypes) {
        if (DEBUG) {
    ...
    static synchronized void dispatchRouteAdded(RouteInfo info) {
        Iterator<CallbackInfo> it = sStatic.mCallbacks.iterator();
        while (it.hasNext()) {
            CallbackInfo cbi = it.next();
            if (cbi.filterRouteEvent(info)) {
    ...
    static synchronized void dispatchRouteRemoved(RouteInfo info) {
        Iterator<CallbackInfo> it = sStatic.mCallbacks.iterator();
        while (it.hasNext()) {
            CallbackInfo cbi = it.next();
            if (cbi.filterRouteEvent(info)) {
    ...
    static synchronized void dispatchRouteGrouped(RouteInfo info, RouteGroup group, int index) {
        Iterator<CallbackInfo> it = sStatic.mCallbacks.iterator();
        while (it.hasNext()) {
            CallbackInfo cbi = it.next();
    ...
    static synchronized void dispatchRouteUngrouped(RouteInfo info, RouteGroup group) {
        Iterator<CallbackInfo> it = sStatic.mCallbacks.iterator();
        while (it.hasNext()) {
            CallbackInfo cbi = it.next();
            if (cbi.filterRouteEvent(group)) {
    ...
    static synchronized void dispatchRouteVolumeChanged(RouteInfo info) {
        Iterator<CallbackInfo> it = sStatic.mCallbacks.iterator();
        while (it.hasNext()) {
            CallbackInfo cbi = it.next();
            if (cbi.filterRouteEvent(info)) {
    ...
    static synchronized void dispatchRoutePresentationDisplayChanged(RouteInfo info) {
        Iterator<CallbackInfo> it = sStatic.mCallbacks.iterator();
        while (it.hasNext()) {
            CallbackInfo cbi = it.next();
            if (cbi.filterRouteEvent(info)) {
    ...
    static synchronized void systemVolumeChanged(int newValue) {
    ...
    static synchronized void updateWifiDisplayStatus(WifiDisplayStatus status) {
    ...
    private static synchronized boolean shouldShowWifiDisplay(WifiDisplay d, WifiDisplay activeDisplay) {
        return d.isRemembered() || d.equals(activeDisplay);
    }

    static synchronized int getWifiDisplayStatusCode(WifiDisplay d, WifiDisplayStatus wfdStatus) {
        int newStatus;
    ...
    static synchronized boolean isWifiDisplayEnabled(WifiDisplay d, WifiDisplayStatus wfdStatus) {
        return d.isAvailable() && (d.canConnect() || d.equals(wfdStatus.getActiveDisplay()));
    }

    static synchronized RouteInfo makeWifiDisplayRoute(WifiDisplay display, WifiDisplayStatus wfdStatus) {
    ...
    private static synchronized void updateWifiDisplayRoute(RouteInfo route, WifiDisplay display, WifiDisplayStatus wfdStatus, boolean disconnected) {
        boolean changed = false;
    ...
    private static synchronized WifiDisplay findWifiDisplay(WifiDisplay[] displays, String deviceAddress) {
    ...
    private static synchronized RouteInfo findWifiDisplayRoute(WifiDisplay d) {
        int count = sStatic.mRoutes.size();
        for (int i = 0; i < count; i++) {
            RouteInfo info = sStatic.mRoutes.get(i);
    ...
    public static class RouteInfo {
        ...
        private protected static final int STATUS_CONNECTING = 2;
        ...
        Drawable mIcon;
        CharSequence mName;
        public private protected int mNameResId;
        ...
        synchronized RouteInfo(RouteCategory category) {
        ...
        public private protected CharSequence getName(Resources res) {
        ...
        synchronized boolean setRealStatusCode(int statusCode) {
        ...
        synchronized boolean resolveStatusCode() {
        ...
        synchronized boolean updatePresentationDisplay() {
        ...
        private synchronized Display choosePresentationDisplay() {
        ...
        private protected String getDeviceAddress() {
        ...
        public synchronized boolean isBluetooth() {
        ...
        synchronized void setStatusInt(CharSequence status) {
        ...
        synchronized void routeUpdated() {
    ...
    public static class UserRouteInfo extends RouteInfo {
        ...
        synchronized UserRouteInfo(RouteCategory category) {
            super(category);
        ...
        private synchronized void updatePlaybackInfoOnRcc() {
            configureSessionVolume();
        }

        private synchronized void configureSessionVolume() {
            if (this.mRcc == null) {
    ...
    public static class RouteGroup extends RouteInfo {
        ...
        synchronized RouteGroup(RouteCategory category) {
            super(category);
        ...
        synchronized CharSequence getName(Resources res) {
            if (this.mUpdateName) {
                updateName();
            }
        ...
        synchronized void memberNameChanged(RouteInfo info, CharSequence name) {
            this.mUpdateName = true;
            routeUpdated();
        }

        synchronized void memberStatusChanged(RouteInfo info, CharSequence status) {
            setStatusInt(status);
        }

        synchronized void memberVolumeChanged(RouteInfo info) {
            updateVolume();
        }
        ...
        synchronized void routeUpdated() {
            ...
            while (true) {
                ...
                RouteInfo route = this.mRoutes.get(types);
                maxVolume2 |= route.mSupportedTypes;
                int routeMaxVolume = route.getVolumeMax();
                if (routeMaxVolume > maxVolume) {
                    maxVolume = routeMaxVolume;
                }
                isFixedVolume2 &= route.getPlaybackType() == 0;
    ...
    public static class RouteCategory {
        ...
        synchronized RouteCategory(CharSequence name, int types, boolean groupable) {
            this.mName = name;
            this.mTypes = types;
            this.mGroupable = groupable;
        }

        synchronized RouteCategory(int nameResId, int types, boolean groupable) {
    ...
    public static class CallbackInfo {
        ...
        public synchronized CallbackInfo(Callback cb, int type, int flags, MediaRouter router) {
            this.cb = cb;
            this.type = type;
            this.flags = flags;
            this.router = router;
        ...
        public synchronized boolean filterRouteEvent(RouteInfo route) {
            return filterRouteEvent(route.mSupportedTypes);
        }

        public synchronized boolean filterRouteEvent(int supportedTypes) {
    ...
    static class VolumeCallbackInfo {
        ...
        public synchronized VolumeCallbackInfo(VolumeCallback vcb, RouteInfo route) {
            this.vcb = vcb;
            this.route = route;
        }
    }
    ...
    public static class VolumeChangeReceiver extends BroadcastReceiver {
        synchronized VolumeChangeReceiver() {
        }
    ...
    public static class WifiDisplayStatusChangedReceiver extends BroadcastReceiver {
        synchronized WifiDisplayStatusChangedReceiver() {
        }
