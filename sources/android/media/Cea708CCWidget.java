...
class Cea708CCWidget extends ClosedCaptionWidget implements Cea708CCParser.DisplayListener {
    ...
    public synchronized Cea708CCWidget(Context context) {
        this(context, null);
    }

    public synchronized Cea708CCWidget(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public synchronized Cea708CCWidget(Context context, AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, 0);
    }

    public synchronized Cea708CCWidget(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        this.mCCHandler = new CCHandler((CCLayout) this.mClosedCaptionLayout);
    }

    @Override // android.media.ClosedCaptionWidget
    public synchronized ClosedCaptionWidget.ClosedCaptionLayout createCaptionLayout(Context context) {
        return new CCLayout(context);
    }

    @Override // android.media.Cea708CCParser.DisplayListener
    public synchronized void emitEvent(Cea708CCParser.CaptionEvent event) {
    ...
    public static class ScaledLayout extends ViewGroup {
        ...
        public synchronized ScaledLayout(Context context) {
            super(context);
        }
        ...
        public static class ScaledLayoutParams extends ViewGroup.LayoutParams {
            ...
            public synchronized ScaledLayoutParams(float scaleStartRow, float scaleEndRow, float scaleStartCol, float scaleEndCol) {
                super(-1, -1);
            ...
            public synchronized ScaledLayoutParams(Context context, AttributeSet attrs) {
                super(-1, -1);
            }
        }
    ...
    public static class CCLayout extends ScaledLayout implements ClosedCaptionWidget.ClosedCaptionLayout {
        ...
        public synchronized CCLayout(Context context) {
            super(context);
        ...
        public synchronized void addOrUpdateViewToSafeTitleArea(CCWindowLayout captionWindowLayout, ScaledLayout.ScaledLayoutParams scaledLayoutParams) {
        ...
        public synchronized void removeViewFromSafeTitleArea(CCWindowLayout captionWindowLayout) {
            this.mSafeTitleAreaLayout.removeView(captionWindowLayout);
        }

        @Override // android.media.ClosedCaptionWidget.ClosedCaptionLayout
        public synchronized void setCaptionStyle(CaptioningManager.CaptionStyle style) {
        ...
        @Override // android.media.ClosedCaptionWidget.ClosedCaptionLayout
        public synchronized void setFontScale(float fontScale) {
            int count = this.mSafeTitleAreaLayout.getChildCount();
            for (int i = 0; i < count; i++) {
                CCWindowLayout windowLayout = (CCWindowLayout) this.mSafeTitleAreaLayout.getChildAt(i);
    ...
    static class CCHandler implements Handler.Callback {
        ...
        public synchronized CCHandler(CCLayout ccLayout) {
        ...
        public synchronized void processCaptionEvent(Cea708CCParser.CaptionEvent event) {
            if (this.mIsDelayed) {
        ...
        private synchronized void setCurrentWindowLayout(int windowId) {
        ...
        private synchronized ArrayList<CCWindowLayout> getWindowsFromBitmap(int windowBitmap) {
            CCWindowLayout windowLayout;
        ...
        private synchronized void clearWindows(int windowBitmap) {
            if (windowBitmap == 0) {
                return;
            }
        ...
        private synchronized void displayWindows(int windowBitmap) {
            if (windowBitmap == 0) {
                return;
            }
            Iterator<CCWindowLayout> it = getWindowsFromBitmap(windowBitmap).iterator();
        ...
        private synchronized void hideWindows(int windowBitmap) {
            if (windowBitmap == 0) {
                return;
            }
            Iterator<CCWindowLayout> it = getWindowsFromBitmap(windowBitmap).iterator();
        ...
        private synchronized void toggleWindows(int windowBitmap) {
            if (windowBitmap == 0) {
                return;
            }
            Iterator<CCWindowLayout> it = getWindowsFromBitmap(windowBitmap).iterator();
        ...
        private synchronized void deleteWindows(int windowBitmap) {
            if (windowBitmap == 0) {
                return;
            }
            Iterator<CCWindowLayout> it = getWindowsFromBitmap(windowBitmap).iterator();
        ...
        private synchronized void setWindowAttr(Cea708CCParser.CaptionWindowAttr windowAttr) {
            if (this.mCurrentWindowLayout != null) {
                this.mCurrentWindowLayout.setWindowAttr(windowAttr);
            }
        }

        private synchronized void defineWindow(Cea708CCParser.CaptionWindow window) {
            int windowId;
        ...
        private synchronized void delay(int tenthsOfSeconds) {
        ...
        private synchronized void delayCancel() {
            this.mIsDelayed = false;
            processPendingBuffer();
        }

        private synchronized void processPendingBuffer() {
        ...
        private synchronized void sendControlToCurrentWindow(char control) {
            if (this.mCurrentWindowLayout != null) {
                this.mCurrentWindowLayout.sendControl(control);
            }
        }

        private synchronized void sendBufferToCurrentWindow(String buffer) {
            if (this.mCurrentWindowLayout != null) {
        ...
        private synchronized void setPenAttr(Cea708CCParser.CaptionPenAttr attr) {
            if (this.mCurrentWindowLayout != null) {
                this.mCurrentWindowLayout.setPenAttr(attr);
            }
        }

        private synchronized void setPenColor(Cea708CCParser.CaptionPenColor color) {
            if (this.mCurrentWindowLayout != null) {
                this.mCurrentWindowLayout.setPenColor(color);
            }
        }

        private synchronized void setPenLocation(Cea708CCParser.CaptionPenLocation location) {
            if (this.mCurrentWindowLayout != null) {
    ...
    public static class CCWindowLayout extends RelativeLayout implements View.OnLayoutChangeListener {
        ...
        public synchronized CCWindowLayout(Context context) {
            this(context, null);
        }

        public synchronized CCWindowLayout(Context context, AttributeSet attrs) {
            this(context, attrs, 0);
        }

        public synchronized CCWindowLayout(Context context, AttributeSet attrs, int defStyleAttr) {
            this(context, attrs, defStyleAttr, 0);
        }

        public synchronized CCWindowLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
            super(context, attrs, defStyleAttr, defStyleRes);
            this.mRowLimit = 0;
        ...
        public synchronized int getCaptionWindowId() {
            return this.mCaptionWindowId;
        }

        public synchronized void setCaptionWindowId(int captionWindowId) {
        ...
        public synchronized void setPenAttr(Cea708CCParser.CaptionPenAttr penAttr) {
        ...
        public synchronized void setPenColor(Cea708CCParser.CaptionPenColor penColor) {
        }

        public synchronized void setPenLocation(int row, int column) {
            if (this.mRow >= 0) {
        ...
        public synchronized void setWindowAttr(Cea708CCParser.CaptionWindowAttr windowAttr) {
        }

        public synchronized void sendBuffer(String buffer) {
            appendText(buffer);
        }

        public synchronized void sendControl(char control) {
        }

        public synchronized void initWindow(CCLayout ccLayout, Cea708CCParser.CaptionWindow captionWindow) {
        ...
        private synchronized void updateWidestChar() {
            Paint paint = new Paint();
            paint.setTypeface(this.mCaptionStyle.getTypeface());
        ...
        private synchronized void updateTextSize() {
            if (this.mCCLayout == null) {
                return;
            }
            StringBuilder widestTextBuilder = new StringBuilder();
        ...
        private synchronized int getScreenColumnCount() {
            return 42;
        }

        public synchronized void removeFromCaptionView() {
            if (this.mCCLayout != null) {
        ...
        private synchronized void updateText(String text, boolean appended) {
            if (!appended) {
                this.mBuilder.clear();
            }
        ...
        public synchronized void setRowLimit(int rowLimit) {
            if (rowLimit < 0) {
    ...
    public static class CCView extends SubtitleView {
        ...
        public synchronized CCView(Context context) {
            this(context, null);
        }

        public synchronized CCView(Context context, AttributeSet attrs) {
            this(context, attrs, 0);
        }

        public synchronized CCView(Context context, AttributeSet attrs, int defStyleAttr) {
            this(context, attrs, defStyleAttr, 0);
        }

        public synchronized CCView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
            super(context, attrs, defStyleAttr, defStyleRes);
        }
