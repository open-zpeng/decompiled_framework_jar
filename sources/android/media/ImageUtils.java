        ...
        while (i < srcPlanes.length) {
            ...
            if (srcRowStride == dstRowStride) {
                ...
                for (int row = 0; row < effectivePlaneSize.getHeight(); row++) {
                    if (row == effectivePlaneSize.getHeight() - 1 && srcByteCount2 > (remainingBytes = srcBuffer.remaining() - srcOffset)) {
                        srcByteCount2 = remainingBytes;
                    }
                    directByteBufferCopy(srcBuffer, srcOffset, dstBuffer, dstOffset, srcByteCount2);
