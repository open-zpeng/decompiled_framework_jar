...
public class WebVttRenderingWidget extends ViewGroup implements SubtitleTrack.RenderingWidget {
    ...
    public synchronized WebVttRenderingWidget(Context context) {
        this(context, null);
    }

    public synchronized WebVttRenderingWidget(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public synchronized WebVttRenderingWidget(Context context, AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, 0);
    }

    public synchronized WebVttRenderingWidget(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private synchronized void manageChangeListener() {
    ...
    public synchronized void setCaptionStyle(CaptioningManager.CaptionStyle captionStyle, float fontSize) {
    ...
    private synchronized void prepForPrune() {
        int regionCount = this.mRegionBoxes.size();
    ...
    private synchronized void layoutRegion(int viewportWidth, int viewportHeight, RegionLayout regionBox) {
    ...
    private synchronized void layoutCue(int viewportWidth, int viewportHeight, CueLayout cueBox) {
        int xPosition;
        int top;
    ...
    private synchronized int calculateLinePosition(CueLayout cueBox) {
        TextTrackCue cue = cueBox.getCue();
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static synchronized int resolveCueAlignment(int layoutDirection, int alignment) {
        switch (alignment) {
            case 201:
    ...
    public static class RegionLayout extends LinearLayout {
        ...
        public synchronized RegionLayout(Context context, TextTrackRegion region, CaptioningManager.CaptionStyle captionStyle, float fontSize) {
            super(context);
        ...
        public synchronized void measureForParent(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        public synchronized void prepForPrune() {
            int cueCount = this.mRegionCueBoxes.size();
            for (int i = 0; i < cueCount; i++) {
                CueLayout cueBox = this.mRegionCueBoxes.get(i);
                cueBox.prepForPrune();
        ...
        public synchronized void put(TextTrackCue cue) {
            int cueCount = this.mRegionCueBoxes.size();
            for (int i = 0; i < cueCount; i++) {
                CueLayout cueBox = this.mRegionCueBoxes.get(i);
        ...
        public synchronized TextTrackRegion getRegion() {
            return this.mRegion;
        }
    }
    ...
    public static class CueLayout extends LinearLayout {
        ...
        public synchronized CueLayout(Context context, TextTrackCue cue, CaptioningManager.CaptionStyle captionStyle, float fontSize) {
            super(context);
            this.mCue = cue;
            this.mCaptionStyle = captionStyle;
            this.mFontSize = fontSize;
        ...
        public synchronized void setCaptionStyle(CaptioningManager.CaptionStyle style, float fontSize) {
        ...
        public synchronized void measureForParent(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            if (absAlignment != 200) {
                ...
                if (maximumSize3 <= 50) {
                    ...
                    maximumSize = (100 - maximumSize4) * 2;
                }
            }
        ...
        public synchronized TextTrackCue getCue() {
            return this.mCue;
        }
    }
    ...
    public static class SpanLayout extends SubtitleView {
        ...
        public synchronized SpanLayout(Context context, TextTrackCueSpan[] spans) {
            super(context);
