...
public class ValueAnimator extends Animator implements AnimationHandler.AnimationFrameCallback {
    ...
    public protected static float sDurationScale = 1.0f;
    ...
    public protected long mDuration = 300;
    ...
    public synchronized void overrideDurationScale(float durationScale) {
        this.mDurationScale = durationScale;
    }

    private synchronized float resolveDurationScale() {
        return this.mDurationScale >= 0.0f ? this.mDurationScale : sDurationScale;
    }

    private synchronized long getScaledDuration() {
    ...
    private synchronized int getCurrentIteration(float fraction) {
        float fraction2 = clampFraction(fraction);
    ...
    private synchronized float getCurrentIterationFraction(float fraction, boolean inReverse) {
        float fraction2 = clampFraction(fraction);
    ...
    private synchronized float clampFraction(float fraction) {
        if (fraction < 0.0f) {
            return 0.0f;
        }
        if (this.mRepeatCount != -1) {
    ...
    private synchronized boolean shouldPlayBackward(int iteration, boolean inReverse) {
    ...
    private synchronized void start(boolean playBackwards) {
    ...
    private synchronized boolean isPulsingInternal() {
        return this.mLastFrameTime >= 0;
    }

    synchronized String getNameForTrace() {
        return "animator";
    }
    ...
    @Override // android.animation.AnimationHandler.AnimationFrameCallback
    public final synchronized boolean doAnimationFrame(long frameTime) {
    ...
    private synchronized void addOneShotCommitCallback() {
        if (!this.mSelfPulse) {
            return;
        }
    ...
    public private protected void animateValue(float fraction) {
    ...
    public static synchronized int getCurrentAnimationsCount() {
    ...
    public synchronized AnimationHandler getAnimationHandler() {
