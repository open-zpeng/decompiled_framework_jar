...
public class MobileRadioPowerCalculator extends PowerCalculator {
    ...
    private synchronized double getMobilePowerPerPacket(long rawRealtimeUs, int statsType) {
    ...
    public synchronized MobileRadioPowerCalculator(PowerProfile profile, BatteryStats stats) {
        double temp = profile.getAveragePowerOrDefault("radio.active", -1.0d);
        int i = 0;
        int i2 = 1;
        if (temp != -1.0d) {
        ...
        if (profile.getAveragePowerOrDefault("radio.on", -1.0d) == -1.0d) {
            ...
            while (true) {
                ...
                this.mPowerBins[i5] = profile.getAveragePower("radio.on", i5);
                i = i5 + 1;
            }
        }
        this.mPowerScan = profile.getAveragePowerOrDefault("radio.scanning", FeatureOption.FO_BOOT_POLICY_CPU);
        this.mStats = stats;
    }

    @Override // com.android.internal.os.PowerCalculator
    public synchronized void calculateApp(BatterySipper app, BatteryStats.Uid u, long rawRealtimeUs, long rawUptimeUs, int statsType) {
    ...
    public synchronized void reset(BatteryStats stats) {
        reset();
        this.mStats = stats;
    }
}
