...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    protected ViewGroup generateLayout(DecorView decor) {
        ...
        boolean overrideToFloating = xpActivityManager.overrideToFloating(getContext().getPackageName());
        if (overrideToFloating) {
            this.mIsFloating = true;
        }
        int flagsToUpdate = (~getForcedWindowFlags()) & 65792;
        if (this.mIsFloating) {
            if (overrideToFloating) {
                Rect bounds = xpActivityManager.getOverrideFloatingAppBounds();
                int width = bounds != null ? bounds.width() : -2;
                int height = bounds != null ? bounds.height() : -2;
                setGravity(17);
        ...
        if (targetSdk >= 11) {
            targetPreHoneycomb = false;
        } else {
            targetPreHoneycomb = true;
        }
        if (targetSdk >= 14) {
            targetPreIcs = false;
        } else {
        ...
        if (getContainer() == null) {
            if (this.mBackgroundDrawable == null) {
                if (this.mFrameResource == 0) {
                    this.mFrameResource = a.getResourceId(2, i);
                }
                if (a.hasValue(1)) {
        ...
        if ((features & 2048) != 0) {
            ...
            int layoutResource2 = features & 24;
