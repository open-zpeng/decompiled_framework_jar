...
public class PipSnapAlgorithm {
    ...
    public Rect findClosestSnapBounds(Rect movementBounds, Rect stackBounds) {
        ...
        if (i == 4 || i == 3) {
            ...
            int i3 = stackBounds.left;
            Point snapTarget = findClosestPoint(i3, stackBounds.top, snapTargets);
            ...
            for (int i4 = 0; i4 < this.mSnapGravities.size(); i4++) {
                Gravity.apply(this.mSnapGravities.get(i4).intValue(), stackBounds.width(), stackBounds.height(), pipBounds, 0, 0, tmpBounds2);
                snapTargets2[i4] = new Point(tmpBounds2.left, tmpBounds2.top);
            }
    ...
    public Size getSizeForAspectRatio(float aspectRatio, float minEdgeSize, int displayWidth, int displayHeight) {
        ...
        if (aspectRatio > this.mMinAspectRatioForMinSize) {
            ...
            if (aspectRatio <= f) {
                float widthAtMaxAspectRatioForMinSize = f * minSize;
        ...
        int width2 = (aspectRatio > 1.0f ? 1 : (aspectRatio == 1.0f ? 0 : -1));
        if (width2 <= 0) {
            width = minSize;
