...
public class FastPrintWriter extends PrintWriter {
    ...
    private static class DummyWriter extends Writer {
        private synchronized DummyWriter() {
        }
    ...
    public synchronized FastPrintWriter(OutputStream out, boolean autoFlush) {
        this(out, autoFlush, 8192);
    }

    public synchronized FastPrintWriter(OutputStream out, boolean autoFlush, int bufferLen) {
    ...
    public synchronized FastPrintWriter(Writer wr) {
        this(wr, false, 8192);
    }

    public synchronized FastPrintWriter(Writer wr, boolean autoFlush) {
        this(wr, autoFlush, 8192);
    }

    public synchronized FastPrintWriter(Writer wr, boolean autoFlush, int bufferLen) {
        super(new DummyWriter(), autoFlush);
        if (wr == null) {
    ...
    public synchronized FastPrintWriter(Printer pr) {
        this(pr, 512);
    }

    public synchronized FastPrintWriter(Printer pr, int bufferLen) {
    ...
    private final synchronized void initEncoder(String csn) throws UnsupportedEncodingException {
        try {
    ...
    private final synchronized void initDefaultEncoder() {
        this.mCharset = Charset.defaultCharset().newEncoder();
        this.mCharset.onMalformedInput(CodingErrorAction.REPLACE);
        this.mCharset.onUnmappableCharacter(CodingErrorAction.REPLACE);
    }

    private synchronized void appendLocked(char c) throws IOException {
        int pos = this.mPos;
    ...
    private synchronized void appendLocked(String str, int i, int length) throws IOException {
    ...
    private synchronized void appendLocked(char[] buf, int i, int length) throws IOException {
        int BUFFER_LEN = this.mBufferLen;
        if (length > BUFFER_LEN) {
            int end = i + length;
            while (i < end) {
    ...
    private synchronized void flushBytesLocked() throws IOException {
        int position;
    ...
    private synchronized void flushLocked() throws IOException {
        if (this.mPos > 0) {
