...
public class NotificationColorUtil {
    ...
    public static synchronized NotificationColorUtil getInstance(Context context) {
    ...
    private synchronized NotificationColorUtil(Context context) {
        this.mGrayscaleIconMaxSize = context.getResources().getDimensionPixelSize(17104901);
    }

    public synchronized boolean isGrayscaleIcon(Bitmap bitmap) {
        boolean result;
        int generationId;
    ...
    public synchronized boolean isGrayscaleIcon(Drawable d) {
        if (d == null) {
            return false;
        }
    ...
    public synchronized boolean isGrayscaleIcon(Context context, Icon icon) {
        if (icon == null) {
            return false;
        }
        switch (icon.getType()) {
    ...
    public synchronized boolean isGrayscaleIcon(Context context, int drawableResId) {
        if (drawableResId == 0) {
            return false;
        }
        try {
    ...
    public synchronized CharSequence invertCharSequenceColors(CharSequence charSequence) {
        Object resultSpan;
    ...
    private synchronized TextAppearanceSpan processTextAppearanceSpan(TextAppearanceSpan span) {
    ...
    public static synchronized CharSequence clearColorSpans(CharSequence charSequence) {
        if (charSequence instanceof Spanned) {
            Spanned ss = (Spanned) charSequence;
            Object[] spans = ss.getSpans(0, ss.length(), Object.class);
            SpannableStringBuilder builder = new SpannableStringBuilder(ss.toString());
    ...
    private synchronized int processColor(int color) {
        return Color.argb(Color.alpha(color), 255 - Color.red(color), 255 - Color.green(color), 255 - Color.blue(color));
    }

    public static synchronized int findContrastColor(int color, int other, boolean findFg, double minRatio) {
        ...
        double low = FeatureOption.FO_BOOT_POLICY_CPU;
        double high = lab[0];
        double a = lab[1];
        double b = lab[2];
    ...
    public static synchronized int findAlphaToMeetContrast(int color, int backgroundColor, double minRatio) {
    ...
    public static synchronized int findContrastColorAgainstDark(int color, int other, boolean findFg, double minRatio) {
        int fg = findFg ? color : other;
        int bg = findFg ? other : color;
        if (ColorUtilsFromCompat.calculateContrast(fg, bg) >= minRatio) {
            return color;
    ...
    public static synchronized int ensureTextContrastOnBlack(int color) {
        return findContrastColorAgainstDark(color, -16777216, true, 12.0d);
    }

    public static synchronized int ensureLargeTextContrast(int color, int bg, boolean isBgDarker) {
        if (isBgDarker) {
    ...
    public static synchronized int ensureTextContrast(int color, int bg, boolean isBgDarker) {
        return ensureContrast(color, bg, isBgDarker, 4.5d);
    }

    public static synchronized int ensureContrast(int color, int bg, boolean isBgDarker, double minRatio) {
        if (isBgDarker) {
    ...
    public static synchronized int ensureTextBackgroundColor(int color, int textColor, int hintColor) {
        return findContrastColor(findContrastColor(color, hintColor, false, 3.0d), textColor, false, 4.5d);
    }

    private static synchronized String contrastChange(int colorOld, int colorNew, int bg) {
    ...
    public static synchronized int resolveColor(Context context, int color) {
        if (color == 0) {
    ...
    public static synchronized int resolveContrastColor(Context context, int notificationColor, int backgroundColor) {
        return resolveContrastColor(context, notificationColor, backgroundColor, false);
    }

    public static synchronized int resolveContrastColor(Context context, int notificationColor, int backgroundColor, boolean isDark) {
    ...
    public static synchronized int changeColorLightness(int baseColor, int amount) {
        double[] result = ColorUtilsFromCompat.getTempDouble3Array();
        ColorUtilsFromCompat.colorToLAB(baseColor, result);
        result[0] = Math.max(Math.min(100.0d, result[0] + amount), (double) FeatureOption.FO_BOOT_POLICY_CPU);
        return ColorUtilsFromCompat.LABToColor(result[0], result[1], result[2]);
    }

    public static synchronized int resolveAmbientColor(Context context, int notificationColor) {
        int resolvedColor = resolveColor(context, notificationColor);
        int color = ensureTextContrastOnBlack(resolvedColor);
        return color;
    }

    public static synchronized int resolvePrimaryColor(Context context, int backgroundColor) {
    ...
    public static synchronized int resolveSecondaryColor(Context context, int backgroundColor) {
        boolean useDark = shouldUseDark(backgroundColor);
        if (useDark) {
    ...
    public static synchronized int resolveDefaultColor(Context context, int backgroundColor) {
        boolean useDark = shouldUseDark(backgroundColor);
        if (useDark) {
            return context.getColor(R.color.notification_default_color_light);
        }
    ...
    public static synchronized int getShiftedColor(int color, int amount) {
        double[] result = ColorUtilsFromCompat.getTempDouble3Array();
        ColorUtilsFromCompat.colorToLAB(color, result);
        if (result[0] >= 4.0d) {
            result[0] = Math.max((double) FeatureOption.FO_BOOT_POLICY_CPU, result[0] - amount);
        } else {
    ...
    private static synchronized boolean shouldUseDark(int backgroundColor) {
    ...
    public static synchronized double calculateLuminance(int backgroundColor) {
        return ColorUtilsFromCompat.calculateLuminance(backgroundColor);
    }

    public static synchronized double calculateContrast(int foregroundColor, int backgroundColor) {
        return ColorUtilsFromCompat.calculateContrast(foregroundColor, backgroundColor);
    }

    public static synchronized boolean satisfiesTextContrast(int backgroundColor, int foregroundColor) {
        return calculateContrast(foregroundColor, backgroundColor) >= 4.5d;
    }

    public static synchronized int compositeColors(int foreground, int background) {
        return ColorUtilsFromCompat.compositeColors(foreground, background);
    }

    public static synchronized boolean isColorLight(int backgroundColor) {
    ...
    public static class ColorUtilsFromCompat {
        ...
        private synchronized ColorUtilsFromCompat() {
        }
        ...
        private static synchronized int compositeAlpha(int foregroundAlpha, int backgroundAlpha) {
            return 255 - (((255 - backgroundAlpha) * (255 - foregroundAlpha)) / 255);
        }

        private static synchronized int compositeComponent(int fgC, int fgA, int bgC, int bgA, int a) {
            if (a == 0) {
                return 0;
            }
        ...
        public static synchronized double calculateLuminance(int color) {
            double[] result = getTempDouble3Array();
            colorToXYZ(color, result);
            return result[1] / XYZ_WHITE_REFERENCE_Y;
        }

        public static synchronized double calculateContrast(int foreground, int background) {
        ...
        public static synchronized void colorToLAB(int color, double[] outLab) {
            RGBToLAB(Color.red(color), Color.green(color), Color.blue(color), outLab);
        }

        public static synchronized void RGBToLAB(int r, int g, int b, double[] outLab) {
            RGBToXYZ(r, g, b, outLab);
            XYZToLAB(outLab[0], outLab[1], outLab[2], outLab);
        }

        public static synchronized void colorToXYZ(int color, double[] outXyz) {
            RGBToXYZ(Color.red(color), Color.green(color), Color.blue(color), outXyz);
        }

        public static synchronized void RGBToXYZ(int r, int g, int b, double[] outXyz) {
            if (outXyz.length != 3) {
        ...
        public static synchronized void XYZToLAB(double x, double y, double z, double[] outLab) {
            if (outLab.length != 3) {
            ...
            outLab[0] = Math.max((double) FeatureOption.FO_BOOT_POLICY_CPU, (116.0d * y2) - 16.0d);
            outLab[1] = 500.0d * (x2 - y2);
            outLab[2] = 200.0d * (y2 - z2);
        }

        public static synchronized void LABToXYZ(double l, double a, double b, double[] outXyz) {
        ...
        public static synchronized int XYZToColor(double x, double y, double z) {
        ...
        public static synchronized int LABToColor(double l, double a, double b) {
            double[] result = getTempDouble3Array();
            LABToXYZ(l, a, b, result);
            return XYZToColor(result[0], result[1], result[2]);
        }

        private static synchronized int constrain(int amount, int low, int high) {
            return amount < low ? low : amount > high ? high : amount;
        }

        private static synchronized float constrain(float amount, float low, float high) {
            return amount < low ? low : amount > high ? high : amount;
        }

        private static synchronized double pivotXyzComponent(double component) {
        ...
        public static synchronized double[] getTempDouble3Array() {
        ...
        public static synchronized int HSLToColor(float[] hsl) {
            float h = hsl[0];
            float s = hsl[1];
            float l = hsl[2];
        ...
        public static synchronized void colorToHSL(int color, float[] outHsl) {
            RGBToHSL(Color.red(color), Color.green(color), Color.blue(color), outHsl);
        }

        public static synchronized void RGBToHSL(int r, int g, int b, float[] outHsl) {
            float h;
            float s;
            float rf = r / 255.0f;
            float gf = g / 255.0f;
