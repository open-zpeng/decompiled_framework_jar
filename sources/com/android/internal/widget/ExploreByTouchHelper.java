...
public abstract class ExploreByTouchHelper extends View.AccessibilityDelegate {
    ...
    protected abstract synchronized int getVirtualViewAt(float f, float f2);

    protected abstract synchronized void getVisibleVirtualViews(IntArray intArray);

    protected abstract synchronized boolean onPerformActionForVirtualView(int i, int i2, Bundle bundle);

    protected abstract synchronized void onPopulateEventForVirtualView(int i, AccessibilityEvent accessibilityEvent);

    protected abstract synchronized void onPopulateNodeForVirtualView(int i, AccessibilityNodeInfo accessibilityNodeInfo);

    public synchronized ExploreByTouchHelper(View forView) {
        if (forView == null) {
    ...
    public synchronized boolean dispatchHoverEvent(MotionEvent event) {
    ...
    public synchronized boolean sendEventForVirtualView(int virtualViewId, int eventType) {
        ViewParent parent;
    ...
    public synchronized void invalidateVirtualView(int virtualViewId) {
        invalidateVirtualView(virtualViewId, 0);
    }

    public synchronized void invalidateVirtualView(int virtualViewId, int changeTypes) {
        ViewParent parent;
        if (virtualViewId != Integer.MIN_VALUE && this.mManager.isEnabled() && (parent = this.mView.getParent()) != null) {
    ...
    public synchronized int getFocusedVirtualView() {
        return this.mFocusedVirtualViewId;
    }

    private synchronized void updateHoveredVirtualView(int virtualViewId) {
    ...
    private synchronized AccessibilityEvent createEvent(int virtualViewId, int eventType) {
        if (virtualViewId == -1) {
    ...
    private synchronized AccessibilityEvent createEventForHost(int eventType) {
    ...
    private synchronized AccessibilityEvent createEventForChild(int virtualViewId, int eventType) {
        AccessibilityEvent event = AccessibilityEvent.obtain(eventType);
    ...
    public synchronized AccessibilityNodeInfo createNode(int virtualViewId) {
        if (virtualViewId == -1) {
    ...
    private synchronized AccessibilityNodeInfo createNodeForHost() {
    ...
    private synchronized AccessibilityNodeInfo createNodeForChild(int virtualViewId) {
        ensureTempRects();
    ...
    private synchronized void ensureTempRects() {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public synchronized boolean performAction(int virtualViewId, int action, Bundle arguments) {
        if (virtualViewId == -1) {
    ...
    private synchronized boolean performActionForHost(int action, Bundle arguments) {
        return this.mView.performAccessibilityAction(action, arguments);
    }

    private synchronized boolean performActionForChild(int virtualViewId, int action, Bundle arguments) {
    ...
    private synchronized boolean manageFocusForChild(int virtualViewId, int action) {
        if (action != 64) {
            if (action == 128) {
    ...
    private synchronized boolean intersectVisibleToUser(Rect localRect) {
    ...
    private synchronized boolean isAccessibilityFocused(int virtualViewId) {
        return this.mFocusedVirtualViewId == virtualViewId;
    }

    private synchronized boolean requestAccessibilityFocus(int virtualViewId) {
    ...
    private synchronized boolean clearAccessibilityFocus(int virtualViewId) {
    ...
    protected synchronized void onPopulateEventForHost(AccessibilityEvent event) {
    }

    protected synchronized void onPopulateNodeForHost(AccessibilityNodeInfo node) {
    }
