...
public class DumpUtils {
    ...
    private static void writeConfiguration(DualDumpOutputStream dump, String idName, long id, UsbConfiguration configuration) {
        long token = dump.start(idName, id);
        dump.write(Instrumentation.REPORT_KEY_IDENTIFIER, 1120986464257L, configuration.getId());
    ...
    private static void writeInterface(DualDumpOutputStream dump, String idName, long id, UsbInterface iface) {
        long token = dump.start(idName, id);
        dump.write(Instrumentation.REPORT_KEY_IDENTIFIER, 1120986464257L, iface.getId());
    ...
    private static void writeEndpoint(DualDumpOutputStream dump, String idName, long id, UsbEndpoint endpoint) {
        ...
        dump.write("attributes", ConfigurationProto.SCREEN_LAYOUT, endpoint.getAttributes());
    ...
    public static void writePort(DualDumpOutputStream dump, String idName, long id, UsbPort port) {
        long token = dump.start(idName, id);
        dump.write(Instrumentation.REPORT_KEY_IDENTIFIER, 1138166333441L, port.getId());
        int mode = port.getSupportedModes();
        if (dump.isProto()) {
            if (mode == 0) {
                dump.write("supported_modes", NetworkCapabilitiesProto.CAPABILITIES, 0);
            } else {
                if ((mode & 3) == 3) {
                    dump.write("supported_modes", NetworkCapabilitiesProto.CAPABILITIES, 3);
                } else if ((mode & 2) == 2) {
                    dump.write("supported_modes", NetworkCapabilitiesProto.CAPABILITIES, 2);
                } else if ((mode & 1) == 1) {
                    dump.write("supported_modes", NetworkCapabilitiesProto.CAPABILITIES, 1);
                }
                if ((mode & 4) == 4) {
                    dump.write("supported_modes", NetworkCapabilitiesProto.CAPABILITIES, 4);
                }
                if ((mode & 8) == 8) {
                    dump.write("supported_modes", NetworkCapabilitiesProto.CAPABILITIES, 8);
                }
            }
        } else {
            dump.write("supported_modes", NetworkCapabilitiesProto.CAPABILITIES, UsbPort.modeToString(mode));
        }
        dump.end(token);
    }
